"""NB-Polar Phase 4-P20E N=32768 +1024 confirmation on new blocks (Stage A, frozen).

Zero-tuning confirmation (strategy stage 2, first confirmation round):
with the frozen Model-F concentration prior, the ``1e-15`` floor, the
accepted P16 construction order, the frozen kernel, representation,
transform and greedy SC all carried over byte-identical from P20C, and
the SAME +1024 L2 order-prefix step (``K2 6492 -> 7516``) on ``B1_L2plus``
unchanged, does +1024 repeat its P20C recovery and maintain exact on
independent NEW real development blocks -- or does it not? Nothing is
varied: the only deliberate differences from P20C are the new-block
population and the new P20E tag domain, both provenance/identity
changes, never algorithm changes.
...
Development population: same-file TRAIN subrange frames ``384..767`` of
the same registered 1M source (same pairs file identity as
P18/P19/P20B/P20C; the closed P18/P19 HOLD blocks ``1600..1983`` select
nothing and are never re-entered; the consumed P20B VAL pool
``1200..1599`` is never re-entered; the consumed P20C DEV blocks
``0..383`` are never re-entered). Three NEW DEV blocks ``384..511`` /
``512..639`` / ``640..767`` plus the declared never-used remainder
``768..1199``.
...
PROTOCOL_NAME = "nbpolar-p20e-plus1024-confirmation"
MODE = "dev-plus1024-confirmation"
...
# 1M source. Same pairs-file identity as P18/P19/P20B/P20C; disjoint from
# closed HOLD 1600..1983 AND from the consumed P20B VAL pool 1200..1599
# AND from the consumed P20C DEV blocks 0..383.
...
SEED_PREFIX = "nbpolar-p20e-plus1024-confirmation-seed"
...
class ArmSpec:
    """One frozen P20E arm (never CLI-tunable, never extensible)."""
...
COMPLETE_LABEL = "TARGET_EMPIRICAL_N32768_DEV_PLUS1024_CONFIRMATION_COMPLETE"
...
DEV_BLOCK_RULE = (
    "TRAIN development formation (P18 pattern, P20E ranges): sort TRAIN rows by "
    ...
    "768..1199 are the declared never-used remainder; closed HOLD 1600..1983, "
    "consumed VAL 1200..1599 and consumed P20C DEV 0..383 never selected; "
...
FROZEN_COMMAND = (
    ...
    "comparison_bench.src.comparison_bench.formal_ir.nbpolar.plus1024_confirmation "
    ...
    "--dev-frames 384 767 --block-frames 128 --remainder-frames 768 1199 "
...
class Plus1024ConfirmationContractError(ValueError):
    """Pre-open DEV-population/formation failure: BLOCKED(<earliest gate>)."""


class Plus1024ConfirmationResourceError(RuntimeError):
...
def verify_dev_block_identity() -> dict:
    """Pin the P20E DEV ranges and their triple disjointness gate.

    Pure code-level check (no I/O). The DEV ranges must overlap NONE of
    the consumed P20C DEV blocks 0..383, the consumed P20B VAL pool
    1200..1599, or the closed P18/P19 blocks 1600..1983; any overlap
    raises before any root exists and before either protected content
    open, consuming nothing. The consumed-P20C check runs first, then
    the consumed-VAL check, then the closed-HOLD check: all three are
    ...
    if not (rem_last < p20c_first or rem_first > p20c_last):
        raise ValueError("P20E DEV remainder overlaps consumed P20C DEV data")
    ...
    if not (rem_last < consumed_first or rem_first > consumed_last):
        raise ValueError("P20E DEV remainder overlaps consumed VAL data")
    ...
    if not (rem_last < closed_first or rem_first > closed_last):
        raise ValueError("P20E DEV remainder overlaps closed HOLD data")
    checks = {
        ...
        "remainder_frame_range": tuple(FROZEN_REMAINDER_FRAME_RANGE) == (768, 1199),
        "remainder_symbols": FROZEN_REMAINDER_SYMBOLS == 110592,
    }
    failing = sorted(name for name, ok in checks.items() if not ok)
    if failing:
        raise ValueError("P20E DEV block-range identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_manifest(path, *, source: str = FROZEN_SOURCE) -> dict:
    ...
    hosts the P20E DEV subrange (frames 384..767). Any mismatch raises
...
def plus1024_seed_bits(
    ...
    *,
    ...
    """Deterministic public per-(arm, block) Toeplitz seed (P20E domain).

    Rule: concatenate SHA-256 over
    ``"nbpolar-p20e-plus1024-confirmation-seed:<master>:<n>:<arm>:
    <block_index>:<counter>"`` for ``counter = 0, 1, ...`` (ASCII decimal),
    unpack each digest MSB-first and truncate to ``bit_length`` (default
    ``10*n + 63``). The P20E prefix and arm tokens differ from the P16, P17,
    P18, P19, P20B and P20C domains on purpose. Seed contents are public control
    ...
    if arm not in FROZEN_ARM_NAMES:
        raise ValueError(f"unknown frozen arm for the P20E seed domain: {arm!r}")
...
def form_dev_blocks(
    ...
    *,
    ...
    P18 ``form_holdout_blocks`` pattern with P20E ranges (the accepted helper
    ...
    try:
        ...
        raise TypeError(f"dev-frames must be a pair of integers: {exc}") from exc
    if pair != tuple(FROZEN_DEV_FRAME_RANGE):
        raise Plus1024ConfirmationContractError(
    ...
    # The declared never-used remainder (768..1199) lies OUTSIDE the DEV
    ...
    # exactly the frozen 432 frames 768..1199 with 110592 rows.
...
def plus1024_confirmation_label(gates: dict) -> str:
...
def _backoff_event(
    *, n, arm, block_index, frame_start, event_id, event_type, direction,
    ...
    event = {
        "event_id": event_id,
        "frame_key": (
            f"nbpolar-p20e-plus1024-confirmation:{int(n)}:{int(frame_start)}:{int(block_index)}"
        ),
        "method": "nbpolar_plus1024_confirmation",
...
class Plus1024ConfirmationRun:
    """In-memory P20E diagnostic run (also returned by the runner)."""
...
def _stub_plan(
    *, out_path, source, floor, n, k1, k2, construction, digest,
    ...
    plan = {
        ...
        "single_factor": "NOTHING varied: carried-over +1024 order-prefix step on B1; prior/floor/construction-order/kernel/representation/SC identical across arms; only the new-block population and new P20E tag domain differ",
        "deferred": ["positive-branch efficiency-optimization round", "negative-branch P20D fixed-disclosure alternative L2 construction"],
    }
    ...
    (out_path / "report.md").write_text("# P20E plus1024 confirmation (RUNNING)\n", encoding="utf-8")
    return plan
...
def _finalize_blocked(out_path, identity_doc, accounting, opened_paths, message: str) -> None:
    ...
    (out_path / "report.md").write_text(
        f"# P20E plus1024 confirmation\n\nBLOCKED({message})\n", encoding="utf-8")


def run_plus1024_confirmation(
    *,
    counts=None,
...
) -> Plus1024ConfirmationRun:
    """Execute the frozen P20E three-arm diagnostic once; write five files.
    ...
    try:
        ...
        outcome_label = plus1024_confirmation_label(gates)
        ...
        opf._write_json(out_path / "input_and_predecessor_identity.json", identity_doc)
        persist(summary)
        return Plus1024ConfirmationRun(summary=summary, records=tuple(records), gates=gates)
    except Plus1024ConfirmationResourceError as exc:
        ...
        try:
            ...
            pass
        raise Plus1024ConfirmationResourceError(label) from exc
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nbpolar-p20e-plus1024-confirmation",
        description=(
            "NB-Polar Phase 4-P20E N=32768 plus1024 confirmation "
...
def main(argv=None) -> int:
    ...
    try:
        ...
        print(f"nbpolar phase4-p20e plus1024 confirmation refused: {exc}", file=sys.stderr)
        return 2
    summary = run.summary
    print(
        json.dumps(
...
__all__ = [
    ...
    "Plus1024ConfirmationContractError", "Plus1024ConfirmationResourceError",
    "Plus1024ConfirmationRun", "check_frozen_arm_table",
    ...
    "plus1024_confirmation_label", "plus1024_block_events",
    "plus1024_recount_events", "run_plus1024_confirmation",
    "build_parser", "main",
]
