"""NB-Polar Phase 4-P20G N=32768 +1024 independent-session confirmation on type2_1p5M_20260121_183806 (Stage A, frozen).

Zero-tuning independent-session confirmation (strategy stage 2, second
half): with the frozen Model-F concentration prior, the ``1e-15``
...
from P20C/P20E/P20F, and the SAME +1024 L2 order-prefix step
(``K2 6492 -> 7516``) on ``B1_L2plus`` unchanged, does +1024 maintain
exact on three NEW independent-session blocks -- and does a
P20C-style restoration event (B0 fail -> B1 exact) replicate on genuinely
new data, or not? Nothing is varied: the only
deliberate differences from P20C/P20E/P20F are the new-session population
and the new P20G tag domain, both provenance/identity changes, never
algorithm changes.
...
PROTOCOL_NAME = "nbpolar-p20g-independent-session"
MODE = "dev-plus1024-independent-session"
...
FROZEN_TAG_MASTER = 2026092210
FROZEN_CHUNK_ROWS = 512
FROZEN_TAG_BITS = TAG_BITS  # 64
SEED_PREFIX = "nbpolar-p20g-independent-session-seed"
...
class ArmSpec:
    """One frozen P20G arm (never CLI-tunable, never extensible)."""
...
COMPLETE_LABEL = "TARGET_EMPIRICAL_N32768_DEV_PLUS1024_INDEPENDENT_SESSION_COMPLETE"
...
DEV_BLOCK_RULE = (
    "TRAIN development formation (P18 pattern, P20G ranges): sort TRAIN rows by "
...
CLAIM_SCOPE = (
    "descriptive zero-tuning +1024 confirmation on NEW independent-session "
    "blocks (1.5M TRAIN prefix, N=32768, three new DEV blocks); B2 is an "
...
FROZEN_COMMAND = (
    ...
    "comparison_bench.src.comparison_bench.formal_ir.nbpolar.plus1024_independent_session "
    f"--counts {FROZEN_COUNTS_PATH} --source 1p5M --floor 1e-15 "
    ...
    "--tag-master 2026092210 --chunk-rows 512 --tag-bits 64 "
...
class Plus1024IndependentSessionContractError(ValueError):
    """Pre-open DEV-population/formation failure: BLOCKED(<earliest gate>)."""


class Plus1024IndependentSessionResourceError(RuntimeError):
...
def verify_dev_source_identity(dev_pairs=None) -> dict:
    ...
    if candidate == REFUSED_1M_DEV_PAIRS_PATH:
        raise ValueError(
            "P20G DEV source gate: 1M full-pool path refused "
            f"({REFUSED_1M_DEV_PAIRS_PATH!r}); the entire 1M pool is "
            "fail-closed against P20G DEV selection"
        )
    if candidate == REFUSED_2M_DEV_PAIRS_PATH:
        raise ValueError(
            "P20G DEV source gate: reserved 2M path refused "
    ...
    if failing:
        raise ValueError("P20G DEV source identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_block_identity() -> dict:
    """Pin the P20G DEV ranges and their intra-file disjointness gate.
    ...
    if not (rem_last < val_first or rem_first > val_last):
        raise ValueError("P20G DEV remainder overlaps 1.5M VAL data")
    ...
    if not (rem_last < hold_first or rem_first > hold_last):
        raise ValueError("P20G DEV remainder overlaps 1.5M HOLD data")
    checks = {
    ...
    failing = sorted(name for name, ok in checks.items() if not ok)
    if failing:
        raise ValueError("P20G DEV block-range identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_manifest(path, *, source: str = FROZEN_SOURCE) -> dict:
    ...
    its validation line-for-line under the P20G source/pins: accepted
    schema, the 1.5M source tag, and exactly the TRAIN 1660/424960 +
    VAL 553/141568 + HOLD 554/141824 counts that host the P20G DEV prefix
...
def plus1024_seed_bits(
    ...
    *,
    ...
    """Deterministic public per-(arm, block) Toeplitz seed (P20G domain).

    Rule: concatenate SHA-256 over
    ``"nbpolar-p20g-independent-session-seed:<master>:<n>:<arm>:
    <block_index>:<counter>"`` for ``counter = 0, 1, ...`` (ASCII decimal),
    unpack each digest MSB-first and truncate to ``bit_length`` (default
    ``10*n + 63``). The P20G prefix and arm tokens differ from the P16, P17,
    ...
    if arm not in FROZEN_ARM_NAMES:
        raise ValueError(f"unknown frozen arm for the P20G seed domain: {arm!r}")
...
def form_dev_blocks(
    ...
    *,
    ...
    P18 ``form_holdout_blocks`` pattern with P20G ranges (the accepted helper
    ...
    try:
        ...
        raise TypeError(f"dev-frames must be a pair of integers: {exc}") from exc
    if pair != tuple(FROZEN_DEV_FRAME_RANGE):
        raise Plus1024IndependentSessionContractError(
...
def plus1024_independent_session_label(gates: dict) -> str:
...
def _backoff_event(
    *, n, arm, block_index, frame_start, event_id, event_type, direction,
    ...
    event = {
        "event_id": event_id,
        "frame_key": (
            f"nbpolar-p20g-independent-session:{int(n)}:{int(frame_start)}:{int(block_index)}"
        ),
        "method": "nbpolar_plus1024_independent_session",
...
class Plus1024IndependentSessionRun:
    """In-memory P20G diagnostic run (also returned by the runner)."""
...
def _stub_plan(
    *, out_path, source, floor, n, k1, k2, construction, digest,
    ...
    tag_master, chunk_rows, tag_bits, identity, manifest, dev_source_pin,
    dev_pin,
) -> dict:
    ...
    plan = {
        ...
        "single_factor": "NOTHING varied: carried-over +1024 order-prefix step on B1; prior/floor/construction-order/kernel/representation/SC identical across arms; only the new-session population and new P20G tag domain differ",
        "deferred": ["second independent session on the reserved 2M file", "disclosure-minimality probe (+512 vs +1024)", "negative-branch P20D fixed-disclosure alternative L2 construction", "positive-branch efficiency-optimization round"],
    }
    ...
    (out_path / "report.md").write_text("# P20G plus1024 independent session (RUNNING)\n", encoding="utf-8")
    return plan


def _render_report(summary: dict) -> str:
    lines = [
        "# NB-Polar Phase 4-P20G plus1024 independent session",
        "",
...
def _finalize_blocked(out_path, identity_doc, accounting, opened_paths, message: str) -> None:
    ...
    (out_path / "report.md").write_text(
        f"# P20G plus1024 extension\n\nBLOCKED({message})\n", encoding="utf-8")


def run_plus1024_independent_session(
    *,
    counts=None,
...
) -> Plus1024IndependentSessionRun:
    """Execute the frozen P20G three-arm diagnostic once; write five files.
    ...
    # protected content open; a mismatch consumes nothing. Gate order is
    ...
    try:
        ...
        outcome_label = plus1024_independent_session_label(gates)
        ...
        opf._write_json(out_path / "input_and_predecessor_identity.json", identity_doc)
        persist(summary)
        return Plus1024IndependentSessionRun(summary=summary, records=tuple(records), gates=gates)
    except Plus1024IndependentSessionResourceError as exc:
        ...
        try:
            ...
            pass
        raise Plus1024IndependentSessionResourceError(label) from exc
...
def _finalize_blocked_with_accounting(out_path, accounting, opened_paths, message: str) -> None:
    ...
    (Path(out_path) / "report.md").write_text(
        f"# P20G plus1024 independent session\n\nBLOCKED({message})\n", encoding="utf-8")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nbpolar-p20g-independent-session",
        description=(
            "NB-Polar Phase 4-P20G N=32768 plus1024 independent-session "
            "confirmation (frozen V25 counts, verified P16 construction, "
            "three NEW 1.5M TRAIN DEV blocks, three frozen arms)"
        ),
    )
...
def main(argv=None) -> int:
    ...
    try:
        run = run_plus1024_independent_session(
        ...
        print(f"nbpolar phase4-p20g plus1024 independent session refused: {exc}", file=sys.stderr)
        return 2
    summary = run.summary
    print(
        json.dumps(
...
__all__ = [
    ...
    "Plus1024IndependentSessionContractError", "Plus1024IndependentSessionResourceError",
    "Plus1024IndependentSessionRun", "check_frozen_arm_table",
    ...
    "plus1024_independent_session_label", "plus1024_block_events",
    "plus1024_recount_events", "run_plus1024_independent_session",
    "build_parser", "main",
]
