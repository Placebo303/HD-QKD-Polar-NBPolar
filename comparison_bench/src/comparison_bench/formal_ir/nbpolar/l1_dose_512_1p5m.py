"""NB-Polar Phase 4-P20K N=32768 L1-dose-512 +512 single-factor diagnostic on type2_1p5M_20260121_183806 (Stage A, frozen).

L1-dose-512 single-factor packet (strategy stage 2): with ONE
preregistered change against P20H/P20I/P20J -- L1 disclosure +ΔK1=512
via the frozen order-prefix rule (Stage-A frozen, Stage-B read-only) --
and everything else carried over from P20H (per-session prior
digest-pinned read-only, K2 base 6492, floor 1e-15, N=32768, greedy SC
only, new P20K tag domains only), zero further tuning: does +512 L1
restore any complete operational block on three NEW 1.5M TRAIN blocks
(1152..1535) -- and does the +256 full-deferral pattern continue or
saturate when L1 disclosure doubles again while L2 disclosure stays at
base? The only deliberate differences from P20H/P20I/P20J are the +512
L1 order-prefix step and the new P20K tag domain, both
provenance/identity changes under the frozen program, never algorithm
changes.

Three hardcoded arms (never CLI-tunable, never extensible):

- ``E0_sc_base``: frozen greedy SC at base disclosure/construction via
...
- ``E1_L1plus512``: frozen greedy SC at base + the frozen +512 L1 step
  via frozen-order-prefix extension (K1 319 -> 831 on the SAME frozen
...
0..383, the consumed P20I DEV segment 384..767 and the consumed P20J
DEV segment 768..1151
(frozen TRAIN base 0: frames ``1152..1535``; remaining 1.5M TRAIN
frames ``1536..1659`` are never used). Three NEW DEV blocks
``1152..1279`` / ``1280..1407`` / ``1408..1535`` plus the declared
never-used remainder ``1536..1659`` (124 frames / 31744 pairs, counted
...
the 1.5M VAL/HOLD frame sets (intra-file gate, checked second), NONE
of the consumed P20H DEV prefix ``0..383`` (P20H-DEV exclusion gate,
checked third), NONE of the consumed P20I DEV segment ``384..767``
(P20I-DEV exclusion gate, checked fourth) and NONE of the consumed
P20J DEV segment ``768..1151`` (P20J-DEV exclusion gate, checked
fifth); the Stage-B prior/table/literal digest must equal the §3
frozen digest before any SC call (calibration-identity gate, checked
sixth).
...
PROTOCOL_NAME = "nbpolar-p20k-l1-dose-512-1p5m"
MODE = "dev-l1-dose-512-1p5m"
...
# afterwards): E1 discloses FROZEN_L1_DELTA_K1 = +512 L1 symbols beyond
# the frozen base by frozen-order-prefix extension (the disclosed L1 set
# is the first FROZEN_K1_E1 positions of the frozen P16 L1 order --
# isomorphic with the accepted K2 prefix rule, no reselection, no
# re-ranking, on ANY data).
FROZEN_L1_DELTA_K1 = 512
FROZEN_K1_E1 = 831  # 319 + 512
FROZEN_K_TOTAL_E1 = 7323  # 831 + 6492
FROZEN_E1_KEY_BIT_DELTA = 2560  # 5 * 512 vs base
FROZEN_LEAKAGE_BITS = 34119  # 5 * 6811 + 64 (E0 base operational)
FROZEN_E1_LEAKAGE_BITS = 36679  # 5 * 7323 + 64 (E1 operational, +512 L1)
...
DISCLOSURE_RULE = (
    "single preregistered L1 disclosure step: E1_L1plus512 discloses "
    "FROZEN_L1_DELTA_K1 = +512 L1 symbols beyond the frozen base "
    "(K1 319 -> 831; K_total 6811 -> 7323) by FROZEN-ORDER-PREFIX "
    "EXTENSION -- the disclosed L1 set is the first 831 positions of "
    ...
    "0..383, P20I DEV 384..767, P20J DEV 768..1151, or new DEV data); "
    "E0 and E2 stay at base disclosure; the step doubles the P20J +256 "
    "L1 dose (K1 575 -> 831), completing the +128/+256/+512 chain; no "
    "second tier (no E1b) without a new packet"
)

# Development population: the NEXT 384 TRAIN frames of the independent
# 1.5M session file in (frame_id, pair_idx) order after the consumed
# P20H DEV prefix 0..383, the consumed P20I DEV segment 384..767 and
# the consumed P20J DEV segment 768..1151.
# Frozen TRAIN base 0 comes from JSON provenance only (split-manifest
# counts + frozen time-ordered split code + 0-based 1M precedent under
# the identical pipeline, carried over from P20H §3); the runner
# fail-closes on any layout drift before any SC call, so a wrong base
# BLOCKS instead of misattributing. The 1M full pool and the reserved
# 2M file are excluded by the cross-file source-tag+digest gate (never
# frame numbers alone); DEV must additionally overlap NONE of the 1.5M
# VAL/HOLD frame sets (intra-file gate, checked second), NONE of the
# consumed P20H DEV prefix 0..383 (P20H-DEV exclusion gate, checked
# third), NONE of the consumed P20I DEV segment 384..767 (P20I-DEV
# exclusion gate, checked fourth) and NONE of the consumed P20J DEV
# segment 768..1151 (P20J-DEV exclusion gate, checked fifth).
FROZEN_DEV_FRAME_RANGE = (1152, 1535)
...
SEED_PREFIX = "nbpolar-p20k-l1-dose-512-1p5m-seed"
...
class ArmSpec:
    """One frozen P20K arm (never CLI-tunable, never extensible)."""
...
FROZEN_ARMS = (
    ArmSpec("E0_sc_base", "operational", FROZEN_K1, FROZEN_K2,
            FROZEN_LEAKAGE_BITS, OPERATIONAL_PROVENANCE),
    ArmSpec("E1_L1plus512", "operational", FROZEN_K1_E1, FROZEN_K2,
            FROZEN_E1_LEAKAGE_BITS, OPERATIONAL_PROVENANCE),
    ArmSpec("E2_true_l1_diagnostic", "oracle_control", 0, FROZEN_K2,
...
)  # 309966 = 3*34119 + 3*36679 + 3*32524
...
# Cross-file source identity (sextuple gate, first): the DEV content open
...
COMPLETE_LABEL = "TARGET_EMPIRICAL_N32768_DEV_L1_DOSE_512_1P5M_COMPLETE"
...
DEV_BLOCK_RULE = (
    "TRAIN development formation (P18 pattern, P20K ranges): sort TRAIN rows by "
    "(frame_id, pair_idx); require exactly 384 frames 1152..1535, 256 rows "
    "per frame, pair_idx 0..255 and symbols 0..1023; blocks 1152..1279 / "
    "1280..1407 / 1408..1535 (128 frames = 32768 pairs each); frames "
    "1536..1659 are the declared never-used remainder; the 1M full pool "
    ...
    "768..1151 never selected (P20J-DEV exclusion gate fifth); "
    "no shuffle, resampling, "
    "overlap, padding, pooling or fitting"
)
ARM_RULE = (
    "three frozen arms in block-major order E0_sc_base / E1_L1plus512 / "
    "E2_true_l1_diagnostic per block; E0 and E1 run the frozen greedy SC "
    "(2 SC + 1 tag per block) with identical prior/floor/order/kernel/ "
    "representation/SC; E0 discloses the frozen base K1=319/K2=6492 while "
    "E1 discloses K1=831/K2=6492 (the single +512 L1 order-prefix step on "
    "the SAME frozen P16 L1 order); "
    "E2 discloses K2 only and runs one oracle-conditioned L2 SC; 15 SC "
    "calls and 9 tags total; E2 carries ORACLE_TRUE_L1_CONTROL provenance, "
...
TRUTH_BOUNDARY = (
    ...
    "and sentinel, identical for E0 and E1); E2 additionally conditions "
...
CLAIM_SCOPE = (
    "descriptive +512 L1-dose-512 diagnostic on new-segment 1.5M TRAIN "
    "blocks (N=32768, three DEV blocks 1152..1535) under the frozen "
    "per-session-calibrated prior; E2 is an "
...
FROZEN_COMMAND = (
    ...
    "comparison_bench.src.comparison_bench.formal_ir.nbpolar.l1_dose_512_1p5m "
    ...
    "--dev-frames 1152 1535 --block-frames 128 --remainder-frames 1536 1659 "
    "--tag-master 2026092250 --chunk-rows 512 --tag-bits 64 "
...
class L1Dose512_1p5mContractError(ValueError):
    """Pre-open DEV-population/formation failure: BLOCKED(<earliest gate>)."""


class L1Dose512_1p5mResourceError(RuntimeError):
...
def check_frozen_arm_table() -> dict:
    ...
    Pure code-level check (no I/O): arm order, K values, the single +512
    ...
    if FROZEN_ARM_NAMES != ("E0_sc_base", "E1_L1plus512", "E2_true_l1_diagnostic"):
    ...
    if (base.kind, base.k1, base.k2, base.leakage_bits, base.provenance) != (
        ...
        raise ValueError("frozen E0_sc_base arm drifted from base")
    plus = FROZEN_ARMS[1]
    if (plus.kind, plus.k1, plus.k2, plus.leakage_bits, plus.provenance) != (
        "operational", FROZEN_K1_E1, FROZEN_K2, FROZEN_E1_LEAKAGE_BITS,
        OPERATIONAL_PROVENANCE,
    ):
        raise ValueError("frozen E1_L1plus512 arm drifted from base + 512")
    if int(plus.k1) - int(base.k1) != FROZEN_L1_DELTA_K1:
        raise ValueError("frozen L1 disclosure step must be exactly +512")
    if int(plus.leakage_bits) - int(base.leakage_bits) != FROZEN_E1_KEY_BIT_DELTA:
        raise ValueError("frozen E1 key-bit delta must be exactly +2560 vs base")
    ...
    if (control.kind, control.k1, control.k2, control.provenance) != (
        ...
        raise ValueError("frozen E2_true_l1_diagnostic arm drifted")
    if FROZEN_K1 + FROZEN_K2 != FROZEN_K_TOTAL:
        raise ValueError("frozen base K1+K2 != K_total")
    if FROZEN_K1_E1 != FROZEN_K1 + FROZEN_L1_DELTA_K1:
        raise ValueError("frozen E1 K1 != base K1 + 512")
    if FROZEN_K1_E1 + FROZEN_K2 != FROZEN_K_TOTAL_E1:
        raise ValueError("frozen E1 K1+K2 != E1 K_total")
    ...
    if FROZEN_E1_LEAKAGE_BITS != DISCLOSED_BITS_PER_COORDINATE * FROZEN_K_TOTAL_E1 + TAG_BITS:
        raise ValueError("frozen E1 leakage != 5*(K1_E1+K2)+64")
    ...
    if FROZEN_L1_DELTA_K1 != 512:
        raise ValueError(f"frozen L1 disclosure step must be +512, got {FROZEN_L1_DELTA_K1}")
    ...
    e1_ratio = float(FROZEN_E1_LEAKAGE_BITS / FROZEN_RAW_INPUT_BITS)
    if not e1_ratio < 0.5:
        raise ValueError("frozen E1 cap must sit well below raw input bits")
    return {
        ...
        "e1_leakage_bits": FROZEN_E1_LEAKAGE_BITS,
        "e1_key_bit_delta_vs_base": FROZEN_E1_KEY_BIT_DELTA,
        "cap_vs_raw_ratio": cap_ratio,
        "e1_cap_vs_raw_ratio": e1_ratio,
    }


def verify_dev_source_identity(dev_pairs=None) -> dict:
    """Pin the 1.5M cross-file source identity (sextuple gate, FIRST).
    ...
    if candidate == REFUSED_1M_DEV_PAIRS_PATH:
        raise ValueError(
            "P20K DEV source gate: 1M full-pool path refused "
            f"({REFUSED_1M_DEV_PAIRS_PATH!r}); the entire 1M pool is "
            "fail-closed against P20K DEV selection"
        )
    if candidate == REFUSED_2M_DEV_PAIRS_PATH:
        raise ValueError(
            "P20K DEV source gate: reserved 2M path refused "
    ...
    if failing:
        raise ValueError("P20K DEV source identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_block_identity() -> dict:
    """Pin the P20K DEV ranges and their disjointness gates.

    Pure code-level check (no I/O). This is the sextuple gate's SECOND,
    THIRD, FOURTH and FIFTH parts: the cross-file source gate
    ...
    gate, checked third), NONE of the consumed P20I DEV segment
    384..767 (P20I-DEV exclusion gate, checked fourth) and NONE of the
    consumed P20J DEV segment 768..1151 (P20J-DEV exclusion gate,
    checked fifth); any overlap raises before any root exists and
    before either protected content open, consuming nothing. The VAL,
    HOLD, P20H-DEV, P20I-DEV and P20J-DEV checks run in that frozen
    order: all are safety properties and must fire even when the ranges
    also mismatch their literals.
    """
    ...
    if not (rem_last < val_first or rem_first > val_last):
        raise ValueError("P20K DEV remainder overlaps 1.5M VAL data")
    ...
    if not (rem_last < hold_first or rem_first > hold_last):
        raise ValueError("P20K DEV remainder overlaps 1.5M HOLD data")
    ...
    if not (rem_last < p20h_first or rem_first > p20h_last):
        raise ValueError("P20K DEV remainder overlaps consumed P20H DEV data")
    ...
    if not (rem_last < p20i_first or rem_first > p20i_last):
        raise ValueError("P20K DEV remainder overlaps consumed P20I DEV data")
    ...
    if not (rem_last < p20j_first or rem_first > p20j_last):
        raise ValueError("P20K DEV remainder overlaps consumed P20J DEV data")
    checks = {
        ...
        "dev_frame_range": tuple(FROZEN_DEV_FRAME_RANGE) == (1152, 1535),
        "dev_frames": FROZEN_DEV_FRAMES == 384,
        "dev_pairs": FROZEN_DEV_PAIRS == 98304,
        "block_ranges": tuple(tuple(r) for r in FROZEN_BLOCK_RANGES)
        == ((1152, 1279), (1280, 1407), (1408, 1535)),
        "remainder_frame_range": tuple(FROZEN_REMAINDER_FRAME_RANGE) == (1536, 1659),
        "remainder_frames": FROZEN_REMAINDER_FRAMES == 124,
        "remainder_symbols": FROZEN_REMAINDER_SYMBOLS == 31744,
    ...
    failing = sorted(name for name, ok in checks.items() if not ok)
    if failing:
        raise ValueError("P20K DEV block-range identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_manifest(path, *, source: str = FROZEN_SOURCE) -> dict:
    ...
    its validation line-for-line under the P20K source/pins: accepted
    schema, the 1.5M source tag, and exactly the TRAIN 1660/424960 +
    VAL 553/141568 + HOLD 554/141824 counts that host the P20K DEV segment
    (frames 1152..1535). Any mismatch raises before any root exists.
    """
...
def l1_dose_512_seed_bits(
    ...
    *,
    ...
    """Deterministic public per-(arm, block) Toeplitz seed (P20K domain).

    Rule: concatenate SHA-256 over
    ``"nbpolar-p20k-l1-dose-512-1p5m-seed:<master>:<n>:<arm>:
    <block_index>:<counter>"`` for ``counter = 0, 1, ...`` (ASCII decimal),
    unpack each digest MSB-first and truncate to ``bit_length`` (default
    ``10*n + 63``). The P20K prefix and arm tokens differ from the P16, P17,
    P18, P19, P20B, P20C, P20E, P20F, P20G, P20H, P20I and P20J domains on
    purpose.
    ...
    if arm not in FROZEN_ARM_NAMES:
        raise ValueError(f"unknown frozen arm for the P20K seed domain: {arm!r}")
...
def form_dev_blocks(
    ...
    *,
    ...
    P18 ``form_holdout_blocks`` pattern with P20K ranges (the accepted helper
    ...
    try:
        ...
        raise TypeError(f"dev-frames must be a pair of integers: {exc}") from exc
    if pair != tuple(FROZEN_DEV_FRAME_RANGE):
        raise L1Dose512_1p5mContractError(
    ...
    # The declared never-used remainder (1536..1659) lies OUTSIDE the DEV
    # selection by design (frozen --dev-frames 1152 1535), so its presence
    ...
    # exactly the frozen 124 frames 1536..1659 with 31744 rows.
...
def l1_prefix_positions(l1_order, k1: int) -> np.ndarray:
    ...
    P16 L1 order (E0: ``k1 = 319``; E1: ``k1 = 831``) -- isomorphic with
    ...
    Positions are fixed by the construction file, never selected on
    closed blocks, the consumed 1M pool, consumed P20H DEV, consumed
    P20I DEV, consumed P20J DEV, or new DEV data. Pure view helper;
    takes no decision input.
    """
...
def l2_prefix_positions(l2_order, k2: int) -> np.ndarray:
    ...
    L2 order (all P20K arms: ``k2 = 6492`` base). Positions are fixed
    ...
    P20J DEV, or new DEV data. Pure view helper; takes no decision input.
    """
...
def oracle_isolation_ok(records, *, final: bool) -> bool:
    """E2 provenance isolation: excluded from every operational aggregate."""
...
def recovery_diagnostics(records) -> dict:
    """Descriptive per-arm recovery plus the E1-vs-E0 disclosure comparison.
    ...
    disclosure_blocks = []
    e0_by_block = {r.get("block_index"): r for r in by_arm["E0_sc_base"]}
    e1_by_block = {r.get("block_index"): r for r in by_arm["E1_L1plus512"]}
    for block_index in sorted(set(e0_by_block) | set(e1_by_block)):
        r0 = e0_by_block.get(block_index, {})
        r1 = e1_by_block.get(block_index, {})
        disclosure_blocks.append({
            "block_index": int(block_index),
            "e0_exact": r0.get("exact"),
            "e0_outcome": r0.get("outcome"),
            "e1_exact": r1.get("exact"),
            "e1_outcome": r1.get("outcome"),
            "l1_delta_k1_applied": r1.get("l1_delta_k1_applied"),
            "e1_restored_given_e0_failed": bool(
                r1.get("exact") is True and r0.get("exact") is not True),
        })
    e1_records = by_arm["E1_L1plus512"]
    return {
        "per_arm": per_arm,
        "e1_restored_count": sum(
            1 for r in disclosure_blocks if r.get("e1_restored_given_e0_failed") is True),
...
def l1_dose_512_1p5m_label(gates: dict) -> str:
...
def _backoff_event(
    *, n, arm, block_index, frame_start, event_id, event_type, direction,
    ...
    event = {
        "event_id": event_id,
        "frame_key": (
            f"nbpolar-p20k-l1-dose-512-1p5m:{int(n)}:{int(frame_start)}:{int(block_index)}"
        ),
        "method": "nbpolar_l1_dose_512_1p5m",
...
def l1_dose_512_block_events(n: int, spec, block_index: int, frame_start: int, record) -> list:
...
def l1_dose_512_recount_events(events) -> dict:
...
class L1Dose512_1p5mRun:
    """In-memory P20K diagnostic run (also returned by the runner)."""
...
def _stub_plan(
    *, out_path, source, floor, n, k1, k2, construction, digest,
    ...
    plan = {
        ...
        "e1_k1": int(FROZEN_K1_E1),
        "e1_k_total": int(FROZEN_K_TOTAL_E1),
        ...
        "e1_leakage_bits": int(FROZEN_E1_LEAKAGE_BITS),
        "e1_key_bit_delta_vs_base": int(FROZEN_E1_KEY_BIT_DELTA),
        ...
        "single_factor": "ONE preregistered change vs P20H/P20I/P20J: L1 disclosure +512 via the frozen order-prefix rule (Stage-A frozen, read-only in Stage B); per-session prior digest-pinned read-only, K2 base, floor, construction-order, kernel, representation and SC carried over from P20H; only the +512 L1 step and the new P20K tag domain differ",
        "deferred": ["e1-restoration maintain-confirmation round", "e1-zero-restoration P20D fixed-disclosure alternative L2 construction", "disclosure-minimality probe (+128/+256/+512 chain complete)", "efficiency optimization round", "second independent session on the reserved 2M file"],
    }
    ...
    (out_path / "report.md").write_text("# P20K l1 dose 512 1p5m (RUNNING)\n", encoding="utf-8")
    return plan
...
def _render_report(summary: dict) -> str:
    lines = [
        ...
        f"k1={summary['k1']}, k2={summary['k2']}, E1 K1={summary['e1_k1']} "
        ...
        f"E1 cap: {summary['e1_leakage_bits']} key bits/block "
        f"(raw {summary['raw_input_bits_per_block']} bits; ratios "
        f"{summary['cap_vs_raw_ratio']:.6f} / {summary['e1_cap_vs_raw_ratio']:.6f})",
    ...
    lines += [
        ...
        "",
        f"- E1 restored a complete block where E0 failed on "
        f"{recovery['e1_restored_count']}/{len(recovery['disclosure_blocks'])} blocks",
    ]
    for row in recovery["disclosure_blocks"]:
        lines.append(
            f"- block {row['block_index']}: E0 {row['e0_outcome']} "
            f"(exact={row['e0_exact']}) vs E1 {row['e1_outcome']} "
            f"(exact={row['e1_exact']}, dK1={row['l1_delta_k1_applied']}, "
            f"restored={row['e1_restored_given_e0_failed']})"
        )
    lines += [
        "",
        "## Integrity",
...
def _integrity_gates(
    *, out_path, n, k1, k2, identity, manifest, dev_source_pin, dev_pin,
    ...
    # Block-major slot order: (E0,E1,E2) per block; each arm covers each block.
    expected_sequence = [
    ...
    try:
        recount = l1_dose_512_recount_events(events)
...
def _finalize_blocked(out_path, identity_doc, accounting, opened_paths, message: str) -> None:
    ...
    (out_path / "report.md").write_text(
        f"# P20K l1 dose 512 1p5m\n\nBLOCKED({message})\n", encoding="utf-8")


def run_l1_dose_512_1p5m(
    *,
    prior=None,
    ...
    """Execute the frozen P20K three-arm diagnostic once; write five files.
    ...
    counts NPZ is never opened in Stage B. The +512 L1 step on E1 is
    hardcoded (never CLI-tunable): E1 runs the accepted operational path
    with ``k1 = base k1 + 512`` on the same frozen L1 order
    ...
    # DEV-vs-VAL/HOLD gate second, consumed-P20H-DEV exclusion third,
    # consumed-P20I-DEV exclusion fourth, consumed-P20J-DEV exclusion
    # fifth, calibration-identity last.
    ...
    try:
        ...
        def partial_summary(outcome_label: str, gates: dict) -> dict:
            ...
            return {
                ...
                "e1_cap_vs_raw_ratio": float(FROZEN_E1_LEAKAGE_BITS / FROZEN_RAW_INPUT_BITS),
        ...
        for slot_index, (arm_index, spec, block_index, block) in enumerate(slots):
            ...
            frame_start = int(block["frame_start"])
            seed = l1_dose_512_seed_bits(tag_master, n, spec.name, block_index,
            ...
            if spec.kind == "operational":
                # E0 and E1 share the accepted operational path bit-for-bit
                # except the disclosed L1 prefix length: E0 runs k1=319
                # (base) while E1 runs k1=831 (the single frozen +512
            ...
            records.append(record)
            opf._append_jsonl(out_path / "per_block_arm_outcomes.jsonl", record)
            for event in l1_dose_512_block_events(n, spec, block_index, frame_start, record):
                events.append(event)
        ...
        outcome_label = l1_dose_512_1p5m_label(gates)
        ...
        opf._write_json(out_path / "input_and_predecessor_identity.json", identity_doc)
        persist(summary)
        return L1Dose512_1p5mRun(summary=summary, records=tuple(records), gates=gates)
    except L1Dose512_1p5mResourceError as exc:
        ...
        try:
            ...
            pass
        raise L1Dose512_1p5mResourceError(label) from exc
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nbpolar-p20k-l1-dose-512-1p5m",
        description=(
            "NB-Polar Phase 4-P20K N=32768 L1-dose-512 +512 single-factor "
...
def main(argv=None) -> int:
    ...
    try:
        ...
        print(f"nbpolar phase4-p20k l1 dose 512 1p5m refused: {exc}", file=sys.stderr)
        return 2
    summary = run.summary
    print(
        json.dumps(
            {
                ...
                "e1_restored_count": summary["aggregates"]["recovery"]["e1_restored_count"],
...
__all__ = [
    ...
    "FROZEN_K_TOTAL", "FROZEN_L1_DELTA_K1", "FROZEN_K1_E1", "FROZEN_K_TOTAL_E1",
    "FROZEN_E1_KEY_BIT_DELTA", "FROZEN_LEAKAGE_BITS", "FROZEN_E1_LEAKAGE_BITS",
    ...
    "L1Dose512_1p5mContractError", "L1Dose512_1p5mResourceError",
    "L1Dose512_1p5mRun", "check_frozen_arm_table",
    "check_frozen_calibration_pins", "verify_stage_b_prior",
    "verify_dev_source_identity", "verify_dev_block_identity",
    "verify_dev_manifest",
    "l1_dose_512_seed_bits", "form_dev_blocks", "candidate_nll_bits",
    ...
    "l1_dose_512_1p5m_label", "l1_dose_512_block_events",
    "l1_dose_512_recount_events", "run_l1_dose_512_1p5m",
    "build_parser", "main",
]
