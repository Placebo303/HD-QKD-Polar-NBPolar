"""NB-Polar Phase 4-P20C N=32768 L2 disclosure backoff (Stage A, frozen).

Single-factor diagnostic (strategy option 1): with the frozen Model-F
concentration prior, the ``1e-15`` floor, the accepted P16 construction
order, the frozen kernel, representation, transform and greedy SC all
held identical across arms, does ONE preregistered, practically
meaningful L2 disclosure step (``+1024`` L2 symbols by frozen-order
prefix extension, ``K2 6492 -> 7516``) recover complete low layers on
...
- ``B0_sc_base``: frozen greedy SC at base disclosure/construction via
  ...
  block).
- ``B1_L2plus``: frozen greedy SC at base + the ONE preregistered +1024
  L2 step via frozen-order-prefix extension, all else identical, via the
  accepted ``run_operational_block`` (operational, the single-factor
  delta; 2 SC + 1 tag per block).
...
Development population: same-file TRAIN frames ``0..1199`` of the same
registered 1M source (same pairs file identity as P18/P19/P20B; the
closed P18/P19 HOLD blocks ``1600..1983`` select nothing and are never
re-entered; the consumed P20B VAL pool ``1200..1599`` is never
re-entered). Three DEV blocks ``0..127`` / ``128..255`` / ``256..383``
plus the declared never-used remainder ``384..1199``.
...
PROTOCOL_NAME = "nbpolar-p20c-l2-disclosure-backoff"
MODE = "dev-l2-disclosure-backoff"
...
# Development population: same-file TRAIN frames of the registered 1M
# source. Same pairs-file identity as P18/P19/P20B; disjoint from closed
# HOLD 1600..1983 AND from the consumed P20B VAL pool 1200..1599.
FROZEN_DEV_FRAME_RANGE = (0, 1199)
FROZEN_DEV_FRAMES = 1200
FROZEN_DEV_PAIRS = 307200
...
SEED_PREFIX = "nbpolar-p20c-l2-disclosure-backoff-seed"
...
class ArmSpec:
    """One frozen P20C arm (never CLI-tunable, never extensible)."""
...
COMPLETE_LABEL = "TARGET_EMPIRICAL_N32768_DEV_L2_DISCLOSURE_BACKOFF_COMPLETE"
...
DEV_BLOCK_RULE = (
    "TRAIN development formation (P18 pattern, P20C ranges): sort TRAIN rows by "
    "(frame_id, pair_idx); require exactly 1200 frames 0..1199, 256 rows "
    "per frame, pair_idx 0..255 and symbols 0..1023; blocks 0..127 / "
    "128..255 / 256..383 (128 frames = 32768 pairs each); frames "
    "384..1199 are the declared never-used remainder; closed HOLD 1600..1983 "
    "and consumed VAL 1200..1599 never selected; no shuffle, resampling, "
...
CLAIM_SCOPE = (
    ...
    "real development data (same-file TRAIN, N=32768, three DEV blocks); B2 is an "
...
FROZEN_COMMAND = (
    ...
    "--dev-frames 0 1199 --block-frames 128 --remainder-frames 384 1199 "
    "--tag-master 2026092090 --chunk-rows 512 --tag-bits 64 "
...
class L2DisclosureBackoffContractError(ValueError):
    """Pre-open DEV-population/formation failure: BLOCKED(<earliest gate>)."""


class L2DisclosureBackoffResourceError(RuntimeError):
...
def verify_dev_block_identity() -> dict:
    """Pin the P20C DEV ranges and their double disjointness gate.

    Pure code-level check (no I/O). The DEV ranges must overlap NEITHER
    the closed P18/P19 blocks 1600..1983 NOR the consumed P20B VAL pool
    1200..1599; any overlap raises before any root exists and before
    either protected content open, consuming nothing. The
    closed-disjointness check runs first, then the consumed-pool check:
    both are safety properties and must fire even when the ranges also
    ...
    if not (rem_last < closed_first or rem_first > closed_last):
        raise ValueError("P20C DEV remainder overlaps closed HOLD data")
    ...
    if not (rem_last < consumed_first or rem_first > consumed_last):
        raise ValueError("P20C DEV remainder overlaps consumed VAL data")
    checks = {
        ...
        "dev_frame_range": tuple(FROZEN_DEV_FRAME_RANGE) == (0, 1199),
        "dev_frames": FROZEN_DEV_FRAMES == 1200,
        "dev_pairs": FROZEN_DEV_PAIRS == 307200,
        "block_ranges": tuple(tuple(r) for r in FROZEN_BLOCK_RANGES)
        == ((0, 127), (128, 255), (256, 383)),
        "remainder_frame_range": tuple(FROZEN_REMAINDER_FRAME_RANGE) == (384, 1199),
        "remainder_symbols": FROZEN_REMAINDER_SYMBOLS == 208896,
    }
    failing = sorted(name for name, ok in checks.items() if not ok)
    if failing:
        raise ValueError("P20C DEV block-range identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_manifest(path, *, source: str = FROZEN_SOURCE) -> dict:
    ...
    if int(manifest.get("train_frames", -1)) != FROZEN_DEV_FRAMES or int(
...
def l2_backoff_seed_bits(
    ...
    *,
    ...
    """Deterministic public per-(arm, block) Toeplitz seed (P20C domain).

    Rule: concatenate SHA-256 over
    ``"nbpolar-p20c-l2-disclosure-backoff-seed:<master>:<n>:<arm>:
    <block_index>:<counter>"`` for ``counter = 0, 1, ...`` (ASCII decimal),
    unpack each digest MSB-first and truncate to ``bit_length`` (default
    ``10*n + 63``). The P20C prefix and arm tokens differ from the P16, P17,
    P18, P19 and P20B domains on purpose. Seed contents are public control
    ...
    if arm not in FROZEN_ARM_NAMES:
        raise ValueError(f"unknown frozen arm for the P20C seed domain: {arm!r}")
...
def form_dev_blocks(
    ...
    *,
    ...
    P18 ``form_holdout_blocks`` pattern with P20C ranges (the accepted helper
    ...
    try:
        ...
        raise TypeError(f"dev-frames must be a pair of integers: {exc}") from exc
    if pair != tuple(FROZEN_DEV_FRAME_RANGE):
        raise L2DisclosureBackoffContractError(
...
def l2_disclosure_backoff_label(gates: dict) -> str:
...
def _backoff_event(
    *, n, arm, block_index, frame_start, event_id, event_type, direction,
    ...
    event = {
        "event_id": event_id,
        "frame_key": (
            f"nbpolar-p20c-l2-disclosure-backoff:{int(n)}:{int(frame_start)}:{int(block_index)}"
        ),
        "method": "nbpolar_l2_disclosure_backoff",
...
def l2_backoff_block_events(n: int, spec, block_index: int, frame_start: int, record) -> list:
...
def l2_backoff_recount_events(events) -> dict:
...
class L2DisclosureBackoffRun:
    """In-memory P20C diagnostic run (also returned by the runner)."""
...
def _stub_plan(
    *, out_path, source, floor, n, k1, k2, construction, digest,
    ...
    plan = {
        ...
        "single_factor": "L2 disclosure only (+1024 order-prefix step on B1); prior/floor/construction-order/kernel/representation/SC frozen",
        "deferred": ["P20D fixed-disclosure alternative L2 construction"],
    }
    ...
    (out_path / "report.md").write_text("# P20C L2 disclosure backoff (RUNNING)\n", encoding="utf-8")
    return plan
...
def _integrity_gates(
    *, out_path, n, k1, k2, identity, manifest, dev_pin, l1_order, l2_order,
    ...
    manifest_ok = bool(
        ...
        and int(manifest.get("dev_frames", -1)) == FROZEN_DEV_FRAMES
        and int(manifest.get("dev_pairs", -1)) == FROZEN_DEV_PAIRS
    )
    try:
    ...
    try:
        recount = l2_backoff_recount_events(events)
...
def _finalize_blocked(out_path, identity_doc, accounting, opened_paths, message: str) -> None:
    ...
    (out_path / "report.md").write_text(
        f"# P20C L2 disclosure backoff\n\nBLOCKED({message})\n", encoding="utf-8")


def run_l2_disclosure_backoff(
    *,
    counts=None,
...
) -> L2DisclosureBackoffRun:
    """Execute the frozen P20C three-arm diagnostic once; write five files.
    ...
    try:
        ...
        for slot_index, (arm_index, spec, block_index, block) in enumerate(slots):
            ...
            frame_start = int(block["frame_start"])
            seed = l2_backoff_seed_bits(tag_master, n, spec.name, block_index,
            ...
            records.append(record)
            opf._append_jsonl(out_path / "per_block_arm_outcomes.jsonl", record)
            for event in l2_backoff_block_events(n, spec, block_index, frame_start, record):
                events.append(event)
        ...
        outcome_label = l2_disclosure_backoff_label(gates)
        ...
        opf._write_json(out_path / "input_and_predecessor_identity.json", identity_doc)
        persist(summary)
        return L2DisclosureBackoffRun(summary=summary, records=tuple(records), gates=gates)
    except L2DisclosureBackoffResourceError as exc:
        ...
        try:
            ...
            pass
        raise L2DisclosureBackoffResourceError(label) from exc
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nbpolar-p20c-l2-disclosure-backoff",
        description=(
            "NB-Polar Phase 4-P20C N=32768 L2 disclosure backoff "
            "(frozen V25 counts, verified P16 construction, three TRAIN DEV "
...
def main(argv=None) -> int:
    ...
    try:
        ...
        print(f"nbpolar phase4-p20c L2 disclosure backoff refused: {exc}", file=sys.stderr)
        return 2
    summary = run.summary
    print(
        json.dumps(
...
__all__ = [
    ...
    "L2DisclosureBackoffContractError", "L2DisclosureBackoffResourceError",
    "L2DisclosureBackoffRun", "check_frozen_arm_table",
    "verify_dev_block_identity", "verify_dev_manifest",
    "l2_backoff_seed_bits", "form_dev_blocks", "candidate_nll_bits",
    ...
    "l2_disclosure_backoff_label", "l2_backoff_block_events",
    "l2_backoff_recount_events", "run_l2_disclosure_backoff",
    "build_parser", "main",
]
