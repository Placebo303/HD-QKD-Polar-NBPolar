"""NB-Polar Phase 4-P20J N=32768 L1-dose-escalation +256 single-factor diagnostic on type2_1p5M_20260121_183806 (Stage A, frozen).

L1-dose-escalation single-factor packet (strategy stage 2): with ONE
preregistered change against P20H/P20I -- L1 disclosure +ΔK1=256 via the
...
only, new P20J tag domains only), zero further tuning: does +256 L1
restore any complete operational block on three NEW 1.5M TRAIN blocks
(768..1151) -- and where does the first error move when L1 disclosure
grows while L2 disclosure stays at base? The only deliberate
differences from P20H/P20I are the +256 L1 order-prefix step and the new
P20J tag domain, both provenance/identity changes under the frozen
program, never algorithm changes.

Three hardcoded arms (never CLI-tunable, never extensible):

- ``D0_sc_base``: frozen greedy SC at base disclosure/construction via
  the accepted ``run_operational_block`` (operational, 2 SC + 1 tag per
  block).
- ``D1_L1plus``: frozen greedy SC at base + the frozen +256 L1 step
  via frozen-order-prefix extension (K1 319 -> 575 on the SAME frozen
...
- ``D2_true_l1_diagnostic``: true-L1-conditioned L2 at BASE disclosure
...
0..383 and the consumed P20I DEV segment 384..767
(frozen TRAIN base 0: frames ``768..1151``; remaining 1.5M TRAIN
frames ``1152..1659`` are never used). Three NEW DEV blocks
``768..895`` / ``896..1023`` / ``1024..1151`` plus the declared
never-used remainder ``1152..1659`` (508 frames / 130048 pairs, counted
...
checked third) and NONE of the consumed P20I DEV segment ``384..767``
(P20I-DEV exclusion gate, checked fourth); the Stage-B
prior/table/literal digest must equal the §3 frozen digest before any
SC call (calibration-identity gate, checked fifth).
...
PROTOCOL_NAME = "nbpolar-p20j-l1-dose-escalation-1p5m"
MODE = "dev-l1-dose-escalation-1p5m"
...
# afterwards): D1 discloses FROZEN_L1_DELTA_K1 = +256 L1 symbols beyond
# the frozen base by frozen-order-prefix extension (the disclosed L1 set
# is the first FROZEN_K1_D1 positions of the frozen P16 L1 order --
# isomorphic with the accepted K2 prefix rule, no reselection, no
# re-ranking, on ANY data).
FROZEN_L1_DELTA_K1 = 256
FROZEN_K1_D1 = 575  # 319 + 256
FROZEN_K_TOTAL_D1 = 7067  # 575 + 6492
FROZEN_D1_KEY_BIT_DELTA = 1280  # 5 * 256 vs base
FROZEN_LEAKAGE_BITS = 34119  # 5 * 6811 + 64 (D0 base operational)
FROZEN_D1_LEAKAGE_BITS = 35399  # 5 * 7067 + 64 (D1 operational, +256 L1)
...
DISCLOSURE_RULE = (
    "single preregistered L1 disclosure step: D1_L1plus discloses "
    "FROZEN_L1_DELTA_K1 = +256 L1 symbols beyond the frozen base "
    "(K1 319 -> 575; K_total 6811 -> 7067) by FROZEN-ORDER-PREFIX "
    "EXTENSION -- the disclosed L1 set is the first 575 positions of "
    ...
    "0..383, P20I DEV 384..767, or new DEV data); D0 and D2 stay at "
    "base disclosure; the step doubles the P20I +128 L1 dose "
    "(K1 447 -> 575); no second tier (no D1b) without a new packet"
)

# Development population: the NEXT 384 TRAIN frames of the independent
# 1.5M session file in (frame_id, pair_idx) order after the consumed
# P20H DEV prefix 0..383 and the consumed P20I DEV segment 384..767.
...
# third) and NONE of the consumed P20I DEV segment 384..767 (P20I-DEV
# exclusion gate, checked fourth).
...
FROZEN_REMAINDER_FRAME_RANGE = (1152, 1659)
FROZEN_REMAINDER_FRAMES = 508
...
FROZEN_TAG_MASTER = 2026092240
FROZEN_CHUNK_ROWS = 512
FROZEN_TAG_BITS = TAG_BITS  # 64
SEED_PREFIX = "nbpolar-p20j-l1-dose-escalation-1p5m-seed"
...
class ArmSpec:
    """One frozen P20J arm (never CLI-tunable, never extensible)."""
...
FROZEN_ARMS = (
    ArmSpec("D0_sc_base", "operational", FROZEN_K1, FROZEN_K2,
            FROZEN_LEAKAGE_BITS, OPERATIONAL_PROVENANCE),
    ArmSpec("D1_L1plus", "operational", FROZEN_K1_D1, FROZEN_K2,
            FROZEN_D1_LEAKAGE_BITS, OPERATIONAL_PROVENANCE),
    ArmSpec("D2_true_l1_diagnostic", "oracle_control", 0, FROZEN_K2,
...
)  # 306126 = 3*34119 + 3*35399 + 3*32524
...
# Cross-file source identity (quintuple gate, first): the DEV content open
...
COMPLETE_LABEL = "TARGET_EMPIRICAL_N32768_DEV_L1_DOSE_ESCALATION_1P5M_COMPLETE"
...
DEV_BLOCK_RULE = (
    "TRAIN development formation (P18 pattern, P20J ranges): sort TRAIN rows by "
    ...
    "1152..1659 are the declared never-used remainder; the 1M full pool "
    ...
    "selected (P20I-DEV exclusion gate fourth); "
    "no shuffle, resampling, "
    "overlap, padding, pooling or fitting"
)
ARM_RULE = (
    "three frozen arms in block-major order D0_sc_base / D1_L1plus / "
    "D2_true_l1_diagnostic per block; D0 and D1 run the frozen greedy SC "
    "(2 SC + 1 tag per block) with identical prior/floor/order/kernel/ "
    "representation/SC; D0 discloses the frozen base K1=319/K2=6492 while "
    "D1 discloses K1=575/K2=6492 (the single +256 L1 order-prefix step on "
    "the SAME frozen P16 L1 order); "
    "D2 discloses K2 only and runs one oracle-conditioned L2 SC; 15 SC "
...
TRUTH_BOUNDARY = (
    ...
    "and sentinel, identical for D0 and D1); D2 additionally conditions "
...
CLAIM_SCOPE = (
    "descriptive +256 L1-dose-escalation diagnostic on new-segment 1.5M TRAIN "
    "blocks (N=32768, three DEV blocks 768..1151) under the frozen "
...
FROZEN_COMMAND = (
    ...
    "comparison_bench.src.comparison_bench.formal_ir.nbpolar.l1_dose_escalation_1p5m "
    ...
    "--dev-frames 768 1151 --block-frames 128 --remainder-frames 1152 1659 "
    "--tag-master 2026092240 --chunk-rows 512 --tag-bits 64 "
...
class L1DoseEscalation1p5mContractError(ValueError):
    """Pre-open DEV-population/formation failure: BLOCKED(<earliest gate>)."""


class L1DoseEscalation1p5mResourceError(RuntimeError):
...
def check_frozen_arm_table() -> dict:
    ...
    Pure code-level check (no I/O): arm order, K values, the single +256
    ...
    if FROZEN_ARM_NAMES != ("D0_sc_base", "D1_L1plus", "D2_true_l1_diagnostic"):
    ...
    if (base.kind, base.k1, base.k2, base.leakage_bits, base.provenance) != (
        ...
        raise ValueError("frozen D0_sc_base arm drifted from base")
    plus = FROZEN_ARMS[1]
    if (plus.kind, plus.k1, plus.k2, plus.leakage_bits, plus.provenance) != (
        "operational", FROZEN_K1_D1, FROZEN_K2, FROZEN_D1_LEAKAGE_BITS,
        OPERATIONAL_PROVENANCE,
    ):
        raise ValueError("frozen D1_L1plus arm drifted from base + 256")
    if int(plus.k1) - int(base.k1) != FROZEN_L1_DELTA_K1:
        raise ValueError("frozen L1 disclosure step must be exactly +256")
    if int(plus.leakage_bits) - int(base.leakage_bits) != FROZEN_D1_KEY_BIT_DELTA:
        raise ValueError("frozen D1 key-bit delta must be exactly +1280 vs base")
    ...
    if (control.kind, control.k1, control.k2, control.provenance) != (
        ...
        raise ValueError("frozen D2_true_l1_diagnostic arm drifted")
    if FROZEN_K1 + FROZEN_K2 != FROZEN_K_TOTAL:
        raise ValueError("frozen base K1+K2 != K_total")
    if FROZEN_K1_D1 != FROZEN_K1 + FROZEN_L1_DELTA_K1:
        raise ValueError("frozen D1 K1 != base K1 + 256")
    if FROZEN_K1_D1 + FROZEN_K2 != FROZEN_K_TOTAL_D1:
        raise ValueError("frozen D1 K1+K2 != D1 K_total")
    ...
    if FROZEN_D1_LEAKAGE_BITS != DISCLOSED_BITS_PER_COORDINATE * FROZEN_K_TOTAL_D1 + TAG_BITS:
        raise ValueError("frozen D1 leakage != 5*(K1_D1+K2)+64")
    ...
    if FROZEN_L1_DELTA_K1 != 256:
        raise ValueError(f"frozen L1 disclosure step must be +256, got {FROZEN_L1_DELTA_K1}")
    ...
    d1_ratio = float(FROZEN_D1_LEAKAGE_BITS / FROZEN_RAW_INPUT_BITS)
    if not d1_ratio < 0.5:
        raise ValueError("frozen D1 cap must sit well below raw input bits")
    return {
        ...
        "d1_leakage_bits": FROZEN_D1_LEAKAGE_BITS,
        "d1_key_bit_delta_vs_base": FROZEN_D1_KEY_BIT_DELTA,
        "cap_vs_raw_ratio": cap_ratio,
        "d1_cap_vs_raw_ratio": d1_ratio,
    }


def verify_dev_source_identity(dev_pairs=None) -> dict:
    """Pin the 1.5M cross-file source identity (quintuple gate, FIRST).
    ...
    if candidate == REFUSED_1M_DEV_PAIRS_PATH:
        raise ValueError(
            "P20J DEV source gate: 1M full-pool path refused "
            f"({REFUSED_1M_DEV_PAIRS_PATH!r}); the entire 1M pool is "
            "fail-closed against P20J DEV selection"
        )
    if candidate == REFUSED_2M_DEV_PAIRS_PATH:
        raise ValueError(
            "P20J DEV source gate: reserved 2M path refused "
    ...
    if failing:
        raise ValueError("P20J DEV source identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_block_identity() -> dict:
    """Pin the P20J DEV ranges and their disjointness gates.

    Pure code-level check (no I/O). This is the quintuple gate's SECOND,
    THIRD and FOURTH parts: the cross-file source gate
    ...
    gate, checked third) and NONE of the consumed P20I DEV segment
    384..767 (P20I-DEV exclusion gate, checked fourth); any overlap
    raises before any root exists and before either protected content
    open, consuming nothing. The VAL, HOLD, P20H-DEV and P20I-DEV checks
    run in that frozen order: all are safety properties and must fire
    even when the ranges also mismatch their literals.
    """
    ...
    if not (rem_last < val_first or rem_first > val_last):
        raise ValueError("P20J DEV remainder overlaps 1.5M VAL data")
    ...
    if not (rem_last < hold_first or rem_first > hold_last):
        raise ValueError("P20J DEV remainder overlaps 1.5M HOLD data")
    ...
    if not (rem_last < p20h_first or rem_first > p20h_last):
        raise ValueError("P20J DEV remainder overlaps consumed P20H DEV data")
    ...
    if not (rem_last < p20i_first or rem_first > p20i_last):
        raise ValueError("P20J DEV remainder overlaps consumed P20I DEV data")
    checks = {
        ...
        "remainder_frame_range": tuple(FROZEN_REMAINDER_FRAME_RANGE) == (1152, 1659),
        "remainder_frames": FROZEN_REMAINDER_FRAMES == 508,
        "remainder_symbols": FROZEN_REMAINDER_SYMBOLS == 130048,
    ...
    failing = sorted(name for name, ok in checks.items() if not ok)
    if failing:
        raise ValueError("P20J DEV block-range identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_manifest(path, *, source: str = FROZEN_SOURCE) -> dict:
    ...
    its validation line-for-line under the P20J source/pins: accepted
    schema, the 1.5M source tag, and exactly the TRAIN 1660/424960 +
    VAL 553/141568 + HOLD 554/141824 counts that host the P20J DEV segment
    (frames 768..1151). Any mismatch raises before any root exists.
    """
...
def l1_dose_escalation_seed_bits(
    master: int,
    n: int,
    arm: str,
    block_index: int,
    *,
    ...
    """Deterministic public per-(arm, block) Toeplitz seed (P20J domain).

    Rule: concatenate SHA-256 over
    ``"nbpolar-p20j-l1-dose-escalation-1p5m-seed:<master>:<n>:<arm>:
    <block_index>:<counter>"`` for ``counter = 0, 1, ...`` (ASCII decimal),
    unpack each digest MSB-first and truncate to ``bit_length`` (default
    ``10*n + 63``). The P20J prefix and arm tokens differ from the P16, P17,
    P18, P19, P20B, P20C, P20E, P20F, P20G, P20H and P20I domains on purpose.
    ...
    if arm not in FROZEN_ARM_NAMES:
        raise ValueError(f"unknown frozen arm for the P20J seed domain: {arm!r}")
...
def form_dev_blocks(
    ...
    *,
    ...
    P18 ``form_holdout_blocks`` pattern with P20J ranges (the accepted helper
    ...
    try:
        ...
        raise TypeError(f"dev-frames must be a pair of integers: {exc}") from exc
    if pair != tuple(FROZEN_DEV_FRAME_RANGE):
        raise L1DoseEscalation1p5mContractError(
    ...
    # The declared never-used remainder (1152..1659) lies OUTSIDE the DEV
    ...
    # exactly the frozen 508 frames 1152..1659 with 130048 rows.
...
def l1_prefix_positions(l1_order, k1: int) -> np.ndarray:
    ...
    P16 L1 order (D0: ``k1 = 319``; D1: ``k1 = 575``) -- isomorphic with
    the accepted K2 prefix rule, no reselection, no re-ranking, on ANY
    data (P20J DEV, P20I DEV, P20H DEV, closed 1M ranges, 2M). Positions
    are fixed by the construction file, never selected on closed blocks,
    the consumed 1M pool, consumed P20H DEV, consumed P20I DEV, or new
    DEV data. Pure view helper; takes no decision input.
    """
...
def l2_prefix_positions(l2_order, k2: int) -> np.ndarray:
    ...
    L2 order (all P20J arms: ``k2 = 6492`` base). Positions are fixed
    by the construction file, never selected on closed blocks, the
    consumed 1M pool, consumed P20H DEV, consumed P20I DEV, or new DEV
...
def oracle_isolation_ok(records, *, final: bool) -> bool:
    """D2 provenance isolation: excluded from every operational aggregate."""
...
def recovery_diagnostics(records) -> dict:
    """Descriptive per-arm recovery plus the D1-vs-D0 disclosure comparison.
    ...
    disclosure_blocks = []
    d0_by_block = {r.get("block_index"): r for r in by_arm["D0_sc_base"]}
    d1_by_block = {r.get("block_index"): r for r in by_arm["D1_L1plus"]}
    for block_index in sorted(set(d0_by_block) | set(d1_by_block)):
        r0 = d0_by_block.get(block_index, {})
        r1 = d1_by_block.get(block_index, {})
        disclosure_blocks.append({
            "block_index": int(block_index),
            "d0_exact": r0.get("exact"),
            "d0_outcome": r0.get("outcome"),
            "d1_exact": r1.get("exact"),
            "d1_outcome": r1.get("outcome"),
            "l1_delta_k1_applied": r1.get("l1_delta_k1_applied"),
            "d1_restored_given_d0_failed": bool(
                r1.get("exact") is True and r0.get("exact") is not True),
        })
    d1_records = by_arm["D1_L1plus"]
    return {
        "per_arm": per_arm,
        "d1_restored_count": sum(
            1 for r in disclosure_blocks if r.get("d1_restored_given_d0_failed") is True),
...
def l1_dose_escalation_1p5m_label(gates: dict) -> str:
...
def _backoff_event(
    *, n, arm, block_index, frame_start, event_id, event_type, direction,
    ...
    event = {
        "event_id": event_id,
        "frame_key": (
            f"nbpolar-p20j-l1-dose-escalation-1p5m:{int(n)}:{int(frame_start)}:{int(block_index)}"
        ),
        "method": "nbpolar_l1_dose_escalation_1p5m",
...
def l1_dose_escalation_block_events(n: int, spec, block_index: int, frame_start: int, record) -> list:
...
def l1_dose_escalation_recount_events(events) -> dict:
...
class L1DoseEscalation1p5mRun:
    """In-memory P20J diagnostic run (also returned by the runner)."""
...
def _stub_plan(
    *, out_path, source, floor, n, k1, k2, construction, digest,
    ...
    plan = {
        ...
        "d1_k1": int(FROZEN_K1_D1),
        "d1_k_total": int(FROZEN_K_TOTAL_D1),
        ...
        "d1_leakage_bits": int(FROZEN_D1_LEAKAGE_BITS),
        "d1_key_bit_delta_vs_base": int(FROZEN_D1_KEY_BIT_DELTA),
        ...
        "single_factor": "ONE preregistered change vs P20H/P20I: L1 disclosure +256 via the frozen order-prefix rule (Stage-A frozen, read-only in Stage B); per-session prior digest-pinned read-only, K2 base, floor, construction-order, kernel, representation and SC carried over from P20H; only the +256 L1 step and the new P20J tag domain differ",
        "deferred": ["d1-restoration maintain-confirmation round", "d1-zero-restoration P20D fixed-disclosure alternative L2 construction", "disclosure-minimality probe (+64 vs +128 vs +256 vs larger)", "efficiency optimization round", "second independent session on the reserved 2M file"],
    }
    ...
    (out_path / "report.md").write_text("# P20J l1 dose escalation 1p5m (RUNNING)\n", encoding="utf-8")
    return plan
...
def _render_report(summary: dict) -> str:
    lines = [
        ...
        f"k1={summary['k1']}, k2={summary['k2']}, D1 K1={summary['d1_k1']} "
        ...
        f"D1 cap: {summary['d1_leakage_bits']} key bits/block "
        f"(raw {summary['raw_input_bits_per_block']} bits; ratios "
        f"{summary['cap_vs_raw_ratio']:.6f} / {summary['d1_cap_vs_raw_ratio']:.6f})",
    ...
    lines += [
        ...
        "",
        f"- D1 restored a complete block where D0 failed on "
        f"{recovery['d1_restored_count']}/{len(recovery['disclosure_blocks'])} blocks",
    ]
    for row in recovery["disclosure_blocks"]:
        lines.append(
            f"- block {row['block_index']}: D0 {row['d0_outcome']} "
            f"(exact={row['d0_exact']}) vs D1 {row['d1_outcome']} "
            f"(exact={row['d1_exact']}, dK1={row['l1_delta_k1_applied']}, "
            f"restored={row['d1_restored_given_d0_failed']})"
        )
    lines += [
        "",
        "## Integrity",
...
def _integrity_gates(
    *, out_path, n, k1, k2, identity, manifest, dev_source_pin, dev_pin,
    ...
    # Block-major slot order: (D0,D1,D2) per block; each arm covers each block.
    expected_sequence = [
    ...
    try:
        recount = l1_dose_escalation_recount_events(events)
...
def _finalize_blocked(out_path, identity_doc, accounting, opened_paths, message: str) -> None:
    ...
    (out_path / "report.md").write_text(
        f"# P20J l1 dose escalation 1p5m\n\nBLOCKED({message})\n", encoding="utf-8")


def run_l1_dose_escalation_1p5m(
    *,
    prior=None,
...
) -> L1DoseEscalation1p5mRun:
    """Execute the frozen P20J three-arm diagnostic once; write five files.
    ...
    counts NPZ is never opened in Stage B. The +256 L1 step on D1 is
    hardcoded (never CLI-tunable): D1 runs the accepted operational path
    with ``k1 = base k1 + 256`` on the same frozen L1 order
    ...
    # consumed-P20I-DEV exclusion fourth, calibration-identity last.
    ...
    try:
        ...
        def partial_summary(outcome_label: str, gates: dict) -> dict:
            ...
            return {
                ...
                "d1_cap_vs_raw_ratio": float(FROZEN_D1_LEAKAGE_BITS / FROZEN_RAW_INPUT_BITS),
        ...
        for slot_index, (arm_index, spec, block_index, block) in enumerate(slots):
            ...
            frame_start = int(block["frame_start"])
            seed = l1_dose_escalation_seed_bits(tag_master, n, spec.name, block_index,
            ...
            if spec.kind == "operational":
                # D0 and D1 share the accepted operational path bit-for-bit
                # except the disclosed L1 prefix length: D0 runs k1=319
                # (base) while D1 runs k1=575 (the single frozen +256
            ...
            records.append(record)
            opf._append_jsonl(out_path / "per_block_arm_outcomes.jsonl", record)
            for event in l1_dose_escalation_block_events(n, spec, block_index, frame_start, record):
                events.append(event)
        ...
        outcome_label = l1_dose_escalation_1p5m_label(gates)
        ...
        opf._write_json(out_path / "input_and_predecessor_identity.json", identity_doc)
        persist(summary)
        return L1DoseEscalation1p5mRun(summary=summary, records=tuple(records), gates=gates)
    except L1DoseEscalation1p5mResourceError as exc:
        ...
        try:
            ...
            pass
        raise L1DoseEscalation1p5mResourceError(label) from exc
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nbpolar-p20j-l1-dose-escalation-1p5m",
        description=(
            "NB-Polar Phase 4-P20J N=32768 L1-dose-escalation +256 single-factor "
...
def main(argv=None) -> int:
    ...
    try:
        run = run_l1_dose_escalation_1p5m(
        ...
        print(f"nbpolar phase4-p20j l1 dose escalation 1p5m refused: {exc}", file=sys.stderr)
        return 2
    summary = run.summary
    print(
        json.dumps(
            {
                ...
                "d1_restored_count": summary["aggregates"]["recovery"]["d1_restored_count"],
...
__all__ = [
    ...
    "FROZEN_K_TOTAL", "FROZEN_L1_DELTA_K1", "FROZEN_K1_D1", "FROZEN_K_TOTAL_D1",
    "FROZEN_D1_KEY_BIT_DELTA", "FROZEN_LEAKAGE_BITS", "FROZEN_D1_LEAKAGE_BITS",
    ...
    "P20H_DEV_FRAME_RANGE", "P20I_DEV_FRAME_RANGE",
    ...
    "L1DoseEscalation1p5mContractError", "L1DoseEscalation1p5mResourceError",
    "L1DoseEscalation1p5mRun", "check_frozen_arm_table",
    "check_frozen_calibration_pins", "verify_stage_b_prior",
    "verify_dev_source_identity", "verify_dev_block_identity",
    "verify_dev_manifest",
    "l1_dose_escalation_seed_bits", "form_dev_blocks", "candidate_nll_bits",
    ...
    "l1_dose_escalation_1p5m_label", "l1_dose_escalation_block_events",
    "l1_dose_escalation_recount_events", "run_l1_dose_escalation_1p5m",
    "build_parser", "main",
]
