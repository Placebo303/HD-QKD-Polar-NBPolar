"""NB-Polar Phase 4-P20H N=32768 +1024 per-session-calibration confirmation on type2_1p5M_20260121_183806 (Stage A, frozen).

Per-session-calibration confirmation (strategy stage 2, second half): with
ONE preregistered change against P20G -- the prior is per-session
...
7516 via frozen order-prefix extension, floor 1e-15, N=32768, new P20H tag
domains only), zero further tuning, does +1024 maintain exact on three
1.5M TRAIN-first-384-frame blocks -- and does a P20C-style restoration
event (B0 fail -> B1 exact) appear on per-session-calibrated data, or
not? The only deliberate differences from P20C/P20E/P20F/P20G are the
per-session-calibrated prior and the new P20H tag domain, both
...
Development population: FIRST 384 TRAIN frames of the independent 1.5M
session file ``type2_1p5M_20260121_183806/pairs.parquet`` in
``(frame_id, pair_idx)`` order (frozen TRAIN base 0: frames ``0..383``;
all remaining 1.5M TRAIN frames ``384..1659`` are never used). Three NEW
DEV blocks ``0..127`` / ``128..255`` / ``256..383`` plus the declared
never-used remainder ``384..1659`` (1276 frames / 326656 pairs, counted
from the pool but never decoded). The ENTIRE 1M pool and the reserved 2M
file are excluded by the fail-closed cross-file source-tag+digest gate
before any SC call; DEV ranges must additionally overlap NONE of the
1.5M VAL/HOLD frame sets (intra-file gate, checked second).
...
MODE = "dev-plus1024-per-session-calibration"
...
# Development population: FIRST 384 TRAIN frames of the independent 1.5M
# session file in (frame_id, pair_idx) order. Frozen TRAIN base 0 comes
# from JSON provenance only (split-manifest counts + frozen time-ordered
# split code + 0-based 1M precedent under the identical pipeline); the
# runner fail-closes on any layout drift before any SC call, so a wrong
# base BLOCKS instead of misattributing. The 1M full pool and the
...
FROZEN_BLOCK_RANGES = ((0, 127), (128, 255), (256, 383))
FROZEN_REMAINDER_FRAME_RANGE = (384, 1659)
FROZEN_REMAINDER_FRAMES = 1276
...
SEED_PREFIX = "nbpolar-p20h-per-session-calibration-seed"
...
class ArmSpec:
    """One frozen P20H arm (never CLI-tunable, never extensible)."""
...
# Stage-A frozen per-session calibration identity (the single deliberate
# delta vs P20G): Stage B loads ONLY this digest-pinned prior file
...
# recalibrated literals are frozen by the Stage-A calibration product
# (filled at Stage-A close; sentinel until then).
FROZEN_PRIOR_PATH = (
...
# Cross-file source identity (double gate, first): the DEV content open
...
COMPLETE_LABEL = "TARGET_EMPIRICAL_N32768_DEV_PLUS1024_PER_SESSION_CALIBRATION_COMPLETE"
...
DEV_BLOCK_RULE = (
    "TRAIN development formation (P18 pattern, P20H ranges): sort TRAIN rows by "
    "(frame_id, pair_idx); require exactly 384 frames 0..383, 256 rows "
    "per frame, pair_idx 0..255 and symbols 0..1023; blocks 0..127 / "
    "128..255 / 256..383 (128 frames = 32768 pairs each); frames "
    "384..1659 are the declared never-used remainder; the 1M full pool "
    "and the reserved 2M file never selected (cross-file gate first); "
    "1.5M VAL 1660..2212 and 1.5M HOLD 2213..2766 never selected "
    "(intra-file gate second); "
...
CLAIM_SCOPE = (
    "descriptive +1024 confirmation on per-session-calibrated 1.5M TRAIN "
    "blocks (N=32768, three DEV blocks); B2 is an "
...
FROZEN_COMMAND = (
    ...
    "comparison_bench.src.comparison_bench.formal_ir.nbpolar.plus1024_per_session_confirmation "
    ...
    "--dev-frames 0 383 --block-frames 128 --remainder-frames 384 1659 "
    "--tag-master 2026092220 --chunk-rows 512 --tag-bits 64 "
...
class Plus1024PerSessionConfirmationContractError(ValueError):
    """Pre-open DEV-population/formation failure: BLOCKED(<earliest gate>)."""


class Plus1024PerSessionConfirmationResourceError(RuntimeError):
...
def verify_dev_source_identity(dev_pairs=None) -> dict:
    """Pin the 1.5M cross-file source identity (double gate, FIRST).
    ...
    if candidate == REFUSED_1M_DEV_PAIRS_PATH:
        raise ValueError(
            "P20H DEV source gate: 1M full-pool path refused "
            f"({REFUSED_1M_DEV_PAIRS_PATH!r}); the entire 1M pool is "
            "fail-closed against P20H DEV selection"
        )
    if candidate == REFUSED_2M_DEV_PAIRS_PATH:
        raise ValueError(
            "P20H DEV source gate: reserved 2M path refused "
    ...
    if failing:
        raise ValueError("P20H DEV source identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_block_identity() -> dict:
    """Pin the P20H DEV ranges and their intra-file disjointness gate.

    Pure code-level check (no I/O). This is the double gate's SECOND half:
    the cross-file source gate (``verify_dev_source_identity``) runs first.
    The DEV ranges must overlap NONE of the 1.5M VAL frame set 1660..2212
    or the 1.5M HOLD frame set 2213..2766; any overlap raises before any
    root exists and before either protected content open, consuming
    nothing. The VAL check runs first, then the HOLD check: both are
    safety properties and must fire even when the ranges also mismatch
    their literals.
    """
    ...
    if not (rem_last < val_first or rem_first > val_last):
        raise ValueError("P20H DEV remainder overlaps 1.5M VAL data")
    ...
    if not (rem_last < hold_first or rem_first > hold_last):
        raise ValueError("P20H DEV remainder overlaps 1.5M HOLD data")
    checks = {
        ...
        "dev_frame_range": tuple(FROZEN_DEV_FRAME_RANGE) == (0, 383),
        ...
        "remainder_frame_range": tuple(FROZEN_REMAINDER_FRAME_RANGE) == (384, 1659),
        "remainder_frames": FROZEN_REMAINDER_FRAMES == 1276,
        "remainder_symbols": FROZEN_REMAINDER_SYMBOLS == 326656,
        "intra_val_range": tuple(INTRA_FILE_VAL_FRAME_RANGE) == (1660, 2212),
        "intra_hold_range": tuple(INTRA_FILE_HOLD_FRAME_RANGE) == (2213, 2766),
    }
    failing = sorted(name for name, ok in checks.items() if not ok)
    if failing:
        raise ValueError("P20H DEV block-range identity mismatch: " + ",".join(failing))
    return {
...
def verify_dev_manifest(path, *, source: str = FROZEN_SOURCE) -> dict:
    ...
    its validation line-for-line under the P20H source/pins: accepted
    schema, the 1.5M source tag, and exactly the TRAIN 1660/424960 +
    VAL 553/141568 + HOLD 554/141824 counts that host the P20H DEV prefix
    (frames 0..383). Any mismatch raises before any root exists.
    """
...
def plus1024_seed_bits(
    ...
    *,
    ...
    """Deterministic public per-(arm, block) Toeplitz seed (P20H domain).

    Rule: concatenate SHA-256 over
    ``"nbpolar-p20h-per-session-confirmation-seed:<master>:<n>:<arm>:
    <block_index>:<counter>"`` for ``counter = 0, 1, ...`` (ASCII decimal),
    unpack each digest MSB-first and truncate to ``bit_length`` (default
    ``10*n + 63``). The P20H prefix and arm tokens differ from the P16, P17,
    P18, P19, P20B, P20C, P20E and P20F domains on purpose. Seed contents are public control
    ...
    if arm not in FROZEN_ARM_NAMES:
        raise ValueError(f"unknown frozen arm for the P20H seed domain: {arm!r}")
...
def form_dev_blocks(
    ...
    *,
    ...
    P18 ``form_holdout_blocks`` pattern with P20H ranges (the accepted helper
    ...
    try:
        ...
        raise TypeError(f"dev-frames must be a pair of integers: {exc}") from exc
    if pair != tuple(FROZEN_DEV_FRAME_RANGE):
        raise Plus1024PerSessionConfirmationContractError(
    ...
    # The declared never-used remainder (384..1659) lies OUTSIDE the DEV
    # selection by design (frozen --dev-frames 0 383), so its presence
    ...
    # exactly the frozen 1276 frames 384..1659 with 326656 rows.
...
def plus1024_per_session_confirmation_label(gates: dict) -> str:
...
def _backoff_event(
    *, n, arm, block_index, frame_start, event_id, event_type, direction,
    ...
    event = {
        "event_id": event_id,
        "frame_key": (
            f"nbpolar-p20h-per-session-confirmation:{int(n)}:{int(frame_start)}:{int(block_index)}"
        ),
        "method": "nbpolar_plus1024_per_session_confirmation",
...
class Plus1024PerSessionConfirmationRun:
    """In-memory P20H diagnostic run (also returned by the runner)."""
...
def _stub_plan(
    *, out_path, source, floor, n, k1, k2, construction, digest,
    ...
    plan = {
        ...
        "single_factor": "ONE preregistered change vs P20H: the prior is per-session calibrated on 1.5M TRAIN counts by the frozen §3 program (Stage-A frozen, read-only in Stage B); +1024 order-prefix step on B1 carried over; floor/construction-order/kernel/representation/SC identical across arms; only the calibrated prior and the new P20H tag domain differ",
        "deferred": ["calibration-positive efficiency-optimization round", "confirmation-negative P20D fixed-disclosure alternative L2 construction", "disclosure-minimality probe (+512 vs +1024)", "second independent session on the reserved 2M file"],
    }
    ...
    (out_path / "report.md").write_text("# P20H plus1024 per-session confirmation (RUNNING)\n", encoding="utf-8")
    return plan
...
def _finalize_blocked(out_path, identity_doc, accounting, opened_paths, message: str) -> None:
    ...
    (out_path / "report.md").write_text(
        f"# P20H plus1024 per-session confirmation\n\nBLOCKED({message})\n", encoding="utf-8")


def run_plus1024_per_session_confirmation(
    *,
    prior=None,
...
) -> Plus1024PerSessionConfirmationRun:
    """Execute the frozen P20H three-arm diagnostic once; write five files.
    ...
    counts NPZ is never opened in Stage B. The +1024 L2 step on B1 is
    hardcoded (never CLI-tunable): B1 runs the accepted operational path
    with ``k2 = base k2 + 1024`` on the same frozen L2 order
    (order-prefix extension).
    """
    ...
    try:
        ...
        outcome_label = plus1024_per_session_confirmation_label(gates)
        ...
        opf._write_json(out_path / "input_and_predecessor_identity.json", identity_doc)
        persist(summary)
        return Plus1024PerSessionConfirmationRun(summary=summary, records=tuple(records), gates=gates)
    except Plus1024PerSessionConfirmationResourceError as exc:
        ...
        try:
            ...
            pass
        raise Plus1024PerSessionConfirmationResourceError(label) from exc
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "NB-Polar Phase 4-P20H N=32768 plus1024 per-session-calibration "
            "confirmation (Stage-A frozen calibrated prior, verified P16 "
            "construction, three 1.5M TRAIN DEV blocks, three frozen arms)"
        ),
    )
...
def main(argv=None) -> int:
    ...
    try:
        run = run_plus1024_per_session_confirmation(
        ...
        print(f"nbpolar phase4-p20h plus1024 per-session confirmation refused: {exc}", file=sys.stderr)
        return 2
    summary = run.summary
    print(
        json.dumps(
...
__all__ = [
    ...
    "Plus1024PerSessionConfirmationContractError", "Plus1024PerSessionConfirmationResourceError",
    "Plus1024PerSessionConfirmationRun", "check_frozen_arm_table",
    ...
    "plus1024_per_session_confirmation_label", "plus1024_block_events",
    "plus1024_recount_events", "run_plus1024_per_session_confirmation",
    "build_parser", "main",
]
