"""Focused Phase 4-P20L N=32768 L1-order same-budget single-factor tests.
...
product, the real Stage-A order file, every real evidence root, the 1M
same-file pools (TRAIN 0..1199 / VAL 1200..1599 / HOLD 1600..1999, incl.
consumed P20C DEV 0..383, P20E DEV 384..767 and P20F DEV 768..1151), the
consumed P20H DEV prefix (1.5M TRAIN 0..383), the consumed P20I DEV
segment (1.5M TRAIN 384..767), the consumed P20J DEV segment (1.5M TRAIN
768..1151), the consumed P20K DEV + remainder (1.5M TRAIN 1152..1659),
the 1.5M VAL remainder 2044..2212, the 1.5M HOLD pool and the reserved
2M file are never touched.
Focused tests use their own fresh seeds ``2026092261..2026092267`` and
never the frozen P20L tag master 2026092260 (nor the P20K master
2026092250, the P20J master 2026092240, the P20I master 2026092230, the
P20H master 2026092220, the P20G master 2026092210, the P20F master
2026092200, the P20E master 2026092100, the P20C master 2026092090, the
P20B master 2026092080, the P19 master 2026092060, the P18 master
2026092050, the P16/P17 streams, the P20L TRAIN seeds 2026092271..2274,
...
prior-reuse rule, pinned against the P20K module read-only) plus the
single deliberate delta (the frozen same-budget L1-order swap, first-319
of the NEW 1.5M L1 order FILE on F1 vs first-319 of the frozen P16 L1
order on F0) are asserted literally, the swap is asserted as a
same-budget order differential (F1 k1=319 on a DIFFERENT frozen L1
order object than F0 with the SAME frozen L2 order, key-bit delta
exactly 0), and the fail-closed septuple gate (cross-file
source-tag+digest gate against the 1M full pool and the reserved 2M
file first, then the intra-file VAL-containment + VAL-exterior/HOLD
gate with VAL first, then the consumed-P20H-DEV exclusion gate, then
the consumed-P20I-DEV exclusion gate, then the consumed-P20J-DEV
exclusion gate, then the consumed-P20K-DEV+remainder exclusion gate,
then the calibration-identity + order-freeze digest gate with zero
Stage-B sampling) is asserted to refuse before any SC call, with zero
protected opens and the 2M file never touched. Every ``test_*`` takes
...
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
    l1_dose_512_1p5m as p20k,  # read-only same-budget/carry-over reference
)
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
...
TEST_SEEDS = (2026092261, 2026092262, 2026092263, 2026092264)
TEST_MASTER = 2026092265
TEST_SPARES = (2026092266, 2026092267)

FROZEN_BASE_LEAKAGE = 34119  # 5 * 6811 + 64 (F0 base operational)
FROZEN_F1_LEAKAGE = 34119  # 5 * 6811 + 64 (F1, new order, same budget)
FROZEN_CONTROL_LEAKAGE = 32524  # 5 * 6492 + 64 (oracle arm)
FROZEN_PUBLIC_BITS = 327743  # 10 * 32768 + 63
TOTAL_KEY_BITS = 3 * FROZEN_BASE_LEAKAGE + 3 * FROZEN_F1_LEAKAGE + 3 * FROZEN_CONTROL_LEAKAGE  # 302286
...
def make_order_file(directory, *, prior_digest, tamper=None,
                    name="new_l1_order_1p5m.json"):
    """Fabricate a canonical-shape order-file copy (never the real Stage-A file)."""
    n = 32768
    rng = np.random.default_rng(TEST_MASTER)
    order = rng.permutation(n).astype(np.int64).tolist()
    if tamper == "order-dup":
        order[0] = order[1]
    derivation = {
        "program": pe.FROZEN_ORDER_PROGRAM_PIN,
        "prior_path": "stub",
        "prior_digest": str(prior_digest),
        "train_seeds": [2026092271, 2026092272, 2026092273, 2026092274],
        "train_blocks_per_seed": 4,
        "train_blocks_attempted": 16,
        "train_blocks_used": 16,
        "train_impossible": 0,
        "train_genie_l1_calls": 16,
    ...
    doc = {
        ...
        "n": n,
        "l1_order": order,
        "k1_frozen_prefix_len": 319,
        "derivation": derivation,
    }
    if tamper == "protocol":
        doc["protocol"] = "nbpolar-p20l-tampered"
    elif tamper == "kind":
        doc["kind"] = "l2-order-file"
    elif tamper == "n":
        doc["n"] = 16384
    elif tamper == "prefix-len":
        doc["k1_frozen_prefix_len"] = 320
...
def injected_dev_table(*, frames=384, first_frame=1660, per_frame=256,
    ...
    Carries the DEV pool (1660..2043) plus the declared never-used remainder
    (2044..2212, counted but never decoded); ``extras=True`` adds one frame
    per forbidden pool (1.5M HOLD 2213, consumed P20H DEV frame 0,
    consumed P20I DEV frame 384, consumed P20J DEV frame 768, consumed
    P20K DEV frame 1152, consumed P20K remainder frame 1536), present in
    the file but never selected.
    """
    rng = np.random.default_rng(seed)
    ...
    if extras:
        # Frames outside the DEV pool, one per forbidden pool (1.5M HOLD,
        # consumed P20H/P20I/P20J DEV, consumed P20K DEV+remainder):
...
def mutate_dev(df, how):
    ...
    if how == "drop-frame":
        ...
        row = out[(out["frame_id"] == 1660) & (out["pair_idx"] == 255)].index[0]
        out.loc[row, "pair_idx"] = 254
    elif how == "symbol-range":
        row = out[out["frame_id"] == 1660].index[0]
        out.loc[row, "bob_symbol"] = 1024
    elif how == "no-dev":
        out = out[out["frame_id"].isin([2213, 0])]
    elif how == "drop-remainder-frame":
        out = out[out["frame_id"] != 2100]
    elif how == "shrink-remainder":
        out = out[~((out["frame_id"] == 2212) & (out["pair_idx"] == 255))]
    else:
...
class FakeOperationalArm:
    """Scripted fake for the accepted ``run_operational_block`` seam (F0 + F1).

    One shared seam serves both operational arms; both disclose the same
    ``k1`` (319 same budget) and the frozen L1 order object identity is
    recorded to pin the same-budget order swap (F1 runs a DIFFERENT L1
    order object than F0); the frozen L2 order identity is recorded too
    (same L2 order on both operational arms, K2 stays base on every arm).
    """
...
class FakeControl:
    """Scripted fake for the ``run_oracle_control_block`` (F2) seam."""
    ...
    def __call__(self, **kwargs):
        ...
        self.seen.append({
            "arm": "F2_true_l1_diagnostic", "outcome": outcome,
...
def full_run(out, *, s0_scripts=None, s2_scripts=None,
             counts=None, dev=None, order_tamper=None, **over):
    tmp = Path(out).parent
    fab_dir = tmp / "fab"
    ...
    order_path, order_hex = make_order_file(
        tmp, prior_digest=prior["digest"], tamper=order_tamper)
    if dev is None:
    ...
    with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
        with patched(pe, "FROZEN_PRIOR_DIGEST", prior["digest"]):
            with patched(pe, "FROZEN_ORDER_DIGEST", order_hex):
                with patched(pe, "run_operational_block", s0):
                    with patched(pe, "run_oracle_control_block", s2):
                        run = pe.run_l1_order_1p5m(**kw)
    return run, s0, s2


def order_kwargs(tmp_path, prior_digest, *, tamper=None,
                 name="new_l1_order_1p5m.json"):
    """Stub order-file kwargs plus its sha for direct-call refusal tests."""
    order_path, order_hex = make_order_file(
        tmp_path, prior_digest=prior_digest, tamper=tamper, name=name)
    return {"order_file": str(order_path), "order_digest": order_hex}, order_hex
...
def test_frozen_constants_and_arm_table_pinned():
    ...
    assert pe.FROZEN_K1_F1 == 319
    assert pe.FROZEN_K_TOTAL_F1 == 6811
    assert pe.FROZEN_F1_KEY_BIT_DELTA == 0
    assert not hasattr(pe, "FROZEN_L1_DELTA_K1")
    assert pe.FROZEN_DEV_FRAME_RANGE == (1660, 2043)
    assert pe.FROZEN_DEV_FRAMES == 384
    assert pe.FROZEN_DEV_PAIRS == 98304
    assert pe.FROZEN_BLOCK_RANGES == ((1660, 1787), (1788, 1915), (1916, 2043))
    assert pe.FROZEN_REMAINDER_FRAME_RANGE == (2044, 2212)
    assert pe.FROZEN_REMAINDER_FRAMES == 169
    assert pe.FROZEN_REMAINDER_SYMBOLS == 43264
    ...
    assert pe.P20K_DEV_REMAINDER_FRAME_RANGE == (1152, 1659)
    ...
    assert pe.FROZEN_TAG_MASTER == 2026092260
    assert pe.SEED_PREFIX == "nbpolar-p20l-l1-order-1p5m-seed"
    ...
    assert pe.FROZEN_F1_LEAKAGE_BITS == FROZEN_F1_LEAKAGE
    ...
    assert abs(pe.FROZEN_F1_LEAKAGE_BITS / pe.FROZEN_RAW_INPUT_BITS - 0.1041229248046875) < 1e-12
    assert pe.FROZEN_ORDER_FILE_PATH == (
        ".workbuddy/queue/NBPOLAR-PHASE4-P20L-L1-ORDER-1P5M/new_l1_order_1p5m.json")
    assert pe.FROZEN_ORDER_DIGEST == (
        "5554404a07a6999c82b54b101db50b65011505267fd8a86a1f441d2dcb313278")
    assert pe.FROZEN_OLD_L1_ORDER_DIGEST == (
        "e3aea072eb3e9454afb59a97028fb968bb9f52a155a82e6a4022e5adb13dec21")
    assert pe.FROZEN_TRAIN_SEEDS == (2026092271, 2026092272, 2026092273, 2026092274)
    assert pe.FROZEN_TRAIN_BLOCKS_PER_SEED == 4
    pin = pe.check_frozen_arm_table()
    assert pin["f1_k1"] == 319
    assert pin["f1_key_bit_delta_vs_f0"] == 0
    assert pin["cap_vs_raw_ratio"] < 0.5
    assert pin["f1_cap_vs_raw_ratio"] < 0.5
    ...
    assert "--dev-frames 1660 2043" in pe.FROZEN_COMMAND
    assert "--remainder-frames 2044 2212" in pe.FROZEN_COMMAND
    assert "--tag-master 2026092260" in pe.FROZEN_COMMAND
    assert "--source 1p5M" in pe.FROZEN_COMMAND
    assert "--prior" in pe.FROZEN_COMMAND
    assert "--counts" not in pe.FROZEN_COMMAND
    assert "--train-seeds" not in pe.FROZEN_COMMAND
    assert "type2_1p5M_20260121_183806/pairs.parquet" in pe.FROZEN_COMMAND
    assert "calibrated_prior.npz" in pe.FROZEN_COMMAND
    assert "new_l1_order_1p5m.json" in pe.FROZEN_COMMAND
    assert "--order-digest 5554404a" in pe.FROZEN_COMMAND
    assert "l1_order_1p5m" in pe.FROZEN_COMMAND
    assert "--dev-frames 768 1151" not in pe.FROZEN_COMMAND
    assert "--dev-frames 1152 1535" not in pe.FROZEN_COMMAND
    assert "--remainder-frames 1536 1659" not in pe.FROZEN_COMMAND
    assert "--tag-master 2026092250" not in pe.FROZEN_COMMAND
    with patched(pe, "FROZEN_K1_F1", 320):
        assert_raises_match(ValueError, "319", pe.check_frozen_arm_table)
    with patched(pe, "FROZEN_F1_LEAKAGE_BITS", 34118):
        assert_raises_match(ValueError, "drifted", pe.check_frozen_arm_table)
    with patched(pe, "FROZEN_F1_KEY_BIT_DELTA", 5):
        assert_raises_match(ValueError, "exactly 0", pe.check_frozen_arm_table)


def test_zero_tuning_carry_over_from_p20k():
    # Every carried-over input is byte-identical with the P20K module
    # (read-only reference): N, floor, base K values, base/oracle caps,
    # the digest-pinned prior reuse (path/digest/lambda/literals), tag
    # shape, budgets, provenance and outcome identity. Only the
    # single-factor L1-order swap, the new-segment population, the
    # order-file mechanism and the new tag domain may differ (plus the
    # three R1delta gate additions).
    assert pe.FROZEN_N == p20k.FROZEN_N == 32768
    assert pe.FROZEN_FLOOR == p20k.FROZEN_FLOOR == 1e-15
    assert (pe.FROZEN_K1, pe.FROZEN_K2, pe.FROZEN_K_TOTAL) == (
        p20k.FROZEN_K1, p20k.FROZEN_K2, p20k.FROZEN_K_TOTAL) == (319, 6492, 6811)
    assert pe.FROZEN_LEAKAGE_BITS == p20k.FROZEN_LEAKAGE_BITS == 34119
    assert pe.FROZEN_CONTROL_LEAKAGE_BITS == p20k.FROZEN_CONTROL_LEAKAGE_BITS == 32524
    assert pe.FROZEN_PUBLIC_CONTROL_BITS == p20k.FROZEN_PUBLIC_CONTROL_BITS == 327743
    assert pe.FROZEN_RAW_INPUT_BITS == p20k.FROZEN_RAW_INPUT_BITS == 327680
    assert pe.PLANNED_SC_CALLS == p20k.PLANNED_SC_CALLS == 15
    assert pe.PLANNED_TAG_INVOCATIONS == p20k.PLANNED_TAG_INVOCATIONS == 9
    assert pe.PLANNED_RECORDS == p20k.PLANNED_RECORDS == 9
    assert pe.OPERATIONAL_PROVENANCE == p20k.OPERATIONAL_PROVENANCE
    assert pe.ORACLE_PROVENANCE == p20k.ORACLE_PROVENANCE
    assert pe.OUTCOMES == p20k.OUTCOMES
    # The prior-reuse freeze is carried over byte-identical (R1): the
    # same Stage-A product, digest, lambda/floor pins and literals.
    assert pe.FROZEN_PRIOR_PATH == p20k.FROZEN_PRIOR_PATH
    assert pe.FROZEN_PRIOR_DIGEST == p20k.FROZEN_PRIOR_DIGEST == (
        "e8dd078a5367ba3af8eba91022d58aeffc114bc30b847b13cd4bb890e5dde43b")
    assert pe.FROZEN_LAMBDA == p20k.FROZEN_LAMBDA == 137.3823795883264
    assert (pe.FROZEN_CAL_H1, pe.FROZEN_CAL_H2, pe.FROZEN_CAL_H_TOTAL) == (
        p20k.FROZEN_CAL_H1, p20k.FROZEN_CAL_H2, p20k.FROZEN_CAL_H_TOTAL)
    assert pe.SUPPORT_RULE == p20k.SUPPORT_RULE
    # Gate order carries the R1delta additions: prior_identity (renamed
    # from calibration_identity), order_derivation_identity and
    # genie_calls_exact are new; everything else is in the same order.
    assert "prior_identity" in pe.INTEGRITY_GATE_ORDER
    assert "order_derivation_identity" in pe.INTEGRITY_GATE_ORDER
    assert "genie_calls_exact" in pe.INTEGRITY_GATE_ORDER
    assert "calibration_identity" not in pe.INTEGRITY_GATE_ORDER
    assert "calibration_identity" in p20k.INTEGRITY_GATE_ORDER
    assert [g for g in pe.INTEGRITY_GATE_ORDER
            if g not in ("prior_identity", "order_derivation_identity",
                         "genie_calls_exact")] == [
        g for g in p20k.INTEGRITY_GATE_ORDER if g != "calibration_identity"
    ]
    # The factor swap: P20L varies L1 POSITIONS at fixed K1=319 (F1 K1 =
    # 319, key delta exactly 0) where P20K disclosed +512 L1 (E1 K1=831,
    # key delta +2560).
    assert pe.FROZEN_K1_F1 == 319
    assert pe.FROZEN_K_TOTAL_F1 == 6811
    assert pe.FROZEN_F1_KEY_BIT_DELTA == 0
    assert pe.FROZEN_F1_LEAKAGE_BITS == 34119
    assert p20k.FROZEN_L1_DELTA_K1 == 512
    assert p20k.FROZEN_K1_E1 == 831
    assert p20k.FROZEN_K_TOTAL_E1 == 7323
    assert p20k.FROZEN_E1_KEY_BIT_DELTA == 2560
    assert p20k.FROZEN_E1_LEAKAGE_BITS == 36679
    assert not hasattr(pe, "FROZEN_K1_E1")
    assert not hasattr(pe, "FROZEN_L1_DELTA_K1")
    assert not hasattr(pe, "DISCLOSURE_RULE")
    assert not hasattr(p20k, "FROZEN_K1_F1")
    assert not hasattr(p20k, "ORDER_SWAP_RULE")
    assert pe.FROZEN_TOTAL_KEY_DEPENDENT_BITS == TOTAL_KEY_BITS == 302286
    assert p20k.FROZEN_TOTAL_KEY_DEPENDENT_BITS == 309966
    assert pe.FROZEN_TOTAL_KEY_DEPENDENT_BITS != p20k.FROZEN_TOTAL_KEY_DEPENDENT_BITS
    assert pe.FROZEN_TOTAL_PUBLIC_CONTROL_BITS == p20k.FROZEN_TOTAL_PUBLIC_CONTROL_BITS
    assert pe.FROZEN_ARM_NAMES != p20k.FROZEN_ARM_NAMES
    assert pe.FROZEN_ARM_NAMES == ("F0_old_order_base", "F1_new_l1_order_base", "F2_true_l1_diagnostic")
    assert p20k.FROZEN_ARM_NAMES == ("E0_sc_base", "E1_L1plus512", "E2_true_l1_diagnostic")
    assert "319" in pe.ORDER_SWAP_RULE
    # The deliberate deltas: new-segment population + order-file mechanism
    # + new tag domain only.
    assert pe.FROZEN_SOURCE == p20k.FROZEN_SOURCE == "1p5M"
    assert pe.FROZEN_DEV_FRAME_RANGE != p20k.FROZEN_DEV_FRAME_RANGE
    assert pe.FROZEN_BLOCK_RANGES != p20k.FROZEN_BLOCK_RANGES
    assert pe.FROZEN_REMAINDER_FRAME_RANGE != p20k.FROZEN_REMAINDER_FRAME_RANGE
    assert pe.P20H_DEV_FRAME_RANGE == p20k.P20H_DEV_FRAME_RANGE == (0, 383)
    assert pe.P20I_DEV_FRAME_RANGE == p20k.P20I_DEV_FRAME_RANGE == (384, 767)
    assert pe.P20J_DEV_FRAME_RANGE == p20k.P20J_DEV_FRAME_RANGE == (768, 1151)
    assert not hasattr(p20k, "P20K_DEV_REMAINDER_FRAME_RANGE")
    assert pe.P20K_DEV_REMAINDER_FRAME_RANGE == (1152, 1659)
    assert pe.FROZEN_TAG_MASTER != p20k.FROZEN_TAG_MASTER
    assert pe.SEED_PREFIX != p20k.SEED_PREFIX
    assert pe.PROTOCOL_NAME != p20k.PROTOCOL_NAME
    assert pe.COMPLETE_LABEL != p20k.COMPLETE_LABEL
...
def test_fresh_test_seeds_disjoint_from_all_frozen_seeds():
    frozen = (
        ...
        | {hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER, p20j.FROZEN_TAG_MASTER,
           p20k.FROZEN_TAG_MASTER, pe.FROZEN_TAG_MASTER}
    )
    for seed in (2026092050, 2026092060,
                 ...
                 2026092084, 2026092085, 2026092086, 2026092087,
                  ...
                  2026092250, 2026092251, 2026092252, 2026092253,
                  2026092254, 2026092255, 2026092256, 2026092257,
                  2026092260,
                  2026092271, 2026092272, 2026092273, 2026092274):
        frozen.add(seed)
    ...
    assert pe.FROZEN_TAG_MASTER not in (2026092050, 2026092060, 2026092080,
                                        ...
                                        2026092230, 2026092240, 2026092250,
                                        p20j.FROZEN_TAG_MASTER, p20k.FROZEN_TAG_MASTER)
    ...
    assert pe.FROZEN_TAG_MASTER != p20k.FROZEN_TAG_MASTER
    assert set(pe.FROZEN_TRAIN_SEEDS).isdisjoint(
        set(TEST_SEEDS) | set(TEST_SPARES) | {TEST_MASTER})
...
def test_accepted_helpers_shared_not_reimplemented():
    ...
    for banned in ("sample_full_block", "sample_bob", "block_genie_risks",
                   "genie_layer_risks", "genie_conditionals",
                   "select_empirical_split",
                   "budget_k_total", "residual_for_orders", "analytic_order",
                   "wilson_lower_bound",
                   ...
                   "calibrate_array", "build_prior_arrays",
    ...
    # No Stage-A derivation import inside the Stage-B runner: the frozen
    # order file is loaded read-only, never derived.
    assert "import derive_l1_order" not in Path(pe.__file__).read_text(encoding="utf-8")
    # The accepted HOLD formation is range-pinned: DEV ranges are refused,
    # which justifies the local P20L formation with identical semantics.
    assert_raises_match(ValueError, "hold-frames",
                         hm.form_holdout_blocks, injected_dev_table(),
                         hold_frames=(1660, 2043), block_frames=128,
                         remainder_frames=(2044, 2212))
    # The accepted split-manifest helper is source-pinned to 1M: the 1p5M
    # source is refused, which justifies the local P20L manifest check
...
def test_manifest_and_dev_pin_refusals_zero_reads_zero_root():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for field, value in (
            ("FROZEN_BLOCK_RANGES", ((1660, 1786), (1787, 1914), (1915, 2042))),
        ):
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
        ...
        for field, value, substr in (
            ("FROZEN_REMAINDER_FRAME_RANGE", (2045, 2212), "remainder_frame_range"),
            ("FROZEN_DEV_FRAME_RANGE", (1660, 2044), "dev_frame_range"),
            ...
            ("P20K_DEV_REMAINDER_FRAME_RANGE", (1152, 1658), "p20k_dev_remainder_range"),
        ):
            with patched(pe, field, value):
                assert_raises_match(ValueError, substr, pe.verify_dev_block_identity)


def test_septuple_gate_source_first_then_val_then_hold_then_p20h_p20i_p20j_p20k_then_freeze():
    ...
    # Intra-file gate second: DEV ranges must lie wholly inside 1.5M VAL
    # 1660..2212 (VAL checked first) with no VAL-exterior/HOLD overlap.
    pin = pe.verify_dev_block_identity()
    assert pin["verified"] is True
    assert pin["intra_val_contained"] is True
    ...
    assert pin["p20k_dev_remainder_disjoint"] is True
    ...
    assert pin["p20k_dev_remainder_frame_range"] == [1152, 1659]
    # Any DEV block outside 1.5M VAL refuses (containment, VAL first).
    with patched(pe, "FROZEN_BLOCK_RANGES", ((1660, 1787), (1788, 1915), (1916, 2044))):
        assert_raises_match(ValueError, "lies outside 1.5M VAL",
                            pe.verify_dev_block_identity)
    with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (2044, 2213)):
        assert_raises_match(ValueError, "lies outside 1.5M VAL",
                            pe.verify_dev_block_identity)
    # The HOLD branch fires once VAL containment passes: narrow HOLD into
    # VAL to reach it (blocks stay VAL-contained).
    with patched(pe, "INTRA_FILE_HOLD_FRAME_RANGE", (2000, 2266)):
        assert_raises_match(ValueError, "overlaps 1.5M HOLD",
                            pe.verify_dev_block_identity)
    with patched(pe, "INTRA_FILE_HOLD_FRAME_RANGE", (2100, 2266)):
        assert_raises_match(ValueError, "remainder overlaps 1.5M HOLD",
                            pe.verify_dev_block_identity)
    # P20H-DEV exclusion gate third: widen VAL to reach the TRAIN branch,
    # then DEV ranges must overlap NONE of the consumed P20H DEV prefix.
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_BLOCK_RANGES", ((0, 127), (1660, 1787), (1788, 1915))):
    ...
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (0, 383)):
            assert_raises_match(ValueError, "remainder overlaps consumed P20H DEV",
                                pe.verify_dev_block_identity)
    # P20I-DEV exclusion gate fourth.
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_BLOCK_RANGES", ((384, 511), (1660, 1787), (1788, 1915))):
    ...
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (384, 767)):
            assert_raises_match(ValueError, "remainder overlaps consumed P20I DEV",
                                pe.verify_dev_block_identity)
    # P20J-DEV exclusion gate fifth.
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_BLOCK_RANGES", ((768, 895), (1660, 1787), (1788, 1915))):
    ...
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (768, 1151)):
            assert_raises_match(ValueError, "remainder overlaps consumed P20J DEV",
                                pe.verify_dev_block_identity)
    # P20K-DEV+remainder exclusion gate sixth.
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_BLOCK_RANGES", ((1152, 1279), (1660, 1787), (1788, 1915))):
            assert_raises_match(ValueError, "consumed P20K DEV",
                                pe.verify_dev_block_identity)
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (1152, 1659)):
            assert_raises_match(ValueError, "remainder overlaps consumed P20K DEV",
                                pe.verify_dev_block_identity)
    # A block outside VAL reports VAL containment first (before any
    # TRAIN exclusion, even when it also touches consumed TRAIN).
    with patched(pe, "FROZEN_BLOCK_RANGES", ((0, 127), (1660, 1787), (1788, 1915))):
        assert_raises_match(ValueError, "lies outside 1.5M VAL",
                            pe.verify_dev_block_identity)
    # A block touching both P20I DEV and P20J DEV reports the P20I set
    # first (P20I-DEV exclusion runs before the P20J-DEV exclusion gate).
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_BLOCK_RANGES", ((384, 511), (768, 895), (1660, 1787))):
            assert_raises_match(ValueError, "consumed P20I DEV",
                                pe.verify_dev_block_identity)
    # A block touching both P20J DEV and P20K DEV+remainder reports the
    # P20J set first.
    with patched(pe, "INTRA_FILE_VAL_FRAME_RANGE", (0, 2212)):
        with patched(pe, "FROZEN_BLOCK_RANGES", ((768, 895), (1152, 1279), (1660, 1787))):
            assert_raises_match(ValueError, "consumed P20J DEV",
                                pe.verify_dev_block_identity)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        man = make_manifest(tmp_path)
        for over, substr in (
            ({"dev_frames": (1661, 2044)}, "dev-frames"),
            ({"dev_frames": (1660, 2044)}, "dev-frames"),
            ({"dev_frames": (1152, 1535)}, "dev-frames"),
            ({"remainder_frames": (2045, 2212)}, "remainder-frames"),
            ({"source": "1M"}, "source="),
            ({"source": "2M"}, "source="),
            ...
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
                assert_raises_match(
                    ValueError, substr, pe.run_l1_order_1p5m,
                    dev_table=injected_dev_table(), **prior_kwargs(),
                    construction=str(fab_path), construction_digest=fab_hex,
...
def test_dev_slicing_blocks_and_remainder_exact():
    ...
    assert formation["dev_frame_range"] == [1660, 2043]
    assert formation["dev_frames"] == 384
    assert formation["dev_pairs"] == 98304
    assert formation["block_ranges"] == [[1660, 1787], [1788, 1915], [1916, 2043]]
    assert [b["frame_start"] for b in formation["blocks"]] == [1660, 1788, 1916]
    assert all(len(b["bob"]) == 32768 for b in formation["blocks"])
    assert formation["remainder"]["frames"] == 169
    assert formation["remainder"]["symbols"] == 43264
    ...
    p20k_first, p20k_last = pe.P20K_DEV_REMAINDER_FRAME_RANGE
    for start, end in formation["block_ranges"]:
        assert val_first <= start and end <= val_last
        ...
        assert end < p20k_first or start > p20k_last
                ...
                with patched(pe, "FROZEN_PRIOR_DIGEST", prior["digest"]):
                    with patched(pe, "FROZEN_ORDER_DIGEST", ohex):
                        assert_raises_match(
                            ...
                            expected_literals=(prior["h1"], prior["h2"], prior["h_total"]),
                            construction=str(fab_path), construction_digest=fab_hex,
                            manifest_path=str(man), out_dir=root, **okw,
                        )
...
def test_no_fitting_or_sampling_tokens_and_exact_call_counts():
    ...
    # Zero Stage-B sampling: no RNG/sampler, genie, K-selection,
    # smoothing/refit, or counts-loader call sites anywhere in Stage B.
    # The derivation-program names appear ONLY inside the frozen
    # FROZEN_ORDER_PROGRAM_PIN provenance string, so carve that literal
    # out before asserting absence everywhere else.
    anchor = "FROZEN_ORDER_PROGRAM_PIN = ("
    start = source.index(anchor)
    depth = 0
    end = None
    for pos in range(start + len(anchor) - 1, len(source)):
        if source[pos] == "(":
            depth += 1
        elif source[pos] == ")":
            depth -= 1
    ...
    code = source[:start] + source[end:]
    for token in ("np.random.default_rng(", "SeedSequence(", "random_state",
                  "sample_full_block(", "sample_bob(",
                  "block_genie_risks(", "genie_layer_risks(",
                  "genie_conditionals(", "select_empirical_split(",
                  "budget_k_total(", "residual_for_orders(", "list_decode",
                  "fwht(", "posterior_APP",
                  "smooth_joint", "calibrate_array(", "build_prior_arrays(",
                  "wilson_lower_bound", "results/",
                  "outputs_comparison", "git commit", "git push"):
        assert token not in code, token
    for name in ("sample_full_block", "sample_bob", "block_genie_risks",
                 "genie_layer_risks", "genie_conditionals",
                 "select_empirical_split", "budget_k_total",
                 "smooth_joint_to_conditional", "calibrate_array",
                 "build_prior_arrays", "load_v25_channel_counts",
                 "default_rng", "SeedSequence"):
        assert not hasattr(pe, name), name
    assert re.search(r"\bSCL\b", source) is None, "SCL"
    assert "add_argument(\"--train-seeds\"" not in source
    assert "import derive_l1_order" not in source
    assert "from derive_l1_order" not in source
    # The frozen program pin documents the Stage-A derivation program
    # (names pinned as provenance, never called in Stage B).
    assert "sample_full_block" in pe.FROZEN_ORDER_PROGRAM_PIN
    assert "genie_layer_risks" in pe.FROZEN_ORDER_PROGRAM_PIN
    assert "disclosure_order_from_stats" in pe.FROZEN_ORDER_PROGRAM_PIN
    for token in ("l1_order_1p5m_seed_bits", "form_dev_blocks", "l1_prefix_positions",
                  "l2_prefix_positions",
                  "candidate_nll_bits", "raw_floor_diagnostics",
                  "order_derivation_identity", "genie_calls_exact",
                  "prior_identity", "MemoryError",
...
# ---- arms, order differential, accounting ----
...
def test_three_arm_semantics_k_leakage_labels_and_call_counts():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert run.summary["aggregates"]["arms"]["F0_old_order_base"]["key_dependent_bits"] == 3 * 34119
        assert run.summary["aggregates"]["arms"]["F1_new_l1_order_base"]["key_dependent_bits"] == 3 * 34119
        assert run.summary["aggregates"]["arms"]["F2_true_l1_diagnostic"]["key_dependent_bits"] == 3 * 32524
        assert run.summary["aggregates"]["operational"]["key_dependent_bits"] == 3 * 34119 + 3 * 34119
        ...
        assert run.summary["integrity"]["genie_calls_exact"] is True
        assert run.summary["integrity"]["order_derivation_identity"] is True
        assert run.summary["integrity"]["prior_identity"] is True
        assert len(s0.seen) == 6 and len(s2.seen) == 3
        # Slot order is block-major F0/F1/F2; every arm ran on every block.
        assert sorted(r["block_index"] for r in run.records
                      if r["arm"] == "F1_new_l1_order_base") == [0, 1, 2]
        # The single-factor differential: F1 discloses the SAME budget
        # (k1=319) on a DIFFERENT frozen L1 order object than F0, with
        # the SAME frozen L2 order and K2 at base on every arm.
        assert {c["k1"] for c in s0.seen} == {319}
        assert {c["k2"] for c in s0.seen} == {6492}
        f0_ids = {c["l1_order_id"] for i, c in enumerate(s0.seen) if i % 2 == 0}
        f1_ids = {c["l1_order_id"] for i, c in enumerate(s0.seen) if i % 2 == 1}
        assert len(f0_ids) == 1 and len(f1_ids) == 1
        assert f0_ids != f1_ids  # different L1 order objects: the swap
        assert {c["l2_order_id"] for c in s0.seen} == {s0.seen[0]["l2_order_id"]}
        stub_hex = run.summary["order_digest"]
        for record in run.records:
            if record["arm"] == "F1_new_l1_order_base":
                assert record["l1_prefix_len"] == 319
                assert record["l2_prefix_len"] == 6492
                assert record["l1_order_source"] == "new-1p5M"
                assert record["l1_order_digest"] == stub_hex
                assert record["key_bit_delta_vs_F0"] == 0
            elif record["arm_kind"] == "operational":
                assert record["l1_prefix_len"] == 319
                assert record["l2_prefix_len"] == 6492
                assert record["l1_order_source"] == "old-1M"
                assert record["l1_order_digest"] == pe.FROZEN_OLD_L1_ORDER_DIGEST
                assert record["key_bit_delta_vs_F0"] == 0
            else:
                assert record["l1_order_source"] == "true-L1-oracle"
                assert record["l1_order_digest"] is None
                assert record["key_bit_delta_vs_F0"] is None


def test_f1_restored_differential_is_descriptive_and_complete():
    # F0 fails block 0 while F1 restores it: the differential is recorded
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert recovery["f1_restored_count"] == 1
        assert recovery["order_blocks"][0]["f1_restored_given_f0_failed"] is True
        assert recovery["order_blocks"][1]["f1_restored_given_f0_failed"] is False
        assert run.summary["outcome_label"] == pe.COMPLETE_LABEL
        assert run.summary["integrity_all_pass"] is True
...
def test_incremental_k_arithmetic_exact():
    ...
    assert 5 * (319 + 6492) + 64 == 34119  # F1 identical budget
    assert 34119 - 34119 == 0  # key delta F1 vs F0
    assert 5 * 6492 + 64 == 32524
    assert 10 * 32768 + 63 == 327743
    assert 3 * 34119 + 3 * 34119 + 3 * 32524 == TOTAL_KEY_BITS == 302286
    assert 3 * 34119 + 3 * 34119 == 204714  # operational
    assert 3 * 32524 == 97572  # oracle
    assert 9 * 327743 == TOTAL_PUBLIC_BITS


def test_p20l_seed_domain_arm_block_separation():
    seeds = {}
    for arm in pe.FROZEN_ARM_NAMES:
        for block in range(3):
            seed = pe.l1_order_1p5m_seed_bits(TEST_MASTER, 32768, arm, block)
    ...
    assert pe.SEED_PREFIX == "nbpolar-p20l-l1-order-1p5m-seed"
    assert p20k.SEED_PREFIX == "nbpolar-p20k-l1-dose-512-1p5m-seed"
    assert pe.FROZEN_TAG_MASTER not in (hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER,
                                        p20j.FROZEN_TAG_MASTER, p20k.FROZEN_TAG_MASTER,
                                        2026092210, 2026092220,
                                        2026092230, 2026092240, 2026092250)
    assert_raises_match(ValueError, "unknown frozen arm",
                         pe.l1_order_1p5m_seed_bits, TEST_MASTER, 32768, "F9", 0)
...
def test_disclosure_diagnostics_present_and_descriptive():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        recovery = run.summary["aggregates"]["recovery"]
        assert set(recovery) == {"per_arm", "f1_restored_count", "order_blocks"}
...
def test_counts_buckets_and_recount_exact_and_tamper():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for record in run.records:
            ...
            events.extend(pe.l1_order_1p5m_block_events(
                32768, spec, record["block_index"], record["frame_start"], record))
        recount = pe.l1_order_1p5m_recount_events(events)
        ...
        assert pe.l1_order_1p5m_recount_events(tampered)["key_dependent_bits"] != TOTAL_KEY_BITS
        ...
        assert_raises_match(ValueError, "N/arm-tagged",
                            pe.l1_order_1p5m_recount_events, bad_arm)
        ...
        assert_raises_match(ValueError, "not frozen",
                            pe.l1_order_1p5m_recount_events, bad_type)
...
def test_no_threshold_labels_all_recovery_patterns():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert pe.l1_order_1p5m_label(gates) == "BLOCKED(undetected_zero)"
            ...
            with patched(pe, "FROZEN_PRIOR_DIGEST", prior["digest"]):
                with patched(pe, "FROZEN_ORDER_DIGEST", ohex):
                    with patched(pe, "run_operational_block", s0):
                        ...
                        except pe.L1Order1p5mResourceError as err:
...
def test_calibration_identity_failure_blocked_no_sc_call():
    ...
    # SC call (prior identity); wrong expected literals refuse the
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fab_path, fab_hex = make_construction_file(fab_dir)
        man_path = make_manifest(tmp_path)
        # First case: the order stub is provenance-bound to the REAL prior
        # pin (unpatched below), so the order gate passes and the prior
        # gate fires.
        okw_real, ohex_real = order_kwargs(tmp_path, pe.FROZEN_PRIOR_DIGEST)
        okw, ohex = order_kwargs(tmp_path, prior["digest"], tamper=None,
                                 name="stub_order.json")
        s0 = FakeOperationalArm()
        s2 = FakeControl()
        with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
            with patched(pe, "FROZEN_ORDER_DIGEST", ohex_real):
                with patched(pe, "run_operational_block", s0):
                    with patched(pe, "run_oracle_control_block", s2):
                        assert_raises_match(
...
def test_module_required_tokens_and_no_forbidden_paths():
    ...
    for token in ("FROZEN_DEV_FRAME_RANGE = (1152, 1535)",
                   ...
                   "F1_new_l1_order_base", "F2_true_l1_diagnostic", "F0_old_order_base",
                   "deployable", "f1_restored_count", "disclosure_blocks",
                   "l1_delta_k1_applied", "frozen-order-prefix-extension",
                    ...
                    "nbpolar-p20l-l1-dose-512-1p5m-seed"):
        assert token in source, token
...
def test_l1_prefix_positions_pin_order_prefix_extension():
    ...
    # K2 stays at base on every P20L arm: the L2 prefix helper pins the
    # carried-over base length on the same frozen order.
    assert np.array_equal(pe.l2_prefix_positions(order, 6492), order[:6492])


def test_tiny_real_operational_and_oracle_via_p20l_closure():
    from comparison_bench.src.comparison_bench.formal_ir.nbpolar.two_layer import (
        seed_bits_for,
        labels_to_bits,
    )
    ...
    def tag_fn(bits, _seed, tag_bits, _fixed=None):
        seed = pe.l1_order_1p5m_seed_bits(TEST_MASTER, n, "F0_old_order_base", 0,
