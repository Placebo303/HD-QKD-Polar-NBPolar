"""Focused Phase 4-P20G N=32768 +1024 independent-session tests.
...
content-opened (real-mode tests use throwaway temporary stubs with
patched loaders); the real P16 construction root, the real split
manifest, every real evidence root, the 1M same-file pools (TRAIN
...
``2026092211..2026092217`` and never the frozen P20G tag master 2026092210
(nor the P20F master 2026092200, the P20E master 2026092100, the P20C
master 2026092090, the P20B master 2026092080, the P19 master 2026092060,
the P18 master 2026092050, the P16/P17 streams, nor any prior/probe
seed) for real scoring.
...
checkpointing all run for real. Zero tuning carry-over from P20F is
asserted literally (same K/floor/caps/order-prefix rule, pinned against
the P20F module read-only), the single +1024 L2 step is asserted as a
...
double gate (cross-file source-tag+digest gate against the 1M full pool
and the reserved 2M file first, then the intra-file DEV-vs-VAL/HOLD
overlap gate) is asserted to refuse before any root exists, with zero
...
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
    plus1024_independent_session as pe,
)
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
...
TEST_SEEDS = (2026092211, 2026092212, 2026092213, 2026092214)
TEST_MASTER = 2026092215
TEST_SPARES = (2026092216, 2026092217)
...
def make_construction_file(directory, *, tamper=None):
    ...
    doc = {
        "protocol": "nbpolar-p20g-tampered" if tamper == "protocol"
    ...
    with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
        with patched(pe, "run_operational_block", s0):
            with patched(pe, "run_oracle_control_block", s2):
                run = pe.run_plus1024_independent_session(**kw)
    return run, s0, s2
...
def test_frozen_constants_and_arm_table_pinned():
    ...
    assert pe.FROZEN_TAG_MASTER == 2026092210
...
def test_zero_tuning_carry_over_from_p20f():
    ...
    assert pe.INTEGRITY_GATE_ORDER == p20f.INTEGRITY_GATE_ORDER
...
def test_fresh_test_seeds_disjoint_from_all_frozen_seeds():
    ...
    assert pe.FROZEN_TAG_MASTER not in (2026092050, 2026092060, 2026092080,
                                        2026092090, p20f.FROZEN_TAG_MASTER)
...
def test_accepted_helpers_shared_not_reimplemented():
    ...
    # which justifies the local P20G formation with identical semantics.
    ...
    # source is refused, which justifies the local P20G manifest check
                        ...
                        with patched(pe, "run_operational_block", forbidden_arm):
                            with patched(pe, "run_oracle_control_block", forbidden_control):
                                assert_raises_match(
                                    ...
                                    pe.run_plus1024_independent_session,
...
def test_double_gate_source_first_then_val_then_hold():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for over, substr in (
            ...
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
                assert_raises_match(
                    ValueError, substr, pe.run_plus1024_independent_session,
                    counts=injected_counts(), dev_table=injected_dev_table(),
                    construction=str(fab_path), construction_digest=fab_hex,
...
def test_p20g_seed_domain_arm_block_separation():
    seeds = {}
    ...
    assert pe.SEED_PREFIX == "nbpolar-p20g-independent-session-seed"
    assert pe.FROZEN_TAG_MASTER not in (hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER,
                                        p20f.FROZEN_TAG_MASTER)
...
def test_no_threshold_labels_all_recovery_patterns():
    with tempfile.TemporaryDirectory() as tmp:
        run, _, _ = full_run(Path(tmp) / "out")
        assert run.summary["outcome_label"] == pe.COMPLETE_LABEL
        assert pe.plus1024_independent_session_label(
        ...
        assert pe.plus1024_independent_session_label(gates) == "BLOCKED(undetected_zero)"
...
def test_budget_stop_abort_fill_and_precedence():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
            with patched(pe, "run_operational_block", s0):
                with patched(pe, "run_oracle_control_block", s2):
                    run = pe.run_plus1024_independent_session(
        ...
        with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
            with patched(pe, "run_operational_block", s0):
                with patched(pe, "run_oracle_control_block", s2):
                    ...
                    except pe.Plus1024IndependentSessionResourceError as err:
...
def test_cli_parser_and_run_contract_refusals_before_open():
    ...
    args = parser.parse_args([
        "--counts", "c", "--source", "1p5M", "--floor", "1e-15",
        ...
        "--tag-master", "2026092210", "--chunk-rows", "512",
        "--tag-bits", "64", "--out-dir", "o",
    ])
    assert args.dev_frames == [0, 383]
    assert args.tag_master == 2026092210
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        fab_path, fab_hex = make_construction_file(tmp_path)
        man = make_manifest(tmp_path)
...
def test_tiny_real_operational_and_oracle_via_p20g_closure():
    from comparison_bench.src.comparison_bench.formal_ir.nbpolar.two_layer import (
        seed_bits_for,
        labels_to_bits,
    )
