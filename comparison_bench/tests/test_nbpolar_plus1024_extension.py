"""Focused Phase 4-P20F N=32768 +1024 extension tests.
...
closed HOLD blocks 1600..1983, the consumed P20B VAL pool 1200..1599, the
consumed P20C DEV blocks 0..383 and the consumed P20E DEV blocks 384..767
are never touched. Focused tests use their own fresh seeds
``2026092201..2026092207`` and never the frozen P20F tag master 2026092200
(nor the P20E master 2026092100, the P20C master 2026092090, the P20B
master 2026092080, the P19 master 2026092060, the P18 master 2026092050,
the P16/P17 streams, nor any prior/probe seed) for real scoring.
...
checkpointing all run for real. Zero tuning carry-over from P20E is
asserted literally (same K/floor/caps/order-prefix rule, pinned against
the P20E module read-only), the single +1024 L2 step is asserted as a
single-step disclosure differential (B1 k2=7516 on the SAME frozen L2
order object as B0, key-bit delta exactly +5120), and the quadruple
overlap gate (consumed P20C DEV first, then consumed P20E DEV, then
consumed VAL, then closed HOLD) is asserted to refuse before any root
exists. Every ``test_*`` takes no arguments, uses plain asserts and
restores any monkeypatched module attribute in a ``finally``.
"""
...
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
    plus1024_extension as pe,
)
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
    plus1024_confirmation as p20e,  # read-only zero-tuning carry-over reference
)
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
...
TEST_SEEDS = (2026092201, 2026092202, 2026092203, 2026092204)
TEST_MASTER = 2026092205
TEST_SPARES = (2026092206, 2026092207)
...
def restored_open_guards():
    ...
    npz, dev = pe._NPZ_CONTENT_OPENED, pe._DEV_PARQUET_CONTENT_OPENED
    try:
        yield
    finally:
        pe._NPZ_CONTENT_OPENED = npz
...
def make_construction_file(directory, *, tamper=None):
    ...
    doc = {
        "protocol": "nbpolar-p20f-tampered" if tamper == "protocol"
...
def injected_dev_table(*, frames=384, first_frame=768, per_frame=256,
    ...
    (1152..1199, counted but never decoded); ``extras=True`` adds one frame
    per forbidden pool (consumed P20C DEV 0, consumed P20E DEV 384,
    consumed VAL 1200, closed HOLD 1600), present in the file but never
    selected.
    """
    rng = np.random.default_rng(seed)
    ...
    rem_frames = np.repeat(np.arange(1152, 1200), per_frame)
    rem = pd.DataFrame({
        "frame_id": rem_frames,
        "pair_idx": np.tile(np.arange(per_frame), 48),
    ...
    if extras:
        ...
        # P20C DEV, consumed P20E DEV, consumed VAL, closed HOLD):
        # present in the file but never selected.
        extra = pd.DataFrame({
            "frame_id": np.repeat([0, 384, 1200, 1600], per_frame),
...
def mutate_dev(df, how):
    ...
    if how == "drop-frame":
        ...
        out = out[out["frame_id"].isin([0, 384, 1200, 1600])]
    elif how == "drop-remainder-frame":
        out = out[out["frame_id"] != 1160]
    ...
    with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
        with patched(pe, "run_operational_block", s0):
            with patched(pe, "run_oracle_control_block", s2):
                run = pe.run_plus1024_extension(**kw)
    return run, s0, s2
...
def test_frozen_constants_and_arm_table_pinned():
    ...
    assert pe.FROZEN_REMAINDER_FRAME_RANGE == (1152, 1199)
    assert pe.FROZEN_REMAINDER_FRAMES == 48
    assert pe.FROZEN_REMAINDER_SYMBOLS == 12288
    assert pe.FROZEN_MANIFEST_TRAIN_FRAMES == 1200
    assert pe.FROZEN_MANIFEST_TRAIN_PAIRS == 307200
    assert pe.CLOSED_HOLD_FRAME_RANGE == (1600, 1983)
    assert pe.CONSUMED_VAL_FRAME_RANGE == (1200, 1599)
    assert pe.CONSUMED_P20C_DEV_FRAME_RANGE == (0, 383)
    assert pe.CONSUMED_P20E_DEV_FRAME_RANGE == (384, 767)
    assert pe.FROZEN_TAG_MASTER == 2026092200
    ...
    assert "--remainder-frames 1152 1199" in pe.FROZEN_COMMAND
    assert "--tag-master 2026092200" in pe.FROZEN_COMMAND
    assert "plus1024_extension" in pe.FROZEN_COMMAND
    assert "1200 1599" not in pe.FROZEN_COMMAND
    assert "384 767" not in pe.FROZEN_COMMAND
...
def test_zero_tuning_carry_over_from_p20e():
    # Every algorithm input is carried over byte-identical from the P20E
    ...
    assert pe.FROZEN_N == p20e.FROZEN_N == 32768
    assert pe.FROZEN_FLOOR == p20e.FROZEN_FLOOR == 1e-15
    assert (pe.FROZEN_K1, pe.FROZEN_K2, pe.FROZEN_K_TOTAL) == (
        p20e.FROZEN_K1, p20e.FROZEN_K2, p20e.FROZEN_K_TOTAL) == (319, 6492, 6811)
    assert pe.FROZEN_L2_DELTA_K2 == p20e.FROZEN_L2_DELTA_K2 == 1024
    assert pe.FROZEN_K2_B1 == p20e.FROZEN_K2_B1 == 7516
    assert pe.FROZEN_K_TOTAL_B1 == p20e.FROZEN_K_TOTAL_B1 == 7835
    assert pe.FROZEN_B1_KEY_BIT_DELTA == p20e.FROZEN_B1_KEY_BIT_DELTA == 5120
    assert pe.FROZEN_LEAKAGE_BITS == p20e.FROZEN_LEAKAGE_BITS == 34119
    assert pe.FROZEN_B1_LEAKAGE_BITS == p20e.FROZEN_B1_LEAKAGE_BITS == 39239
    assert pe.FROZEN_CONTROL_LEAKAGE_BITS == p20e.FROZEN_CONTROL_LEAKAGE_BITS == 32524
    assert pe.FROZEN_PUBLIC_CONTROL_BITS == p20e.FROZEN_PUBLIC_CONTROL_BITS == 327743
    assert pe.FROZEN_RAW_INPUT_BITS == p20e.FROZEN_RAW_INPUT_BITS == 327680
    assert pe.FROZEN_TOTAL_KEY_DEPENDENT_BITS == p20e.FROZEN_TOTAL_KEY_DEPENDENT_BITS
    assert pe.FROZEN_TOTAL_PUBLIC_CONTROL_BITS == p20e.FROZEN_TOTAL_PUBLIC_CONTROL_BITS
    assert pe.PLANNED_SC_CALLS == p20e.PLANNED_SC_CALLS == 15
    assert pe.PLANNED_TAG_INVOCATIONS == p20e.PLANNED_TAG_INVOCATIONS == 9
    assert pe.PLANNED_RECORDS == p20e.PLANNED_RECORDS == 9
    assert pe.FROZEN_ARM_NAMES == p20e.FROZEN_ARM_NAMES
    assert pe.OPERATIONAL_PROVENANCE == p20e.OPERATIONAL_PROVENANCE
    assert pe.ORACLE_PROVENANCE == p20e.ORACLE_PROVENANCE
    assert pe.DISCLOSURE_RULE == p20e.DISCLOSURE_RULE
    assert pe.OUTCOMES == p20e.OUTCOMES
    assert pe.INTEGRITY_GATE_ORDER == p20e.INTEGRITY_GATE_ORDER
    # The deliberate deltas: new-block population + new tag domain only.
    assert pe.FROZEN_DEV_FRAME_RANGE != p20e.FROZEN_DEV_FRAME_RANGE
    assert pe.FROZEN_BLOCK_RANGES != p20e.FROZEN_BLOCK_RANGES
    assert pe.FROZEN_TAG_MASTER != p20e.FROZEN_TAG_MASTER
    assert pe.SEED_PREFIX != p20e.SEED_PREFIX
    assert pe.PROTOCOL_NAME != p20e.PROTOCOL_NAME
    assert pe.COMPLETE_LABEL != p20e.COMPLETE_LABEL


def test_fresh_test_seeds_disjoint_from_all_frozen_seeds():
    frozen = (
        set(opf.FROZEN_DEV_SEEDS) | set(opf.FROZEN_TRAIN_SEEDS)
        | {hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER, p20e.FROZEN_TAG_MASTER,
    ...
    assert pe.FROZEN_TAG_MASTER not in (2026092050, 2026092060, 2026092080,
                                        2026092090, p20e.FROZEN_TAG_MASTER)
...
def test_accepted_helpers_shared_not_reimplemented():
    ...
    assert pe.target_preconditions.__module__.endswith("target_construction")
    assert pe.load_v25_channel_counts.__module__.endswith("v35_algorithm_development")
...
def test_predecessor_identity_tamper_refusals_zero_reads_zero_root():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for idx, tamper in enumerate(("k1", "k2", "order-dup", "n", "digest", "protocol")):
            ...
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
                with patched(pe, "load_v25_channel_counts", forbidden_loader):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for idx, tamper in enumerate(("train-frames", "train-pairs", "hold-frames",
            ...
            assert_raises_match(ValueError, "manifest identity",
                                pe.verify_dev_manifest, man, source="1M")
            root = tmp_path / f"out{idx}"
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
                with patched(pe, "load_v25_channel_counts", forbidden_loader):
                    with patched(pe, "load_pairs_table", forbidden_loader):
        ...
        for field, value, substr in (
            ("FROZEN_REMAINDER_FRAME_RANGE", (1153, 1199), "remainder_frame_range"),
...
def test_quadruple_overlap_gate_p20c_first_then_p20e_then_val_then_hold():
    ...
    assert pin["consumed_p20e_dev_frame_range"] == [384, 767]
    assert pin["consumed_val_frame_range"] == [1200, 1599]
    # Any DEV block touching the consumed P20C DEV range refuses FIRST.
    with patched(pe, "FROZEN_BLOCK_RANGES", ((0, 127), (896, 1023), (1024, 1151))):
        assert_raises_match(ValueError, "overlaps consumed P20C DEV",
                            pe.verify_dev_block_identity)
    with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (0, 1151)):
        assert_raises_match(ValueError, "remainder overlaps consumed P20C",
                            pe.verify_dev_block_identity)
    # A remainder overlapping all four pools reports the P20C pool first.
    with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (0, 1600)):
        assert_raises_match(ValueError, "consumed P20C",
                            pe.verify_dev_block_identity)
    # Any DEV block touching the consumed P20E DEV range refuses second.
    with patched(pe, "FROZEN_BLOCK_RANGES", ((384, 511), (896, 1023), (1024, 1151))):
        assert_raises_match(ValueError, "overlaps consumed P20E DEV",
                            pe.verify_dev_block_identity)
    with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (384, 1151)):
        assert_raises_match(ValueError, "remainder overlaps consumed P20E",
                            pe.verify_dev_block_identity)
    # A remainder overlapping P20E, VAL and HOLD reports the P20E pool first.
    with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (384, 1600)):
        assert_raises_match(ValueError, "consumed P20E",
                            pe.verify_dev_block_identity)
    # Any DEV block touching the consumed VAL pool refuses third.
    with patched(pe, "FROZEN_BLOCK_RANGES", ((768, 895), (896, 1023), (1024, 1200))):
        assert_raises_match(ValueError, "overlaps consumed VAL",
                            pe.verify_dev_block_identity)
    with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (1152, 1200)):
        assert_raises_match(ValueError, "remainder overlaps consumed VAL",
                            pe.verify_dev_block_identity)
    # Any DEV block touching the closed HOLD range refuses fourth.
    with patched(pe, "FROZEN_BLOCK_RANGES", ((768, 895), (896, 1023), (1600, 1727))):
        assert_raises_match(ValueError, "overlaps closed HOLD",
                            pe.verify_dev_block_identity)
    with patched(pe, "FROZEN_REMAINDER_FRAME_RANGE", (1600, 1727)):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for over, substr in (
            ...
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
                assert_raises_match(
                    ValueError, substr, pe.run_plus1024_extension,
                    counts=injected_counts(), dev_table=injected_dev_table(),
                    construction=str(fab_path), construction_digest=fab_hex,
...
def test_dev_slicing_blocks_and_remainder_exact():
    ...
    assert formation["remainder"]["frames"] == 48
    assert formation["remainder"]["symbols"] == 12288
    assert formation["remainder"]["used"] is False
    closed_first, closed_last = pe.CLOSED_HOLD_FRAME_RANGE
    consumed_first, consumed_last = pe.CONSUMED_VAL_FRAME_RANGE
    p20c_first, p20c_last = pe.CONSUMED_P20C_DEV_FRAME_RANGE
    p20e_first, p20e_last = pe.CONSUMED_P20E_DEV_FRAME_RANGE
    for start, end in formation["block_ranges"]:
        ...
        assert end < p20e_first or start > p20e_last
...
def test_one_open_guards_refuse_reopen_with_zero_execution():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        man = make_manifest(tmp_path)
        with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
            with patched(pe, "_NPZ_CONTENT_OPENED", True):
                with patched(pe, "load_v25_channel_counts", forbidden_loader):
                    assert_raises_match(
    ...
    assert pe._NPZ_CONTENT_OPENED is False
...
def test_p20f_seed_domain_arm_block_separation():
    seeds = {}
    ...
    assert pe.SEED_PREFIX == "nbpolar-p20f-plus1024-extension-seed"
    assert pe.FROZEN_TAG_MASTER not in (hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER,
                                        p20e.FROZEN_TAG_MASTER)
...
def test_no_threshold_labels_all_recovery_patterns():
    with tempfile.TemporaryDirectory() as tmp:
        run, _, _ = full_run(Path(tmp) / "out")
        assert run.summary["outcome_label"] == pe.COMPLETE_LABEL
        assert pe.plus1024_extension_label(
        ...
        assert pe.plus1024_extension_label(gates) == "BLOCKED(undetected_zero)"
        ...
        with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
            with patched(pe, "run_operational_block", s0):
                with patched(pe, "run_oracle_control_block", s2):
                    ...
                    except pe.Plus1024ExtensionResourceError as err:
...
def test_real_mode_accounting_with_patched_loaders():
    with restored_open_guards():
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
                with patched(pe, "EXPECTED_NPZ_BYTES", 4):
                    with patched(pe, "load_v25_channel_counts", fake_counts):
...
def test_cli_parser_and_run_contract_refusals_before_open():
    ...
    args = parser.parse_args([
        ...
        "--block-frames", "128", "--remainder-frames", "1152", "1199",
        "--tag-master", "2026092200", "--chunk-rows", "512",
        "--tag-bits", "64", "--out-dir", "o",
    ])
    assert args.dev_frames == [768, 1151]
    assert args.tag_master == 2026092200
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        fab_path, fab_hex = make_construction_file(tmp_path)
        man = make_manifest(tmp_path)
        for over, substr in (
            ...
            ({"tag_master": 2026092100}, "tag-master"),
        ):
...
def test_module_required_tokens_and_no_forbidden_paths():
    ...
    for token in ("FROZEN_DEV_FRAME_RANGE = (768, 1151)",
                  ...
                  "CONSUMED_P20E_DEV_FRAME_RANGE = (384, 767)",
        ...
        assert token in source, token
    for token in ("plus1024_confirmation", "Plus1024Confirmation",
...
def test_tiny_real_operational_and_oracle_via_p20f_closure():
    from comparison_bench.src.comparison_bench.formal_ir.nbpolar.two_layer import (
        seed_bits_for,
        labels_to_bits,
    )
