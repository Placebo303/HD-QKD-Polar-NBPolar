"""Focused Phase 4-P20J N=32768 L1-dose-escalation +256 single-factor tests.
...
384..767) and the reserved 2M file are never touched.
Focused tests use their own fresh seeds ``2026092241..2026092247`` and
never the frozen P20J tag master 2026092240 (nor the P20I master
2026092230, the P20H master 2026092220, the P20G master 2026092210, the
P20F master 2026092200, the P20E master 2026092100, the P20C master
2026092090, the P20B master 2026092080, the P19 master 2026092060, the
P18 master 2026092050, the P16/P17 streams, nor any prior/probe seed)
for real scoring.
...
prior-reuse rule, pinned against the P20I module read-only) plus the
single deliberate delta (the frozen +256 L1 order-prefix step,
K1 319 -> 575 on the SAME frozen P16 L1 order) are asserted literally,
the single +256 L1 step is asserted as a single-step disclosure
differential (D1 k1=575 on the SAME frozen L1 order object as D0,
key-bit delta exactly +1280), and the fail-closed quintuple gate
...
consumed-P20I-DEV exclusion gate, then the
calibration-identity digest gate) is asserted to refuse before any SC
call, with zero protected opens and the 2M file never touched. Every
``test_*`` takes no arguments, uses plain asserts and restores any
...
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
    l1_dose_escalation_1p5m as pe,
)
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
    l1_disclosure_1p5m as p20i,  # read-only prior-reuse/population carry-over reference
)
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
...
TEST_SEEDS = (2026092241, 2026092242, 2026092243, 2026092244)
TEST_MASTER = 2026092245
TEST_SPARES = (2026092246, 2026092247)

FROZEN_BASE_LEAKAGE = 34119  # 5 * 6811 + 64 (C0 base operational)
FROZEN_D1_LEAKAGE = 35399  # 5 * 7067 + 64 (D1, +256 L1)
FROZEN_CONTROL_LEAKAGE = 32524  # 5 * 6492 + 64 (oracle arm)
FROZEN_PUBLIC_BITS = 327743  # 10 * 32768 + 63
TOTAL_KEY_BITS = 3 * FROZEN_BASE_LEAKAGE + 3 * FROZEN_D1_LEAKAGE + 3 * FROZEN_CONTROL_LEAKAGE  # 306126
...
def injected_dev_table(*, frames=384, first_frame=768, per_frame=256,
                       seed=TEST_SEEDS[1], extras=True):
    """Deterministic TRAIN-like pairs frame; shuffled rows, extra non-DEV rows.

    Carries the DEV pool (768..1151) plus the declared never-used remainder
    (1152..1659, counted but never decoded); ``extras=True`` adds one frame
    per intra-file forbidden pool (1.5M VAL 1660, 1.5M HOLD 2213) plus one
    frame of the consumed P20H DEV prefix (frame 0) and one frame of the
    consumed P20I DEV segment (frame 384), present in the file but never
    selected.
    """
    rng = np.random.default_rng(seed)
    ...
    rem_frames = np.repeat(np.arange(1152, 1660), per_frame)
    rem = pd.DataFrame({
        "frame_id": rem_frames,
        "pair_idx": np.tile(np.arange(per_frame), 508),
    ...
    if extras:
        ...
        # 1.5M HOLD, consumed P20H DEV): present in the file but never
        # selected.
        extra = pd.DataFrame({
            "frame_id": np.repeat([1660, 2213, 0, 384], per_frame),
            "pair_idx": np.tile(np.arange(per_frame), 4),
            "alice_symbol": rng.integers(0, 1024, size=4 * per_frame),
            "bob_symbol": rng.integers(0, 1024, size=4 * per_frame),
        })
...
def mutate_dev(df, how):
    ...
    if how == "drop-frame":
        ...
        row = out[(out["frame_id"] == 768) & (out["pair_idx"] == 255)].index[0]
        out.loc[row, "pair_idx"] = 254
    elif how == "symbol-range":
        row = out[out["frame_id"] == 768].index[0]
...
class FakeOperationalArm:
    """Scripted fake for the accepted ``run_operational_block`` seam (D0 + D1).

    One shared seam serves both operational arms; the arm is identified by
    the disclosed ``k1`` (319 base vs 575 +256 step) and the frozen L1
...
class FakeControl:
    """Scripted fake for the ``run_oracle_control_block`` (D2) seam."""
    ...
    def __call__(self, **kwargs):
        ...
        self.seen.append({
            "arm": "D2_true_l1_diagnostic", "outcome": outcome,
        ...
        with patched(pe, "FROZEN_PRIOR_DIGEST", prior["digest"]):
            with patched(pe, "run_operational_block", s0):
                with patched(pe, "run_oracle_control_block", s2):
                    run = pe.run_l1_dose_escalation_1p5m(**kw)
    return run, s0, s2
...
def test_frozen_constants_and_arm_table_pinned():
    ...
    assert pe.FROZEN_L1_DELTA_K1 == 256
    assert pe.FROZEN_K1_D1 == 575
    assert pe.FROZEN_K_TOTAL_D1 == 7067
    assert pe.FROZEN_D1_KEY_BIT_DELTA == 1280
    assert pe.FROZEN_DEV_FRAME_RANGE == (768, 1151)
    assert pe.FROZEN_DEV_FRAMES == 384
    assert pe.FROZEN_DEV_PAIRS == 98304
    assert pe.FROZEN_BLOCK_RANGES == ((768, 895), (896, 1023), (1024, 1151))
    assert pe.FROZEN_REMAINDER_FRAME_RANGE == (1152, 1659)
    assert pe.FROZEN_REMAINDER_FRAMES == 508
    assert pe.FROZEN_REMAINDER_SYMBOLS == 130048
    ...
    assert pe.FROZEN_TAG_MASTER == 2026092240
    assert pe.SEED_PREFIX == "nbpolar-p20j-l1-dose-escalation-1p5m-seed"
    ...
    assert pe.FROZEN_D1_LEAKAGE_BITS == FROZEN_D1_LEAKAGE
    ...
    assert abs(pe.FROZEN_D1_LEAKAGE_BITS / pe.FROZEN_RAW_INPUT_BITS - 0.1080291748046875) < 1e-12
    pin = pe.check_frozen_arm_table()
    assert pin["l1_delta_k1"] == 256
    assert pin["d1_k1"] == 575
    assert pin["d1_key_bit_delta_vs_base"] == 1280
    assert pin["cap_vs_raw_ratio"] < 0.5
    assert pin["d1_cap_vs_raw_ratio"] < 0.5
    ...
    assert "--dev-frames 768 1151" in pe.FROZEN_COMMAND
    assert "--remainder-frames 1152 1659" in pe.FROZEN_COMMAND
    assert "--tag-master 2026092240" in pe.FROZEN_COMMAND
    ...
    assert "l1_dose_escalation_1p5m" in pe.FROZEN_COMMAND
    assert "--dev-frames 384 767" not in pe.FROZEN_COMMAND
    assert "--remainder-frames 768 1659" not in pe.FROZEN_COMMAND
    assert "--tag-master 2026092230" not in pe.FROZEN_COMMAND
    with patched(pe, "FROZEN_K1_D1", 574):
        assert_raises_match(ValueError, "256", pe.check_frozen_arm_table)
    with patched(pe, "FROZEN_D1_LEAKAGE_BITS", 35398):
        assert_raises_match(ValueError, "drifted", pe.check_frozen_arm_table)


def test_zero_tuning_carry_over_from_p20i():
    # Every carried-over input is byte-identical with the P20I module
    ...
    assert pe.FROZEN_N == p20i.FROZEN_N == 32768
    assert pe.FROZEN_FLOOR == p20i.FROZEN_FLOOR == 1e-15
    assert (pe.FROZEN_K1, pe.FROZEN_K2, pe.FROZEN_K_TOTAL) == (
        p20i.FROZEN_K1, p20i.FROZEN_K2, p20i.FROZEN_K_TOTAL) == (319, 6492, 6811)
    assert pe.FROZEN_LEAKAGE_BITS == p20i.FROZEN_LEAKAGE_BITS == 34119
    assert pe.FROZEN_CONTROL_LEAKAGE_BITS == p20i.FROZEN_CONTROL_LEAKAGE_BITS == 32524
    assert pe.FROZEN_PUBLIC_CONTROL_BITS == p20i.FROZEN_PUBLIC_CONTROL_BITS == 327743
    assert pe.FROZEN_RAW_INPUT_BITS == p20i.FROZEN_RAW_INPUT_BITS == 327680
    assert pe.PLANNED_SC_CALLS == p20i.PLANNED_SC_CALLS == 15
    assert pe.PLANNED_TAG_INVOCATIONS == p20i.PLANNED_TAG_INVOCATIONS == 9
    assert pe.PLANNED_RECORDS == p20i.PLANNED_RECORDS == 9
    assert pe.OPERATIONAL_PROVENANCE == p20i.OPERATIONAL_PROVENANCE
    assert pe.ORACLE_PROVENANCE == p20i.ORACLE_PROVENANCE
    assert pe.OUTCOMES == p20i.OUTCOMES
    ...
    assert pe.FROZEN_PRIOR_PATH == p20i.FROZEN_PRIOR_PATH
    assert pe.FROZEN_PRIOR_DIGEST == p20i.FROZEN_PRIOR_DIGEST == (
        "e8dd078a5367ba3af8eba91022d58aeffc114bc30b847b13cd4bb890e5dde43b")
    assert pe.FROZEN_LAMBDA == p20i.FROZEN_LAMBDA == 137.3823795883264
    assert (pe.FROZEN_CAL_H1, pe.FROZEN_CAL_H2, pe.FROZEN_CAL_H_TOTAL) == (
        p20i.FROZEN_CAL_H1, p20i.FROZEN_CAL_H2, p20i.FROZEN_CAL_H_TOTAL)
    assert pe.SUPPORT_RULE == p20i.SUPPORT_RULE
    assert pe.INTEGRITY_GATE_ORDER == p20i.INTEGRITY_GATE_ORDER
    assert "calibration_identity" in pe.INTEGRITY_GATE_ORDER
    # The factor swap: P20J discloses +256 L1 (D1 K1=575, K2 stays base)
    # where P20I disclosed +128 L1 (C1 K1=447, K2 stayed base).
    assert pe.FROZEN_L1_DELTA_K1 == 256
    assert pe.FROZEN_K1_D1 == 575
    assert pe.FROZEN_K_TOTAL_D1 == 7067
    assert pe.FROZEN_D1_KEY_BIT_DELTA == 1280
    assert pe.FROZEN_D1_LEAKAGE_BITS == 35399
    assert p20i.FROZEN_L1_DELTA_K1 == 128
    assert p20i.FROZEN_K1_C1 == 447
    assert not hasattr(pe, "FROZEN_K1_C1")
    assert not hasattr(p20i, "FROZEN_K1_D1")
    assert pe.FROZEN_TOTAL_KEY_DEPENDENT_BITS == TOTAL_KEY_BITS == 306126
    assert pe.FROZEN_TOTAL_KEY_DEPENDENT_BITS != p20i.FROZEN_TOTAL_KEY_DEPENDENT_BITS
    assert pe.FROZEN_TOTAL_PUBLIC_CONTROL_BITS == p20i.FROZEN_TOTAL_PUBLIC_CONTROL_BITS
    assert pe.FROZEN_ARM_NAMES != p20i.FROZEN_ARM_NAMES
    assert pe.FROZEN_ARM_NAMES == ("D0_sc_base", "D1_L1plus", "D2_true_l1_diagnostic")
    assert pe.DISCLOSURE_RULE != p20i.DISCLOSURE_RULE
    assert "575" in pe.DISCLOSURE_RULE
    # The deliberate deltas: new-segment population + new tag domain only.
    assert pe.FROZEN_SOURCE == p20i.FROZEN_SOURCE == "1p5M"
    assert pe.FROZEN_DEV_FRAME_RANGE != p20i.FROZEN_DEV_FRAME_RANGE
    assert pe.FROZEN_BLOCK_RANGES != p20i.FROZEN_BLOCK_RANGES
    assert pe.FROZEN_REMAINDER_FRAME_RANGE != p20i.FROZEN_REMAINDER_FRAME_RANGE
    assert pe.P20H_DEV_FRAME_RANGE == p20i.P20H_DEV_FRAME_RANGE == (0, 383)
    assert not hasattr(p20i, "P20I_DEV_FRAME_RANGE")
    assert pe.P20I_DEV_FRAME_RANGE == (384, 767)
    assert pe.FROZEN_TAG_MASTER != p20i.FROZEN_TAG_MASTER
    assert pe.SEED_PREFIX != p20i.SEED_PREFIX
    assert pe.PROTOCOL_NAME != p20i.PROTOCOL_NAME
    assert pe.COMPLETE_LABEL != p20i.COMPLETE_LABEL


def test_fresh_test_seeds_disjoint_from_all_frozen_seeds():
    frozen = (
        set(opf.FROZEN_DEV_SEEDS) | set(opf.FROZEN_TRAIN_SEEDS)
        | {hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER, p20i.FROZEN_TAG_MASTER,
    ...
    assert pe.FROZEN_TAG_MASTER not in (2026092050, 2026092060, 2026092080,
                                        ...
                                        2026092230, p20i.FROZEN_TAG_MASTER)
...
def test_accepted_helpers_shared_not_reimplemented():
    ...
    # which justifies the local P20I formation with identical semantics.
    assert_raises_match(ValueError, "hold-frames",
                         hm.form_holdout_blocks, injected_dev_table(),
                         hold_frames=(768, 1151), block_frames=128,
                         remainder_frames=(1152, 1659))
    # The accepted split-manifest helper is source-pinned to 1M: the 1p5M
    # source is refused, which justifies the local P20I manifest check
                        ...
                        with patched(pe, "run_operational_block", forbidden_arm):
                            with patched(pe, "run_oracle_control_block", forbidden_control):
                                assert_raises_match(
                                    ...
                                    pe.run_l1_dose_escalation_1p5m,
...
def test_manifest_and_dev_pin_refusals_zero_reads_zero_root():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for field, value in (
            ("FROZEN_BLOCK_RANGES", ((768, 894), (895, 1022), (1023, 1150))),
        ):
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
        ...
        for field, value, substr in (
            ("FROZEN_REMAINDER_FRAME_RANGE", (1153, 1659), "remainder_frame_range"),
            ("FROZEN_DEV_FRAME_RANGE", (768, 1152), "dev_frame_range"),
...
def test_quintuple_gate_source_first_then_val_then_hold_then_p20h_dev_then_p20i_dev():
    ...
    with patched(pe, "FROZEN_BLOCK_RANGES", ((0, 127), (768, 895), (896, 1023))):
    ...
    with patched(pe, "FROZEN_BLOCK_RANGES", ((384, 511), (768, 895), (896, 1023))):
    ...
    with patched(pe, "FROZEN_BLOCK_RANGES", ((0, 127), (896, 1023), (1660, 1787))):
        assert_raises_match(ValueError, "overlaps 1.5M VAL",
                            pe.verify_dev_block_identity)
    # A block touching both P20I DEV and VAL reports the VAL set first
    # (intra-file gate runs before the P20I-DEV exclusion gate).
    with patched(pe, "FROZEN_BLOCK_RANGES", ((384, 511), (896, 1023), (1660, 1787))):
        assert_raises_match(ValueError, "overlaps 1.5M VAL",
                            pe.verify_dev_block_identity)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        man = make_manifest(tmp_path)
        for over, substr in (
            ({"dev_frames": (769, 1152)}, "dev-frames"),
            ({"dev_frames": (768, 1152)}, "dev-frames"),
            ({"dev_frames": (384, 767)}, "dev-frames"),
            ({"remainder_frames": (1153, 1659)}, "remainder-frames"),
            ({"source": "1M"}, "source="),
            ({"source": "2M"}, "source="),
            ...
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
                assert_raises_match(
                    ValueError, substr, pe.run_l1_dose_escalation_1p5m,
                    dev_table=injected_dev_table(), **prior_kwargs(),
                    construction=str(fab_path), construction_digest=fab_hex,
...
def test_dev_slicing_blocks_and_remainder_exact():
    ...
    assert formation["dev_frame_range"] == [768, 1151]
    assert formation["dev_frames"] == 384
    assert formation["dev_pairs"] == 98304
    assert formation["block_ranges"] == [[768, 895], [896, 1023], [1024, 1151]]
    assert [b["frame_start"] for b in formation["blocks"]] == [768, 896, 1024]
    assert all(len(b["bob"]) == 32768 for b in formation["blocks"])
    assert formation["remainder"]["frames"] == 508
    assert formation["remainder"]["symbols"] == 130048
...
def test_no_fitting_or_sampling_tokens_and_exact_call_counts():
    ...
    for token in ("l1_dose_escalation_seed_bits", "form_dev_blocks", "l1_prefix_positions",
                  "l2_prefix_positions",
...
def test_three_arm_semantics_k_leakage_labels_and_call_counts():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert run.summary["aggregates"]["arms"]["D0_sc_base"]["key_dependent_bits"] == 3 * 34119
        assert run.summary["aggregates"]["arms"]["D1_L1plus"]["key_dependent_bits"] == 3 * 35399
        assert run.summary["aggregates"]["arms"]["D2_true_l1_diagnostic"]["key_dependent_bits"] == 3 * 32524
        assert run.summary["aggregates"]["operational"]["key_dependent_bits"] == 3 * 34119 + 3 * 35399
        ...
        # Slot order is block-major D0/D1/D2; every arm ran on every block.
        assert sorted(r["block_index"] for r in run.records
                      if r["arm"] == "D1_L1plus") == [0, 1, 2]
        # The single-factor differential: D1 discloses exactly +256 L1
        # symbols on the SAME frozen L1 order object as D0, with K2 at
        # base on every arm.
        d0_calls = [c for c in s0.seen if c["k1"] == 319]
        d1_calls = [c for c in s0.seen if c["k1"] == 575]
        assert len(d0_calls) == 3 and len(d1_calls) == 3
        ...
        for record in run.records:
            if record["arm"] == "D1_L1plus":
                assert record["l1_prefix_len"] == 575
                assert record["l2_prefix_len"] == 6492
                assert record["l1_delta_k1_applied"] == 256
                assert record["l1_disclosure_rule"] == "frozen-order-prefix-extension"
                assert record["key_bit_delta_vs_base"] == 1280
...
def test_d1_restored_differential_is_descriptive_and_complete():
    # D0 fails block 0 while D1 restores it: the differential is recorded
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert recovery["d1_restored_count"] == 1
        assert recovery["disclosure_blocks"][0]["d1_restored_given_d0_failed"] is True
        assert recovery["disclosure_blocks"][1]["d1_restored_given_d0_failed"] is False
        assert run.summary["outcome_label"] == pe.COMPLETE_LABEL
        assert run.summary["integrity_all_pass"] is True
...
def test_incremental_k_arithmetic_exact():
    ...
    assert 5 * (575 + 6492) + 64 == 35399
    assert 35399 - 34119 == 1280
    assert 1280 == 5 * 256
    assert 5 * 6492 + 64 == 32524
    assert 10 * 32768 + 63 == 327743
    assert 3 * 34119 + 3 * 35399 + 3 * 32524 == TOTAL_KEY_BITS == 306126
    assert 3 * 34119 + 3 * 35399 == 208554  # operational
    assert 3 * 32524 == 97572  # oracle
    assert 9 * 327743 == TOTAL_PUBLIC_BITS


def test_p20j_seed_domain_arm_block_separation():
    seeds = {}
    for arm in pe.FROZEN_ARM_NAMES:
        for block in range(3):
            seed = pe.l1_dose_escalation_seed_bits(TEST_MASTER, 32768, arm, block)
    ...
    assert pe.FROZEN_TAG_MASTER not in (hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER,
                                        p20i.FROZEN_TAG_MASTER, 2026092210, 2026092220,
                                        2026092230)
    assert_raises_match(ValueError, "unknown frozen arm",
                         pe.l1_dose_escalation_seed_bits, TEST_MASTER, 32768, "D0", 0)
...
def test_disclosure_diagnostics_present_and_descriptive():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        recovery = run.summary["aggregates"]["recovery"]
        assert set(recovery) == {"per_arm", "d1_restored_count", "disclosure_blocks"}
...
def test_counts_buckets_and_recount_exact_and_tamper():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for record in run.records:
            ...
            events.extend(pe.l1_dose_escalation_block_events(
                32768, spec, record["block_index"], record["frame_start"], record))
        recount = pe.l1_dose_escalation_recount_events(events)
        ...
        assert pe.l1_dose_escalation_recount_events(tampered)["key_dependent_bits"] != TOTAL_KEY_BITS
        ...
        assert_raises_match(ValueError, "N/arm-tagged",
                            pe.l1_dose_escalation_recount_events, bad_arm)
        ...
        assert_raises_match(ValueError, "not frozen",
                            pe.l1_dose_escalation_recount_events, bad_type)
...
def test_no_threshold_labels_all_recovery_patterns():
    with tempfile.TemporaryDirectory() as tmp:
        run, _, _ = full_run(Path(tmp) / "out")
        assert run.summary["outcome_label"] == pe.COMPLETE_LABEL
        assert pe.l1_dose_escalation_1p5m_label(
        ...
        assert pe.l1_dose_escalation_1p5m_label(gates) == "BLOCKED(undetected_zero)"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
            with patched(pe, "FROZEN_PRIOR_DIGEST", prior["digest"]):
                with patched(pe, "run_operational_block", s0):
                    with patched(pe, "run_oracle_control_block", s2):
                        run = pe.run_l1_dose_escalation_1p5m(
            ...
            with patched(pe, "FROZEN_PRIOR_DIGEST", prior["digest"]):
                with patched(pe, "run_operational_block", s0):
                    with patched(pe, "run_oracle_control_block", s2):
                        ...
                        except pe.L1DoseEscalation1p5mResourceError as err:
...
def test_cli_parser_and_run_contract_refusals_before_open():
    ...
    args = parser.parse_args([
        ...
        "--dev-pairs", "p", "--dev-frames", "768", "1151",
        "--block-frames", "128", "--remainder-frames", "1152", "1659",
        "--tag-master", "2026092240", "--chunk-rows", "512",
        "--tag-bits", "64", "--out-dir", "o",
    ])
    assert args.dev_frames == [768, 1151]
    assert args.tag_master == 2026092240
    assert args.prior == "p"
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        fab_path, fab_hex = make_construction_file(tmp_path)
        ...
        for over, substr in (
            ...
            ({"dev_frames": (768, 1152)}, "dev-frames"),
            ({"tag_master": 2026092230}, "tag-master"),
        ):
...
def test_module_required_tokens_and_no_forbidden_paths():
    source = Path(pe.__file__).read_text(encoding="utf-8")
    for token in ("FROZEN_DEV_FRAME_RANGE = (768, 1151)",
                   ...
                   "D1_L1plus", "D2_true_l1_diagnostic", "D0_sc_base",
                   "deployable", "d1_restored_count", "disclosure_blocks",
                   "l1_delta_k1_applied", "frozen-order-prefix-extension",
                    "key_bit_delta_vs_base", "FROZEN_D1_LEAKAGE_BITS = 35399",
                    "FROZEN_K1_D1 = 575",
...
def test_tiny_real_operational_and_oracle_via_p20j_closure():
    ...
    def tag_fn(bits, _seed, tag_bits, _fixed=None):
        seed = pe.l1_dose_escalation_seed_bits(TEST_MASTER, n, "D0_sc_base", 0,
