"""Focused Phase 4-P20I N=32768 L1-disclosure +128 single-factor tests.
...
prefix (1.5M TRAIN 0..383) and the reserved 2M file are never touched.
Focused tests use their own fresh seeds ``2026092231..2026092237`` and
never the frozen P20I tag master 2026092230 (nor the P20H master
2026092220, the P20G master 2026092210, the P20F master 2026092200, the
P20E master 2026092100, the P20C master 2026092090, the P20B master
...
prior-reuse rule, pinned against the P20H module read-only) plus the
single deliberate delta (the frozen +128 L1 order-prefix step,
K1 319 -> 447 on the SAME frozen P16 L1 order) are asserted literally,
the single +128 L1 step is asserted as a single-step disclosure
differential (C1 k1=447 on the SAME frozen L1 order object as C0,
key-bit delta exactly +640), and the fail-closed quadruple gate
...
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
    l1_disclosure_1p5m as pe,
)
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
    plus1024_per_session_confirmation as p20h,  # read-only prior-reuse/population carry-over reference
)
from comparison_bench.src.comparison_bench.formal_ir.nbpolar import (
...
TEST_SEEDS = (2026092231, 2026092232, 2026092233, 2026092234)
TEST_MASTER = 2026092235
TEST_SPARES = (2026092236, 2026092237)

FROZEN_BASE_LEAKAGE = 34119  # 5 * 6811 + 64 (C0 base operational)
FROZEN_C1_LEAKAGE = 34759  # 5 * 6939 + 64 (C1, +128 L1)
FROZEN_CONTROL_LEAKAGE = 32524  # 5 * 6492 + 64 (oracle arm)
FROZEN_PUBLIC_BITS = 327743  # 10 * 32768 + 63
TOTAL_KEY_BITS = 3 * FROZEN_BASE_LEAKAGE + 3 * FROZEN_C1_LEAKAGE + 3 * FROZEN_CONTROL_LEAKAGE  # 304206
...
def injected_dev_table(*, frames=384, first_frame=384, per_frame=256,
    ...
    (768..1659, counted but never decoded); ``extras=True`` adds one frame
    per intra-file forbidden pool (1.5M VAL 1660, 1.5M HOLD 2213) plus one
    frame of the consumed P20H DEV prefix (frame 0), present in the file
    but never selected.
    """
    rng = np.random.default_rng(seed)
    ...
    rem_frames = np.repeat(np.arange(768, 1660), per_frame)
    rem = pd.DataFrame({
        "frame_id": rem_frames,
        "pair_idx": np.tile(np.arange(per_frame), 892),
    ...
    if extras:
        ...
        # selected.
        extra = pd.DataFrame({
            "frame_id": np.repeat([1660, 2213, 0], per_frame),
...
class FakeOperationalArm:
    """Scripted fake for the accepted ``run_operational_block`` seam (C0 + C1).

    One shared seam serves both operational arms; the arm is identified by
    the disclosed ``k1`` (319 base vs 447 +128 step) and the frozen L1
...
class FakeControl:
    """Scripted fake for the ``run_oracle_control_block`` (C2) seam."""
    ...
    def __call__(self, **kwargs):
        ...
        self.seen.append({
            "arm": "C2_true_l1_diagnostic", "outcome": outcome,
        ...
        with patched(pe, "FROZEN_PRIOR_DIGEST", prior["digest"]):
            with patched(pe, "run_operational_block", s0):
                with patched(pe, "run_oracle_control_block", s2):
                    run = pe.run_l1_disclosure_1p5m(**kw)
    return run, s0, s2
...
def test_frozen_constants_and_arm_table_pinned():
    ...
    assert pe.FROZEN_L1_DELTA_K1 == 128
    assert pe.FROZEN_K1_C1 == 447
    assert pe.FROZEN_K_TOTAL_C1 == 6939
    assert pe.FROZEN_C1_KEY_BIT_DELTA == 640
    assert pe.FROZEN_DEV_FRAME_RANGE == (384, 767)
    assert pe.FROZEN_DEV_FRAMES == 384
    assert pe.FROZEN_DEV_PAIRS == 98304
    assert pe.FROZEN_BLOCK_RANGES == ((384, 511), (512, 639), (640, 767))
    assert pe.FROZEN_REMAINDER_FRAME_RANGE == (768, 1659)
    assert pe.FROZEN_REMAINDER_FRAMES == 892
    assert pe.FROZEN_REMAINDER_SYMBOLS == 228352
    ...
    assert pe.FROZEN_TAG_MASTER == 2026092230
    assert pe.SEED_PREFIX == "nbpolar-p20i-l1-disclosure-1p5m-seed"
    ...
    assert pe.FROZEN_C1_LEAKAGE_BITS == FROZEN_C1_LEAKAGE
    ...
    assert abs(pe.FROZEN_C1_LEAKAGE_BITS / pe.FROZEN_RAW_INPUT_BITS - 0.1060760498046875) < 1e-12
    pin = pe.check_frozen_arm_table()
    assert pin["l1_delta_k1"] == 128
    assert pin["c1_k1"] == 447
    assert pin["c1_key_bit_delta_vs_base"] == 640
    assert pin["cap_vs_raw_ratio"] < 0.5
    assert pin["c1_cap_vs_raw_ratio"] < 0.5
    ...
    assert "--dev-frames 384 767" in pe.FROZEN_COMMAND
    assert "--remainder-frames 768 1659" in pe.FROZEN_COMMAND
    assert "--tag-master 2026092230" in pe.FROZEN_COMMAND
    ...
    assert "l1_disclosure_1p5m" in pe.FROZEN_COMMAND
    assert "--dev-frames 0 383" not in pe.FROZEN_COMMAND
    assert "--remainder-frames 384 1659" not in pe.FROZEN_COMMAND
    assert "--tag-master 2026092220" not in pe.FROZEN_COMMAND
    with patched(pe, "FROZEN_K1_C1", 446):
        assert_raises_match(ValueError, "128", pe.check_frozen_arm_table)
    with patched(pe, "FROZEN_C1_LEAKAGE_BITS", 34758):
        assert_raises_match(ValueError, "drifted", pe.check_frozen_arm_table)


def test_zero_tuning_carry_over_from_p20h():
    # Every carried-over input is byte-identical with the P20H module
    ...
    assert pe.FROZEN_N == p20h.FROZEN_N == 32768
    assert pe.FROZEN_FLOOR == p20h.FROZEN_FLOOR == 1e-15
    assert (pe.FROZEN_K1, pe.FROZEN_K2, pe.FROZEN_K_TOTAL) == (
        p20h.FROZEN_K1, p20h.FROZEN_K2, p20h.FROZEN_K_TOTAL) == (319, 6492, 6811)
    assert pe.FROZEN_LEAKAGE_BITS == p20h.FROZEN_LEAKAGE_BITS == 34119
    assert pe.FROZEN_CONTROL_LEAKAGE_BITS == p20h.FROZEN_CONTROL_LEAKAGE_BITS == 32524
    assert pe.FROZEN_PUBLIC_CONTROL_BITS == p20h.FROZEN_PUBLIC_CONTROL_BITS == 327743
    assert pe.FROZEN_RAW_INPUT_BITS == p20h.FROZEN_RAW_INPUT_BITS == 327680
    assert pe.PLANNED_SC_CALLS == p20h.PLANNED_SC_CALLS == 15
    assert pe.PLANNED_TAG_INVOCATIONS == p20h.PLANNED_TAG_INVOCATIONS == 9
    assert pe.PLANNED_RECORDS == p20h.PLANNED_RECORDS == 9
    assert pe.OPERATIONAL_PROVENANCE == p20h.OPERATIONAL_PROVENANCE
    assert pe.ORACLE_PROVENANCE == p20h.ORACLE_PROVENANCE
    assert pe.OUTCOMES == p20h.OUTCOMES
    ...
    assert pe.FROZEN_PRIOR_PATH == p20h.FROZEN_PRIOR_PATH
    assert pe.FROZEN_PRIOR_DIGEST == p20h.FROZEN_PRIOR_DIGEST == (
        "e8dd078a5367ba3af8eba91022d58aeffc114bc30b847b13cd4bb890e5dde43b")
    assert pe.FROZEN_LAMBDA == p20h.FROZEN_LAMBDA == 137.3823795883264
    assert (pe.FROZEN_CAL_H1, pe.FROZEN_CAL_H2, pe.FROZEN_CAL_H_TOTAL) == (
        p20h.FROZEN_CAL_H1, p20h.FROZEN_CAL_H2, p20h.FROZEN_CAL_H_TOTAL)
    assert pe.SUPPORT_RULE == p20h.SUPPORT_RULE
    assert pe.INTEGRITY_GATE_ORDER == p20h.INTEGRITY_GATE_ORDER
    assert "calibration_identity" in pe.INTEGRITY_GATE_ORDER
    # The factor swap: P20I discloses +128 L1 (C1 K1=447, K2 stays base)
    # where P20H disclosed +1024 L2 (B1 K2=7516, K1 stayed base).
    assert pe.FROZEN_L1_DELTA_K1 == 128
    assert pe.FROZEN_K1_C1 == 447
    assert pe.FROZEN_K_TOTAL_C1 == 6939
    assert pe.FROZEN_C1_KEY_BIT_DELTA == 640
    assert pe.FROZEN_C1_LEAKAGE_BITS == 34759
    assert p20h.FROZEN_L2_DELTA_K2 == 1024
    assert p20h.FROZEN_K2_B1 == 7516
    assert not hasattr(pe, "FROZEN_L2_DELTA_K2")
    assert not hasattr(p20h, "FROZEN_L1_DELTA_K1")
    assert pe.FROZEN_TOTAL_KEY_DEPENDENT_BITS == TOTAL_KEY_BITS == 304206
    assert pe.FROZEN_TOTAL_KEY_DEPENDENT_BITS != p20h.FROZEN_TOTAL_KEY_DEPENDENT_BITS
    assert pe.FROZEN_TOTAL_PUBLIC_CONTROL_BITS == p20h.FROZEN_TOTAL_PUBLIC_CONTROL_BITS
    assert pe.FROZEN_ARM_NAMES != p20h.FROZEN_ARM_NAMES
    assert pe.FROZEN_ARM_NAMES == ("C0_sc_base", "C1_L1plus", "C2_true_l1_diagnostic")
    assert pe.DISCLOSURE_RULE != p20h.DISCLOSURE_RULE
    assert "447" in pe.DISCLOSURE_RULE
    # The deliberate deltas: new-segment population + new tag domain only.
    assert pe.FROZEN_SOURCE == p20h.FROZEN_SOURCE == "1p5M"
    assert pe.FROZEN_DEV_FRAME_RANGE != p20h.FROZEN_DEV_FRAME_RANGE
    assert pe.FROZEN_BLOCK_RANGES != p20h.FROZEN_BLOCK_RANGES
    assert pe.FROZEN_REMAINDER_FRAME_RANGE != p20h.FROZEN_REMAINDER_FRAME_RANGE
    assert not hasattr(p20h, "P20H_DEV_FRAME_RANGE")
    assert pe.P20H_DEV_FRAME_RANGE == (0, 383)
    assert pe.FROZEN_TAG_MASTER != p20h.FROZEN_TAG_MASTER
    assert pe.SEED_PREFIX != p20h.SEED_PREFIX
    assert pe.PROTOCOL_NAME != p20h.PROTOCOL_NAME
    assert pe.COMPLETE_LABEL != p20h.COMPLETE_LABEL


def test_fresh_test_seeds_disjoint_from_all_frozen_seeds():
    frozen = (
        set(opf.FROZEN_DEV_SEEDS) | set(opf.FROZEN_TRAIN_SEEDS)
        | {hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER, p20h.FROZEN_TAG_MASTER,
...
def test_manifest_and_dev_pin_refusals_zero_reads_zero_root():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for field, value, substr in (
            ("FROZEN_REMAINDER_FRAME_RANGE", (769, 1659), "remainder_frame_range"),
...
def test_quadruple_gate_source_first_then_val_then_hold_then_p20h_dev():
    ...
    with patched(pe, "FROZEN_BLOCK_RANGES", ((0, 127), (384, 511), (512, 639))):
    ...
    with patched(pe, "FROZEN_BLOCK_RANGES", ((0, 127), (512, 639), (1660, 1787))):
        assert_raises_match(ValueError, "overlaps 1.5M VAL",
                            pe.verify_dev_block_identity)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        man = make_manifest(tmp_path)
        for over, substr in (
            ({"dev_frames": (385, 768)}, "dev-frames"),
            ({"dev_frames": (384, 768)}, "dev-frames"),
            ({"dev_frames": (0, 383)}, "dev-frames"),
            ({"remainder_frames": (769, 1659)}, "remainder-frames"),
            ({"source": "1M"}, "source="),
            ({"source": "2M"}, "source="),
            ...
            with patched(pe, "FROZEN_CONSTRUCTION_DIGEST", fab_hex):
                assert_raises_match(
                    ValueError, substr, pe.run_l1_disclosure_1p5m,
                    dev_table=injected_dev_table(), **prior_kwargs(),
                    construction=str(fab_path), construction_digest=fab_hex,
...
def test_dev_slicing_blocks_and_remainder_exact():
    ...
    assert formation["dev_frame_range"] == [384, 767]
    ...
    assert formation["remainder"]["frames"] == 892
    assert formation["remainder"]["symbols"] == 228352
...
def test_no_fitting_or_sampling_tokens_and_exact_call_counts():
    ...
    for token in ("l1_disclosure_seed_bits", "form_dev_blocks", "l1_prefix_positions",
                  "l2_prefix_positions",
...
def test_three_arm_semantics_k_leakage_labels_and_call_counts():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert run.summary["aggregates"]["arms"]["C0_sc_base"]["key_dependent_bits"] == 3 * 34119
        assert run.summary["aggregates"]["arms"]["C1_L1plus"]["key_dependent_bits"] == 3 * 34759
        assert run.summary["aggregates"]["arms"]["C2_true_l1_diagnostic"]["key_dependent_bits"] == 3 * 32524
        assert run.summary["aggregates"]["operational"]["key_dependent_bits"] == 3 * 34119 + 3 * 34759
        ...
        # Slot order is block-major C0/C1/C2; every arm ran on every block.
        assert sorted(r["block_index"] for r in run.records
                      if r["arm"] == "C1_L1plus") == [0, 1, 2]
        # The single-factor differential: C1 discloses exactly +128 L1
        # symbols on the SAME frozen L1 order object as C0, with K2 at
        # base on every arm.
        c0_calls = [c for c in s0.seen if c["k1"] == 319]
        c1_calls = [c for c in s0.seen if c["k1"] == 447]
        assert len(c0_calls) == 3 and len(c1_calls) == 3
        ...
        for record in run.records:
            if record["arm"] == "C1_L1plus":
                assert record["l1_prefix_len"] == 447
                assert record["l2_prefix_len"] == 6492
                assert record["l1_delta_k1_applied"] == 128
                assert record["l1_disclosure_rule"] == "frozen-order-prefix-extension"
                assert record["key_bit_delta_vs_base"] == 640
...
def test_c1_restored_differential_is_descriptive_and_complete():
    # C0 fails block 0 while C1 restores it: the differential is recorded
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert recovery["c1_restored_count"] == 1
        assert recovery["disclosure_blocks"][0]["c1_restored_given_c0_failed"] is True
        assert recovery["disclosure_blocks"][1]["c1_restored_given_c0_failed"] is False
        assert run.summary["outcome_label"] == pe.COMPLETE_LABEL
        assert run.summary["integrity_all_pass"] is True
...
def test_incremental_k_arithmetic_exact():
    ...
    assert 5 * (447 + 6492) + 64 == 34759
    assert 34759 - 34119 == 640
    assert 640 == 5 * 128
    assert 5 * 6492 + 64 == 32524
    assert 10 * 32768 + 63 == 327743
    assert 3 * 34119 + 3 * 34759 + 3 * 32524 == TOTAL_KEY_BITS == 304206
    assert 3 * 34119 + 3 * 34759 == 206634  # operational
    assert 3 * 32524 == 97572  # oracle
    assert 9 * 327743 == TOTAL_PUBLIC_BITS


def test_p20i_seed_domain_arm_block_separation():
    seeds = {}
    for arm in pe.FROZEN_ARM_NAMES:
        for block in range(3):
            seed = pe.l1_disclosure_seed_bits(TEST_MASTER, 32768, arm, block)
    ...
    assert pe.FROZEN_TAG_MASTER not in (hm.FROZEN_TAG_MASTER, hb.FROZEN_TAG_MASTER,
                                        p20h.FROZEN_TAG_MASTER, 2026092210, 2026092220)
    assert_raises_match(ValueError, "unknown frozen arm",
                         pe.l1_disclosure_seed_bits, TEST_MASTER, 32768, "C0", 0)
...
def test_disclosure_diagnostics_present_and_descriptive():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        recovery = run.summary["aggregates"]["recovery"]
        assert set(recovery) == {"per_arm", "c1_restored_count", "disclosure_blocks"}
...
def test_counts_buckets_and_recount_exact_and_tamper():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for record in run.records:
            ...
            events.extend(pe.l1_disclosure_block_events(
                32768, spec, record["block_index"], record["frame_start"], record))
        recount = pe.l1_disclosure_recount_events(events)
        ...
        assert pe.l1_disclosure_recount_events(tampered)["key_dependent_bits"] != TOTAL_KEY_BITS
        ...
        assert_raises_match(ValueError, "N/arm-tagged",
                            pe.l1_disclosure_recount_events, bad_arm)
        ...
        assert_raises_match(ValueError, "not frozen",
                            pe.l1_disclosure_recount_events, bad_type)
...
def test_no_threshold_labels_all_recovery_patterns():
    with tempfile.TemporaryDirectory() as tmp:
        run, _, _ = full_run(Path(tmp) / "out")
        assert run.summary["outcome_label"] == pe.COMPLETE_LABEL
        assert pe.l1_disclosure_1p5m_label(
        ...
        assert pe.l1_disclosure_1p5m_label(gates) == "BLOCKED(undetected_zero)"
            ...
            with patched(pe, "FROZEN_PRIOR_DIGEST", prior["digest"]):
                with patched(pe, "run_operational_block", s0):
                    with patched(pe, "run_oracle_control_block", s2):
                        ...
                        except pe.L1Disclosure1p5mResourceError as err:
...
def test_cli_parser_and_run_contract_refusals_before_open():
    ...
    args = parser.parse_args([
        ...
        "--block-frames", "128", "--remainder-frames", "768", "1659",
        "--tag-master", "2026092230", "--chunk-rows", "512",
        "--tag-bits", "64", "--out-dir", "o",
    ])
    assert args.dev_frames == [384, 767]
    assert args.tag_master == 2026092230
    assert args.prior == "p"
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        fab_path, fab_hex = make_construction_file(tmp_path)
        ...
        for over, substr in (
            ...
            ({"dev_frames": (384, 768)}, "dev-frames"),
            ({"tag_master": 2026092220}, "tag-master"),
        ):
...
def test_module_required_tokens_and_no_forbidden_paths():
    ...
    for token in ("FROZEN_DEV_FRAME_RANGE = (384, 767)",
                  ...
                  "C1_L1plus", "C2_true_l1_diagnostic", "C0_sc_base",
                  "deployable", "c1_restored_count", "disclosure_blocks",
                  "l1_delta_k1_applied", "frozen-order-prefix-extension",
                   "key_bit_delta_vs_base", "FROZEN_C1_LEAKAGE_BITS = 34759",
                   "FROZEN_K1_C1 = 447",
...
def test_tiny_real_operational_and_oracle_via_p20i_closure():
    ...
    def tag_fn(bits, _seed, tag_bits, _fixed=None):
        seed = pe.l1_disclosure_seed_bits(TEST_MASTER, n, "C0_sc_base", 0,
