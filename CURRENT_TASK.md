Status: **M2-prior G2 adjudicated `NBPOLAR_M2_PRIOR_G2_SUCCESS` (Tier-Y one-shot three-arm decode, SHG `_1`, w=200/CIRCULAR; B 11/14 vs A2 0/14 strict Wilson non-overlap; A1 0/14 descriptive no-gate; undetected 0 isolated; one-shot g2_runs 1 reruns 0; M2 stays CANDIDATE); D4 adjudicated `NBPOLAR_M2_PRIOR_K_RESPLIT_D4_COMPLETE_REPORT_ONLY` (neither branch adopted; G2/G3 stay frozen 319/6492); next active task = G3-CONFIRM Phase 0 + A, dispatch pending milestone commit (G3 modifies `scripts/m2_prior_validation.py`; SHG `_2` untouched)** — 2026-09-22

Previous status (retained): **M2-prior G1R2 adjudicated `NBPOLAR_M2_PRIOR_G1R2_COMPLETE_DESCRIPTIVE` (descriptive/non-claim, decoder-free; w=200 CIRCULAR δ-tail PASS is a consequence of S11, not new ±1 validation; q_rest=0 both populations at 32-frame resolution; M2 stays CANDIDATE, not validated; next gate = main-thread route decision — G2 freeze preparation if authorized; T6/T7 no longer blocked by G1; G3 still void unless G2 succeeds)** — 2026-09-22

G1R2 (delta-successor of the executed+adjudicated G1 contract: w=500→200, MOD LINEAR_ONLY→CIRCULAR; 8 CHANGED items, item 8 = runner re-parameterization) executed once on SHG `_1`, descriptive / non-claim, decoder-free; decoder never called; SHG `_2` untouched; G1's record byte-identical. Basis: verbatim user authorization (kai, 2026-09-22) + operator all-complete return (both phases) + independent delta review `DELTA_PASS_WITH_COMMENTS` (after one DELTA_FAIL cycle on the runner block, reworked) + independent closure verification `G1R2_CLOSURE_PASS_WITH_COMMENTS` + independent focused numerical review `G1R2_NUMERICS_PASS_WITH_COMMENTS` (all numbers independently recomputed). Reproduction 5/5 exact; post-skip ledger 4,219; reserve 4190–4218 (29). CHAR δ-profile (200,192 pairs): circular tail **0** (linear tail 45; wrap accounting closes exactly); δ=0 fraction 75.382% vs full-stream 75.419%. CAL32 circular triple q0/q+1/q−1/q_rest = 0.7562255859375/0.241943359375/0.0018310546875/**0**; H_M2 **0.8168138**, H_M0 0.6910589; held-out NLL_M0 2.9623453 / NLL_M2 0.8231435, Δ = 2.1392018 > 0.020 (PASS). δ-tail PASS (U = 1.4986e-5 < B_tail 2.0e-4). Remainder (frozen sessions, no gate): q_rest **0**, pooled Δ = 1.9575437; remainder H_M0 bit-identical across contracts (0.7081275). Adjudication: (1) δ-tail PASS is a consequence of S11, NOT new ±1 validation — new information is the CAL32 triple, H values, held-out NLL; (2) q_rest=0 both populations is weak by construction (3/8192 = 3.66e-4), hence the CHAR gate; (3) M0 sparse-table pathology confirmed again (Δ 2.1392; pooled 1.9575 ≈ G1's 1.9579); (4) no cross-contract superiority — G1's bounded negative stands; (5) M2 CANDIDATE, premise blocker on the G2 path lifted for this contract, but decode/FER/efficiency/reliability require the G2 one-shot three-arm decode (own freeze, Pre-EXECUTE + authorization). G2 inputs: H_M2 = 0.8168138 (D4 literal ⇒ K_total ≈ 6,946 vs frozen 7,020 — fixed-f vs fixed-K DEFERRED; G2 decodes at frozen K1=319/K2=6492 regardless; re-split only via frozen selector); truncation caveat (w=200 timing-truncated population) mandatory in any rate/efficiency/leakage reading; incumbent 1024-frame arm A1 is one of G2's three arms. Evidence: `.workbuddy/queue/NBPOLAR-M2-PRIOR-G1R2-W200-CIRCULAR/` (`G1R2_ADJUDICATION.md`, `DELTA_REVIEW.md`, `g1r2_freeze_config.json`) + `workspace/m2_prior_validation/20260113_SHG_Type2PPLN_3s_g1r2/` + `workspace/m2_prior_validation/remainder_101f_g1r2/`. OpenSpec `nbpolar-prior-rebaseline`: no box checked (G1R2 is a successor execution; T6/T7 remain unchecked; T2–T5 checked). Consistent with `docs/nbpolar/STATE.md` (M2 CANDIDATE; descriptive-only scope) except the route-consequence delta noted below.

Route-consequence delta vs STATE.md (reported, not fixed — STATE.md is main-thread owned): STATE.md §2/§4 record "T6/T7 blocked" under the G1 STOP; G1R2 lifts the premise blocker for this contract, so T6/T7 are no longer blocked BY G1 — they remain gated on the main-thread route decision + G2 freeze + explicit authorization, and G3 stays void unless G2 succeeds.

Previous status (retained): **S11 Tier-X probe adjudicated `NBPOLAR_S11_SHG_TAIL_NATURE_COMPLETE_DESCRIPTIVE` (descriptive/non-claim, decoder-free; w=500 δ-tail = window-scaling pairing contamination, not channel structure; w=200 CIRCULAR tail = 0; census far-offset baseline structured ≈0.40, not uniform-accidental; next gate = main-thread route decision — narrow-window-centred validation is the leading candidate route input, NOT selected); M2 stays CANDIDATE; G1 bounded negative stands; T6/T7 blocked** — 2026-09-21

S11 executed once per prereg on SHG `_1` (vendored (N) nearest-unique pairing, frozen constants, derived offset +50 ps, LINEAR_ONLY), descriptive / non-claim, decoder-free; decoder never called; no rerun/tuning/seed/window/K change. Basis: verbatim user authorization (Kai, 2026-09-21) + operator all-complete return (S11-P..S11-R PASS) after one main-thread scoped packet correction (anchor = post-skip frames 1056–1837, the frozen SEG_RANGES / G1 CHAR segment) + independent focused numerical review `S11_NUMERICS_PASS` (every recomputed number agrees to full precision; prereg predates body predates results; anchor cell-exact). One run, 19.5 s / 1.24 GiB. Anchor (w=500, skip-702): tail 1005/200,192, p̂ = 0.0050202 — cell-exact G1 CHAR profile {0: 149852, +1: 48639, −1: 696, tail: 1005}; full-stream n_pairs 1,269,268. W-grid p̂ = 0.0002389 (w=200, 301/1,259,992, entirely wrap-alias 8+293; CIRCULAR tail 0) / 0.0050155 (w=500) / 0.0168189 (w=1000) / 0.0394334 (w=2000); ratio 165.07. Offset scan (w=500) min at offset 0, spread 10.83% — offset-insensitive, no alignment signature. Far-offset (50±409600 ps) p̂ = 0.4067/0.3988 vs uniform 0.99707 — census far-offset baseline STRUCTURED, not a uniform-accidental reference. Clustering: 578/782 frames with ≥1 tail vs Poisson 565.70 (λ=1.28517); dispersion 0.94269 — independent-per-pair consistent. Tail vs census accidental estimates steeper than proportional (ratios 0.041→0.348→0.592→0.711; OLS slope 0.803, intercept −0.0055); "proportional" NOT supported. Adjudication binds nothing: no prior-form decision, no window decision, no route selection; post-hoc window switching stays forbidden; W_S=200 marginal bias (gauss cover 0.9247) stands; M2 CANDIDATE status and G1 bounded negative untouched. Route input for main-thread planning (not selected): narrow-window-centred validation is the leading candidate — needs its own freeze (W_P 500→200 contract change with bias caveat; MOD/tail-budget re-derivation noting w=200 LINEAR p̂ = 0.0002389 still exceeds frozen B_tail = 2.0e-4 while CIRCULAR tail = 0; gates re-preregistered; own authorization). Evidence: `.workbuddy/queue/NBPOLAR-S11-SHG-TAIL-NATURE/` (`S11_ADJUDICATION.md`) + `workspace/probes/nbpolar_s11_shg_tail_nature/` (gitignored). OpenSpec `nbpolar-prior-rebaseline` T6/T7 remain unchecked/blocked. Consistent with `docs/nbpolar/STATE.md` (M2 CANDIDATE; descriptive-only scope; no execution authorization).

Previous status (retained): **M2-prior G1 adjudicated `NBPOLAR_M2_PRIOR_G1_COMPLETE_ADJUDICATED_BOUNDED_NEGATIVE` (δ-tail FAIL ⇒ preregistered STOP, NO G2; NLL PASS near-uninformative by design); M2 stays CANDIDATE, not validated; next gate = main-thread route planning; T6/T7 blocked; G3 void unless new candidate** — 2026-09-21

G1 executed once per freeze on SHG `_1` (`20260113_SHG_Type2PPLN_3s`), descriptive / non-claim, decoder-free; decoder never called; SHG `_2` never read; no rerun/tuning/seed/window/K change. Basis: verbatim user authorization (kai, 20260921) + operator all-complete return (G1-0, G1-1..G1-4, remainder check) + independent closure verification `CLOSURE_PASS_WITH_COMMENTS` (Phase B unblocked) + independent focused numerical review `G1_NUMERICS_PASS` (every recomputed number agrees; gate rules applied exactly; no operator discretion in the FAIL). δ-tail FAIL: p̂ = 1005/200,192 = 0.0050202, U = 0.0052879 vs B_tail = 2.0e-4 (~25× over budget); linear profile {0: 149852, +1: 48639, −1: 696, tail: 1005}; the ±1 premise does not hold on this source. NLL PASS: Δ = 3.043859 − 1.093192 = 1.950667 > Δ_min 0.020, matched 32-frame CAL, FIT32 — with the binding caveat that matched-CAL M0 is NOT the incumbent (1024-frame regime measurable only by G2 arm A1, now blocked); this NLL never promotes M2. Remainder check (frozen sessions, NLL/H only, no gate): pooled Δ = 1.957900; per-segment 1.978823 / 2.024941 / 1.791009; matched-CAL M0 pathology replicates on both populations. What G1 does NOT establish: the M0 floor-mishandling mechanism on frozen sessions is not refuted; M2 is not refuted on every source (frozen tail ≈ 0, ladder exhausted — 101 frames / 69 after CAL); no new route licensed without preregistration. Evidence: `.workbuddy/queue/NBPOLAR-M2-PRIOR-G1-REALDATA-NLL/` (`G1_ADJUDICATION.md`, `FREEZE_REVIEW.md`, `g1_freeze_config.json`) + `workspace/m2_prior_validation/`. OpenSpec `nbpolar-prior-rebaseline` T5 complete; T6/T7 blocked; G3 void unless a new candidate emerges. Consistent with `docs/nbpolar/STATE.md` (M2 CANDIDATE; descriptive-only scope).

Previous status (retained): **M2-prior Stage-1 implementation COMPLETE/ACCEPTED (`NBPOLAR_M2_PRIOR_STAGE1_IMPLEMENTATION_COMPLETE_ACCEPTED`); focused review PASS_WITH_COMMENTS, zero blocking; next gate = Stage-2 G1 freeze preparation pending independent freeze review + explicit user authorization** — 2026-09-21 (superseded by the G1 adjudication above)

M2 Stage-1 delivered decoder-free, synthetic fixtures only, zero protected reads, zero data contact (no Pre-EXECUTE/Pre-RESULT gates applicable): strict M2 ±1 adapter beside `formal_ir/nbpolar/` (M0 switch bit-exact vs the P7 rule), T1–T12 suite 21/21, Spec-5 runner contract (19 TO-FREEZE keys null ⇒ exit 2; bodies exit 3), SECURITY_MODEL +39-line CAL note. S1-1..S1-8 all PASS. Scientific status unchanged, consistent with `docs/nbpolar/STATE.md`: M2 stays CANDIDATE pending real-data validation; S9 synthetic-only, never FER/efficiency evidence; no K decision (fixed-K vs fixed-f DEFERRED); no freeze values; G1/G2/G3 unauthorized. G2 (Tier-Y one-shot at frozen K1=319/K2=6492 on SHG `_1`) is a separate later freeze; SHG `_2` reserved for G3. Evidence: `.workbuddy/queue/NBPOLAR-M2-PRIOR-STAGE1-IMPLEMENTATION/` (`MAIN_THREAD_ACCEPTANCE.md`, `REVIEW.md`).

Previous status (retained): **Route A closed (population-exhaustion terminal, NOT a verdict closure); route B C-P1 = RETIRE-ceiling (axis (i) disclosure placement retired after 1 probe) and C-P2 (axis (ii) disclosure amount/structure) freeze review PASS with findings F1–F3 fixed (`a774b464`) — next gate = C-P2 B4 implementation pending explicit user authorization (route budget 2/6); literature fit-check corrections applied (`84e77bba`); SCL stays locked; Tier-X queue still 4/6** — 2026-09-21
