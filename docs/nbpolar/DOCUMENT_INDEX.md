...
| `../../.workbuddy/queue/NBPOLAR-M2-PRIOR-REALDATA-VALIDATION/` | M2-prior real-data validation parent packet (19 TO-FREEZE keys all filled — see `g1_freeze.md` + `g1r2_delta.md`; children own Stage 1 / G1 / G1R2 / G2 / D4 / G3) | Stage 1 accepted; **G1** bounded negative (w=500/LINEAR); **G1R2** descriptive PASS (w=200/CIRCULAR); **G2 SUCCESS** `NBPOLAR_M2_PRIOR_G2_SUCCESS`; G3 owned by `NBPOLAR-M2-PRIOR-G3-CONFIRM/` |
...
| `../../.workbuddy/queue/NBPOLAR-M2-PRIOR-G2-DECODE/G2_ADJUDICATION.md` | G2 Tier-Y one-shot three-arm decode adjudication (`NBPOLAR_M2_PRIOR_G2_SUCCESS`; B 11/14 vs A2 0/14 strict non-overlap; A1 0/14 descriptive; M2 stays CANDIDATE) | adjudicated 2026-09-22; T6/T7 complete; G3 precondition MET |
| `../../.workbuddy/queue/NBPOLAR-M2-PRIOR-K-RESPLIT-D4/D4_RECORD.md` | D4 K-resplit report-only record (`NBPOLAR_M2_PRIOR_K_RESPLIT_D4_COMPLETE_REPORT_ONLY`; neither branch adopted; G2/G3 stay frozen 319/6492) | complete 2026-09-22; planning input only |
| `../../.workbuddy/queue/NBPOLAR-M2-PRIOR-K-RESPLIT-D4/` | D4 packet dir (`TASK_PACKET.md` / `PROMPT.md` / `AUTHORIZATION_PROMPT.md` / `STATUS.yaml` / `D4_RECORD.md`) | report-only; `sc_calls_on_protected: 0`; `next_gate: MILESTONE_BATCH_COMPLETE` |
| `../../.workbuddy/queue/NBPOLAR-M2-PRIOR-G3-CONFIRM/` | G3-CONFIRM packet dir (`TASK_PACKET.md` / `PROMPT.md` / `AUTHORIZATION_PROMPT.md` / `STATUS.yaml`) | authorization operative 2026-09-22; dispatch held until milestone commit; SHG `_2` untouched |
