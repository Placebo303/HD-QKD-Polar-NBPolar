...
**Alternatives considered**:
- Authorize C-P2 B4 despite the dispersion margin: rejected — it would spend route budget testing a knob already shown 54.8σ in surplus. [SUPERSEDED 2026-09-21 — see :5287: the 54.8σ margin is an ideal-model ML/uniform-input optimistic bound, not a refutation of H-A (rate).]
...
## 2026-09-21 Prior form is the converging root cause: nonparametric M0 floor prices sparse rare cells at ~4e-18 (S8/S9/S10/F-accounting/V25-identity) [TITLE CORRECTED 2026-09-21: floor acts on probabilities ⇒ 40.4214 bits per floored true −1 delta, not ~4e-18/~48 bits; see :5287]
...
## 2026-09-21: M2-prior Stage-1 implementation accepted (CANDIDATE; S1-1..S1-8 PASS; focused review PASS_WITH_COMMENTS; G1 freeze preparation next)

**Decision**: Record packet `NBPOLAR-M2-PRIOR-STAGE1-IMPLEMENTATION` as `NBPOLAR_M2_PRIOR_STAGE1_IMPLEMENTATION_COMPLETE_ACCEPTED`. The Stage-1 decoder-free surface is complete and accepted: NEW strict M2 ±1 adapter `comparison_bench/src/comparison_bench/formal_ir/prior_m2.py` (beside — never inside — `formal_ir/nbpolar/`; numpy+stdlib, zero `nbpolar/` imports, `FLOOR=1e-15` literal, MOD frozen `LINEAR_ONLY` default, M0 switch reproduces the P7 raw-MLE-plus-floor rule bit-exact); NEW `comparison_bench/tests/test_nbpolar_prior_m2.py` (T1–T12, 21 passed, 0 failed, ~8 s); `scripts/m2_prior_validation.py` upgraded to the Spec-5 contract (all 19 TO-FREEZE keys enforced, null/absent ⇒ exit 2 never a default; `--authorized` store_true; flag↔key cross-check; K pinned 319/6492; `--out-root` confined to `workspace/`; post-validation bodies exit 3 `STAGE_BODY_PENDING_FREEZE`, zero data contact); `docs/SECURITY_MODEL.md` +39 lines (32-frame CAL sacrificed and excluded from the key denominator; reveal ~18–22 bits diagnostic-only, never a λ_prior term; claim scope unchanged; M2 labelled CANDIDATE; 0-bit public-message inventory skeleton).

**Context**: Basis: user authorization pasted verbatim (Stage-1 implementation only; `execution_authorized: false`) + operator all-complete return (acceptance IDs S1-1..S1-8 all PASS) + independent focused review `REVIEW.md` verdict PASS_WITH_COMMENTS with zero blocking issues (acceptance `MAIN_THREAD_ACCEPTANCE.md`). Scope framing: decoder-free, synthetic fixtures only, zero protected reads, zero data contact; no Pre-EXECUTE/Pre-RESULT gates applied (none applicable — nothing claim-bearing executed). Two cosmetic non-blocking items carried (selfcheck fn rename; out-root forbidden-tree guard note) for a concurrent tiny code task; no docs action.

**Alternatives considered**:
- Treat Stage-1 completion as real-data validation of M2: rejected — S9 stays synthetic-only (16 blocks, pro-M2 ground truth by design) and is never FER/efficiency evidence; M2 remains a CANDIDATE pending real-data validation.
- Decide fixed-K vs fixed-f now or set any freeze value: rejected — the K choice stays DEFERRED; no freeze values set; all 19 TO-FREEZE keys remain null.
- Authorize G1/G2/G3 in this entry: rejected — G1/G2/G3 remain unauthorized; G2 (Tier-Y one-shot decode at frozen K1=319/K2=6492 on SHG `_1` only) is a separate later freeze; SHG `_2` stays reserved for G3.

**Consequences**: `openspec/changes/nbpolar-prior-rebaseline/tasks.md` T2/T3/T4 checked (exact Stage-1 scope: adapter+tests, K re-split path, SECURITY_MODEL note); T1/T5–T8 untouched. Next gate: Stage-2 G1 freeze preparation (19 TO-FREEZE values; parent-STATUS recommendations: W_P=500 DATA-INFORMED, W_S=200, skip=702 INHERITED_NOT_DERIVED, MOD=LINEAR_ONLY, NCHAR≥200000, B_tail=2.0e-4, Δ_min=0.020 bits/symbol, CAL-split RULE-FIT32-SCORE-HELDOUT, g2_blocks=16, tag_master frozen at freeze time), gated on independent freeze review + explicit user authorization. No rerun/retuning; no commit/push by this entry.

**Cautions**: M2 is a CANDIDATE, not a baseline; S9 is synthetic-only and never FER/efficiency evidence. Descriptive only — no FER, no qualification, no promotion, no claim.

## 2026-09-21: M2-prior G1 real-data NLL adjudicated BOUNDED_NEGATIVE (δ-tail FAIL stops the M2 route on SHG `_1`; NLL PASS near-uninformative by design; T5 complete; T6/T7 blocked)

**Decision**: Record packet `NBPOLAR-M2-PRIOR-G1-REALDATA-NLL` as `NBPOLAR_M2_PRIOR_G1_COMPLETE_ADJUDICATED_BOUNDED_NEGATIVE` (record `.workbuddy/queue/NBPOLAR-M2-PRIOR-G1-REALDATA-NLL/G1_ADJUDICATION.md`). G1 executed exactly once per freeze on SHG `_1`, DESCRIPTIVE / NON-CLAIM and DECODER-FREE; the preregistered STOP applies — NO G2. OpenSpec `nbpolar-prior-rebaseline` T5 is complete; T6/T7 (G2) are BLOCKED pending a main-thread route decision; G3 (SHG `_2`) is void unless a new candidate emerges. M2 remains CANDIDATE, not validated.

**Context**: Basis: verbatim user authorization (kai, 20260921; Phase 0/A/B scoped, decoder-free) + operator all-complete return (G1-0, G1-1..G1-4, remainder check) + independent closure verification `CLOSURE_PASS_WITH_COMMENTS` (Phase B unblocked) + independent focused numerical review `G1_NUMERICS_PASS` (every recomputed number agrees; gate rules applied exactly per freeze §6; no operator discretion in the FAIL). No rerun, no tuning, no seed/window/K change; decoder never called; SHG `_2` never read. Regime: SHG `_1` = `20260113_SHG_Type2PPLN_3s`, (N) W_P=500, MOD LINEAR_ONLY, skip-702; CHAR 200,192 pairs; CAL32 = post-skip frames 1024–1055 sacrificed; HELDOUT = 1838–2397, 143,360 symbols. Reproduction gate bit-exact vs the Phase-A closure (peak 50 / σ 112.45189572400645 / ok / n_pairs 1,269,268 / n_frames 4,958 pre-skip; post-skip ledger 4,256). **δ-tail gate FAIL**: p̂ = 1005/200,192 = 0.0050202; one-sided 95% upper U = 0.0052879 (Clopper-Pearson) vs B_tail = 2.0e-4 — ~25× over budget; the ±1 parametric premise does not hold on this source. Linear profile {0: 149852, +1: 48639, −1: 696, tail: 1005}. **NLL gate PASS (near-uninformative by design)**: Δ = 3.043859 − 1.093192 = 1.950667 > Δ_min 0.020, matched 32-frame CAL, FIT32, seed N/A — with the binding caveat that matched-CAL M0 is NOT the incumbent (its as-deployed regime is 1024 frames, measurable only by G2 arm A1, now blocked); this NLL never promotes M2. CAL triple q0/q+1/q−1/q_rest = 0.746704/0.243652/0.004395/0.005249; H_M0 = 0.730680 / H_M2 = 0.841180. Remainder check (frozen sessions, NLL/H only, no gate): pooled Δ = 1.957900; per-segment 1.978823 / 2.024941 / 1.791009; the matched-CAL M0 sparse-table pathology replicates on both populations; frozen-session 32-frame CAL q_rest = 0.0 (tail ≈ 0) vs SHG `_1` 0.50%.

**Alternatives considered**:
- Proceed to G2 despite the δ-tail FAIL: rejected — preregistered STOP; the ±1 premise fails on this source at the frozen window.
- Read the NLL PASS as M2 validation or promotion: rejected — near-uninformative by design; matched-CAL M0 is not the incumbent; M2 stays CANDIDATE.
- Treat the bounded negative as refuting the M0 floor-mishandling MECHANISM on frozen sessions: rejected — the matched-CAL gap ~1.95 replicates on both populations; the sparse-table pathology is real and reproducible.
- Treat the bounded negative as refuting M2 on every source: rejected — frozen sessions show tail ≈ 0, but that ladder is exhausted (101 frames; 69 after CAL).
- License a new route (richer parametric δ model, SHG-tail investigation, M0-small-CAL finding) in this entry: rejected — each needs its own preregistration; none selected here.

**Consequences**: T5 complete; T6/T7 blocked; G3 void unless a new candidate emerges; next gate is main-thread route planning. No rerun/retuning; no commit/push by this entry. Diagnostic observations recorded, not adjudicated: the 0.50% CHAR tail is BELOW the uniform-accidental prediction from the census w=500 accidental estimate (1.44%) — not pure uniform accidentals; q−1 on SHG `_1` (0.0044) is ~2.5× the frozen-session value (0.0014).

**Cautions**: Descriptive only — no claim, no qualification, no promotion. Carried non-blocking items from the reviews: per-venv test record (timetagger venv G1 15/15 + Stage-1 uncollectable-pandas; sibling venv 36/36 incl. real pytest); freeze-config string glosses semantically identical; T12i edit in untracked Stage-1 test file (suggest committing Stage-1 files at next commit); evidence root lacks primary `.ttbin` identity pin (future freeze should pin it).

## 2026-09-21: S11 SHG tail-nature Tier-X probe adjudicated COMPLETE_DESCRIPTIVE (w=500 δ-tail is window-scaling pairing contamination, not channel structure; census far-offset baseline structured; no route selected)

**Decision**: Record Tier-X probe `NBPOLAR-S11-SHG-TAIL-NATURE` as `NBPOLAR_S11_SHG_TAIL_NATURE_COMPLETE_DESCRIPTIVE` (record `.workbuddy/queue/NBPOLAR-S11-SHG-TAIL-NATURE/S11_ADJUDICATION.md`). The probe is DESCRIPTIVE / NON-CLAIM and DECODER-FREE; it makes no prior-form decision, no window decision, and no route selection. Post-hoc window switching remains forbidden; the G1 bounded negative (recorded under the frozen w=500 contract) and M2's CANDIDATE status are untouched. OpenSpec `nbpolar-prior-rebaseline` checkboxes unchanged (S11 is a Tier-X probe outside the change's task list; T6/T7 remain unchecked/blocked).

**Context**: Question: on SHG `_1`, is the |δ|≥2 tail (0.50% at (N) w=500) an accidental-pairing artifact, an alignment artifact, or channel structure — and is the census far-offset accidental baseline itself uniform? Basis: verbatim user authorization (Kai, 2026-09-21) + operator all-complete return (S11-P..S11-R PASS) after one main-thread scoped packet correction (anchor = post-skip frames 1056–1837, the frozen SEG_RANGES / G1 CHAR segment, replacing the contradictory "first 782 post-skip frames" wording) + independent focused numerical review `S11_NUMERICS_PASS` (every recomputed number agrees to full precision; prereg 22:21:04 < body 22:23:02 < results 22:23:30; anchor cell-exact incl. sparse histogram cells). One run, 19.5 s / 1.24 GiB, no rerun. Probe artifacts gitignored under `workspace/probes/nbpolar_s11_shg_tail_nature/` (prereg.md / body.py / results.json). Regime: SHG `_1`, vendored (N) nearest-unique pairing, frozen constants, frozen derived offset +50 ps (σ=112.45189572400645), LINEAR_ONLY semantics. Results (all independently recomputed): (1) Anchor (w=500, skip-702, frames 1056–1837): tail 1005/200,192, p̂ = 0.0050202 — cell-exact reproduction of the G1 CHAR profile {0: 149852, +1: 48639, −1: 696, tail: 1005}; full-stream n_pairs 1,269,268. (2) W-grid (offset +50, no skip): p̂ = 0.0002389 (w=200, 301/1,259,992) / 0.0050155 (w=500, 6366/1,269,268) / 0.0168189 (w=1000, 21604/1,284,506) / 0.0394334 (w=2000, 51845/1,314,747); ratio p̂_2000/p̂_200 = 165.07; the w=200 LINEAR tail (301) is entirely wrap-alias cells (8+293) and the w=200 CIRCULAR tail = 0; wrap+ fixed at 8, wrap− 293→294 across windows. (3) Offset scan (w=500): p̂ = {50: 0.0050155, 25: 0.0049358, 75: 0.0052069, 0: 0.0048899, 100: 0.0054196, −50: 0.0051207}; minimum at offset 0 (not the derived +50); spread (max−min)/min = 10.83% — offset-insensitive, no alignment signature. (4) Far-offset baseline (w=500, 50±409600 ps): p̂ = 0.4067/0.3988 vs uniform 0.99707 and in-window 0.0050155 — the census far-offset baseline is STRUCTURED (≈0.41× uniform, ≈80× in-window), not a uniform-accidental reference; far n (18340/18224) matches the census far yields exactly. (5) Clustering (anchor, 782 frames): 578 frames with ≥1 tail vs Poisson-expected 565.70 (λ=1.28517); dispersion 0.94269 — independent-per-pair consistent. Tail vs census accidental estimates (0.0058/0.0144/0.0284/0.0555): per-point p̂/accidental ratios 0.041→0.348→0.592→0.711; OLS slope 0.803, intercept −0.0055 — steeper than proportional; "proportional" NOT supported (reviewer-corrected).

**Alternatives considered**:
- Read the w=500 δ-tail as a channel-structure refutation of the ±1 premise: not supported — the tail scales steeply with window (165×), is offset-insensitive, unclustered, and independent-per-pair consistent; at w=200 the CIRCULAR δ-mass ≤ 1 holds exactly on the full stream (non-wrap tail 0).
- Read the census far-offset accidental estimates as uniform-accidental levels: rejected — the far-offset tail ≈ 0.40 retains channel structure; mixture models must carry this caveat.
- Decide a window, a prior form, or a route in this entry: rejected — Tier-X descriptive only; post-hoc window switching stays forbidden; W_S=200's marginal bias (gauss cover 0.9247) stands as recorded.
- Switch the pairing contract W_P 500→200 on the strength of S11: rejected here — a narrow-window-centred validation is the leading candidate route INPUT for main-thread planning (not selected); it would need its own freeze (explicit pairing-contract change with bias caveat; MOD/tail-budget re-derivation — note at w=200 LINEAR p̂ = 0.0002389 still exceeds frozen B_tail = 2.0e-4 while CIRCULAR tail = 0, so gate semantics must be preregistered explicitly; gates re-preregistered; own authorization).

**Consequences**: No OpenSpec box checked; T6/T7 remain blocked; G3 void unless a new candidate emerges; next gate is main-thread route decision. Probe artifacts stay worktree-only/gitignored by design; durable record = the adjudication + this entry + the packet dir. No rerun/retuning; no commit/push by this entry.

**Cautions**: Tier-X descriptive only — no pass/fail on science, no route decision, no window decision. M2 stays CANDIDATE (never called a baseline here); the G1 bounded negative stands. No FER/efficiency language applies to this probe.

## 2026-09-22: M2-prior G1R2 narrow-window delta-successor COMPLETE_DESCRIPTIVE (w=200 CIRCULAR; δ-tail PASS is a consequence of S11, not new ±1 validation; CAL32 triple + held-out NLL is the new information; T6/T7 no longer blocked by G1; G3 still void)

**Decision**: Record delta-successor packet `NBPOLAR-M2-PRIOR-G1R2-W200-CIRCULAR` as `NBPOLAR_M2_PRIOR_G1R2_COMPLETE_DESCRIPTIVE` (record `.workbuddy/queue/NBPOLAR-M2-PRIOR-G1R2-W200-CIRCULAR/G1R2_ADJUDICATION.md`). G1R2 is the executed+adjudicated G1 contract with 8 CHANGED items (w=500→200, MOD LINEAR_ONLY→CIRCULAR; item 8 = runner re-parameterization). G1R2 executed exactly once, DESCRIPTIVE / NON-CLAIM and DECODER-FREE; no rerun, no tuning; decoder never called; SHG `_2` untouched; G1's record byte-identical. M2 remains CANDIDATE, never validated, never a baseline.

**Context**: Basis: verbatim user authorization (kai, 2026-09-22) + operator all-complete return (both phases) + independent delta review `DELTA_PASS_WITH_COMMENTS` (after one DELTA_FAIL cycle on the runner block, reworked) + independent closure verification `G1R2_CLOSURE_PASS_WITH_COMMENTS` + independent focused numerical review `G1R2_NUMERICS_PASS_WITH_COMMENTS` (all numbers independently recomputed). Regime: SHG `_1` (`20260113_SHG_Type2PPLN_3s`), (N) W_P=200, MOD CIRCULAR, skip-702, W_S=500 readout without switching. Reproduction 5/5 exact (peak 50 / σ 112.45189572400645 / ok / n_pairs 1,259,992 / pre-skip 4,921); post-skip ledger 4,219 (216 trailing dropped); reserve 4190–4218 (29), no id ≥4219. δ-profile on CHAR (200,192 pairs): linear {−1023: 44, −1: 450, **0: 150909**, +1: 48788, +1023: 1}, tail 45; circular {−1: 451, 0: 150909, +1: 48832}, **tail 0**; wrap accounting closes exactly (linear −1023 → circular +1; linear +1023 → circular −1); CHAR δ=0 fraction 75.382% vs full-stream 75.419% (representative subset). CAL32 (frames 1024–1055, 8,192 pairs, sacrificed): M2 circular triple **q0 0.7562255859375 / q+1 0.241943359375 / q−1 0.0018310546875 / q_rest 0**; matched M0 from the same 32 frames; HELDOUT 560 f = 143,360 symbols ⇒ NLL_M0 2.9623453 / NLL_M2 0.8231435. H_M2 = 0.0252536+0.7915602 = **0.8168138**; H_M0 = 0.0215025+0.6695565 = **0.6910589**. δ-tail **PASS** (n_tail 0, p̂ 0, U = 3/200192 = 1.4986e-5 < B_tail 2.0e-4). NLL **PASS** (Δ = 2.1392018 > 0.020). Remainder check (frozen sessions, no gate): CAL stream 0–31 (1.5M VAL 2172–2203); HELDOUT 69 f = 17,664 symbols; CAL circular triple q0/q+/q− = 0.7476806640625 / 0.251220703125 / 0.0010986328125, **q_rest 0**; H_M2 0.8251321, H_M0 0.7081275; pooled Δ = 1.9575437 = symbol-weighted mean of VAL-9f 1.9786419 (2304) / HOLD-42f 2.0244344 (10752) / 2M-18f 1.7909163 (4608).

**Adjudication** (five points, recorded not re-decided here):
- (1) The δ-tail PASS is a **consequence of S11** (CHAR ⊆ the w=200 circular-tail-0 full stream), NOT new validation of the ±1 premise — the new information is the CAL32 triple, the H values and the held-out NLL.
- (2) q_rest = 0 on BOTH populations at 32-frame CAL resolution is a weak statement by construction (zero-obs upper 3/8192 = 3.66e-4) — that is precisely why the large CHAR gate exists and why G1R2 does not replace it.
- (3) The M0 sparse-table pathology is confirmed again under the narrow-window contract (Δ 2.1392 SHG `_1`; remainder pooled 1.9575 ≈ G1's 1.9579); remainder H_M0 is bit-identical across contracts (0.7081275), consistent with M0 window-independence there (observation only).
- (4) **No cross-contract superiority claim** — G1 and G1R2 measure different (window, MOD) populations; G1's bounded negative stands under its own contract.
- (5) **M2 remains a CANDIDATE, not validated** — the premise blocker on the G2 path is lifted for this contract, but decoding success/FER/efficiency/reliability still require the G2 one-shot three-arm decode (Tier-Y, own freeze, Pre-EXECUTE + authorization).

**Alternatives considered**:
- Read the δ-tail PASS as ±1-premise validation or M2 promotion: rejected — consequence of S11 by construction; Tier-Y decode still required.
- Read q_rest = 0 (either population) as a tail bound: rejected — weak by construction at 8192 symbols; the CHAR gate carries the bound.
- Read the M0 Δ ≈ 2.14 as a cross-contract improvement over G1's Δ ≈ 1.95: rejected — different (window, MOD) populations; G1's record stands.
- Check an OpenSpec box for G1R2: rejected — G1R2 is a successor execution, not a task in the `nbpolar-prior-rebaseline` list; T6/T7 remain unchecked.
