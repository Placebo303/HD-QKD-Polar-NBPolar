## 2026-09-22 M2 G2 w=200/CIRCULAR Tier-Y three-arm decode SUCCESS (B 11/14 vs A2 0/14 strict non-overlap; M2 still CANDIDATE; G3 unlocked)

- Verdict [repo-observed]: label `NBPOLAR_M2_PRIOR_G2_SUCCESS` (record `.workbuddy/queue/NBPOLAR-M2-PRIOR-G2-DECODE/G2_ADJUDICATION.md`; full record in `docs/decision-log.md:27` + `docs/nbpolar/STATE.md` §1). Tier-Y one-shot three-arm decode on SHG `_1` (`20260113_SHG_Type2PPLN_3s`), contract w=200/CIRCULAR: B (M2 CANDIDATE, matched 32f CAL) 11/14 exact, Wilson 95% [0.5241027623, 0.9242875166]; A2 (M0 control, matched 32f) 0/14, Wilson [0.0, 0.2153170119]; strict non-overlap 0.5241 > 0.2153 = preregistered SUCCESS gate. A1 (M0 incumbent, own 1024f) 0/14 all `verify_failed` — descriptive, NO gate, first incumbent-regime measurement on this source (not an incumbent failure claim). Basis: verbatim user authorization + independent freeze review PASS (after three FAIL cycles) + Pre-EXECUTE PASS + one-shot execution + Pre-RESULT PASS (zero blocking). Counters `g2_runs 1` / `sc_calls_on_protected 126` / `tag_invocations 42` / `reruns 0`; `undetected` 0/42 isolated (never merged into success/FER); `nonfinite`/`decode_failed`/`resource_abort` 0; budget 855.1 s ≤ 900, RSS 1.12 GiB ≤ 2, `budget_aborted: false`.
- Contract [repo-observed, descriptive/non-claim]: frozen contract intact at K1=319/K2=6492, P16 orders len 32768 + pinned inner digest `055c9064…c3faea1b`, W_P=200/W_S=500/CIRCULAR/skip=702, EVAL 2398–4189 = 14 blocks, segments disjoint, alignment repro peak 50 / σ 112.45189572400645 / ok, ledger 4219 (216 dropped); disclosure recount mismatch 0 (pointers only — full key/public/tag_master literals stay in `G2_ADJUDICATION.md` §2 + `docs/decision-log.md:27`).
- Status/boundaries [decision]: M2 remains CANDIDATE until G3 passes — no FER-as-population estimate; no efficiency/qualification/promotion/composable-net-key claim; no cross-contract superiority vs G1 (w=500/LINEAR bounded negative stands unchanged); no claim that ±1 is "true" (q_rest=0 weak at 32f CAL, zero-obs bound 3/8192 = 3.66e-4, unchanged); no G3/SHG `_2` result. G3 (SHG `_2` independent-session confirmation) UNLOCKED; held authorization (kai 2026-09-22) operative; contract inherited; tag_master 2026110101; re-split still DEFERRED; milestone batch is main-thread work.
- Diagnostic [repo-observed, descriptive/non-claim]: oracle-vs-operational L2 domain trap carried forward (Pre-RESULT comment 1, gate unaffected): isolated oracle third SC call conditions on `views["u1"]` (polar-transformed) while frozen operational `operational_f13.run_operational_block` conditions L2 on `high_hat` (untransformed high, the domain `derive_p2` is built in) — visible as `oracle_l2_exact` 0/42 on ALL arms (even the 11 B-exact blocks) and non-discriminative `nll_l2_trueH` ~6.3 vs `nll_l2_candH` ~45–47, while the operational hard-L2 path discriminates (B 11/14 vs A1/A2 0/14). Gate uses only operational path + tag + label ⇒ SUCCESS arithmetic unaffected. For G3 and any future NLL reading, "H-conditioned L2" must state WHICH domain.
- Process [procedure]: three-FAIL freeze-review cycle before PASS is the durable lesson — the freeze caught blocking defects before authorization (B1–B5 then two residual one-line fixes per adjudication §1), same discipline as the G1R2 delta review catching four hardcodes; never authorize a frozen Tier-Y execution on a FAIL review. Tier-Y one-shot discipline held: single verdict-bearing run, no rerun/seed/threshold/K/window/MOD change.

## 2026-09-22 M2 G1R2 w=200/CIRCULAR delta-successor COMPLETE descriptive (δ-tail PASS is S11 consequence; M2 still CANDIDATE; G2 premise blocker lifted for this contract)

- Verdict [repo-observed]: label `NBPOLAR_M2_PRIOR_G1R2_COMPLETE_DESCRIPTIVE` (record `.workbuddy/queue/NBPOLAR-M2-PRIOR-G1R2-W200-CIRCULAR/G1R2_ADJUDICATION.md`; full record in `docs/decision-log.md`). Basis: verbatim user authorization + operator all-complete return + independent delta review PASS_WITH_COMMENTS (after one DELTA_FAIL cycle) + closure verification PASS_WITH_COMMENTS + focused numerical review PASS_WITH_COMMENTS. Executed once per contract; decoder-free; SHG `_2` untouched; G1's record byte-identical.
- δ-tail [repo-observed, descriptive/non-claim]: PASS (p̂ = 0, U = 3/200192 = 1.4986e-5 < B_tail 2.0e-4) is a CONSEQUENCE of S11, NOT new validation of the ±1 premise — CHAR (200,192 pairs) is a subset of the w=200 circular-tail-0 full stream. Genuinely new under this contract: CAL32 (frames 1024–1055) circular triple q0 0.7562255859375 / q+1 0.241943359375 / q−1 0.0018310546875 / q_rest 0; H_M2 = 0.8168138 / H_M0 = 0.6910589; held-out NLL (560 f: M0 2.9623453 vs M2 0.8231435, Δ = 2.1392018). Reproduction 5/5 exact; post-skip ledger 4,219; reserve 4190–4218 (29).
- Weak-q_rest + mechanism [decision]: q_rest = 0 on BOTH populations at 32-frame CAL resolution is weak by construction (zero-obs upper 3/8192 = 3.66e-4) — that is why the large CHAR gate exists and G1R2 does not replace it. M0 sparse-table pathology confirmed again under the narrow-window contract (SHG `_1` Δ 2.1392; remainder pooled Δ 1.9575437 ≈ G1's 1.9579001; remainder H_M0 bit-identical across contracts at 0.7081275 — observation only, window-independence not claimed).
- Status/route [decision]: M2 remains CANDIDATE, NOT validated. G1's bounded negative (w=500/LINEAR) stands under its own contract — no retroactive change, no cross-contract superiority claim (different window/MOD populations). G2 premise blocker lifted FOR THIS CONTRACT: T6/T7 no longer blocked by G1; G3 (SHG `_2`) still void unless G2 succeeds. G2 inputs (planning input, NOT decision): H_M2 = 0.8168138 ⇒ D4 budget literal K_total ≈ 6,946 at N=32768 vs frozen 7,020 — fixed-f vs fixed-K still DEFERRED; G2 decodes at frozen K1=319/K2=6492; any re-split from frozen selector only. Truncation caveat: w=200 keeps a timing-truncated population (c0 identical across windows, jitter-heavy coincidences preferentially dropped) — must be stated in any rate/efficiency/leakage reading.
- Process [procedure]: delta-successor fast path caught a blocking defect BEFORE authorization — first delta asserted "runner already G1-capable, must NOT be modified"; independent review proved four hardcodes made w=200 unexecutable (REPRO_GATE w=500 literals; SEG_RESERVE would emit 37 non-existent frame ids; G1_PACKET_DIR would overwrite G1's adjudicated config; missing bootstrap config). Rework: window-keyed G1/G1R2 contract table + ledger-driven reserve + G1R2 routing, G1 path/record preserved. Lesson: a "no code change needed" claim in a frozen packet is itself a reviewable claim.

## 2026-09-21 S11 SHG tail-nature Tier-X probe COMPLETE descriptive (pairing-contamination; G1 bounded negative stands; M2 still CANDIDATE)

- Verdict [repo-observed]: label `NBPOLAR_S11_SHG_TAIL_NATURE_COMPLETE_DESCRIPTIVE` (record `.workbuddy/queue/NBPOLAR-S11-SHG-TAIL-NATURE/S11_ADJUDICATION.md`; full record in `docs/decision-log.md`). Basis: verbatim user authorization + operator all-complete return (S11-P..S11-R PASS) + independent focused numerical review PASS (all numbers recomputed to full precision). One run, 19.5 s / 1.24 GiB, no rerun; SHG `_1` (`20260113_SHG_Type2PPLN_3s`), vendored (N) nearest-unique pairing, LINEAR_ONLY, frozen offset +50 ps (σ=112.45189572400645); artifacts gitignored under `workspace/probes/nbpolar_s11_shg_tail_nature/`.
- Tail-nature [repo-observed, descriptive/non-claim]: the G1 δ-tail FAIL at w=500 is substantially a pairing-contamination effect, NOT a channel-structure refutation of the ±1 premise. SHG `_1` tail scales steeply with pairing window — p̂ = 0.0002389 (w=200, 301/1,259,992) / 0.0050155 (w=500, 6366/1,269,268) / 0.0168189 (w=1000) / 0.0394334 (w=2000), ratio 165.07 — offset-insensitive (w=500 scan min at offset 0, spread 10.83%), unclustered (dispersion 0.94269; 578/782 frames with ≥1 tail vs Poisson 565.70), independent-per-pair consistent. At w=200 CIRCULAR δ-mass ≤ 1 holds exactly on the full stream (non-wrap tail = 0; the 301 LINEAR tail is entirely wrap-alias cells 8+293). Anchor cell-exact vs G1 (1005/200,192; p̂ = 0.0050202; full-stream n_pairs 1,269,268).
- Baseline caveat [repo-observed]: census far-offset accidental baseline is STRUCTURED, not uniform — far-offset (50±409600 ps) tail ≈ 0.4067/0.3988 vs uniform 0.99707 and in-window 0.0050155 (≈0.41× uniform, ≈80× in-window); accidental estimates across census/G1 arithmetic cannot be read as uniform-accidental levels, mixture models must carry this caveat. Tail vs accidental estimate is steeper than proportional (per-point ratios 0.041→0.348→0.592→0.711; OLS slope 0.803, intercept −0.0055).
- Status/route [decision]: M2 remains CANDIDATE (not validated); G1 bounded negative stands under the frozen w=500 contract; no prior-form/window/route decision made; post-hoc window switching remains forbidden; W_S=200's marginal bias (gauss cover 0.9247) stands. Route input only (NOT selected): narrow-window-centred validation is the leading candidate route — would need its own freeze (pairing-contract change W_P 500→200 with bias caveat; MOD/tail-budget re-derivation — at w=200 LINEAR p̂ = 0.0002389 still exceeds frozen B_tail = 2.0e-4 while CIRCULAR tail = 0, so gate semantics must be preregistered; own authorization).
- Process [procedure]: probe packet anchor-wording contradiction ("first 782 post-skip frames" vs frozen SEG_RANGES 1056–1837) caught by operator BEFORE prereg (STOP, no compute), resolved by main-thread scoped packet correction — ambiguity-stop discipline worked as designed.

## 2026-09-21 M2 G1 real-data premise test BOUNDED NEGATIVE on SHG `_1` (M2 still CANDIDATE; G2 BLOCKED; no claim)

- Verdict [repo-observed]: label `NBPOLAR_M2_PRIOR_G1_COMPLETE_ADJUDICATED_BOUNDED_NEGATIVE` (record `.workbuddy/queue/NBPOLAR-M2-PRIOR-G1-REALDATA-NLL/G1_ADJUDICATION.md`; full record in `docs/decision-log.md`). Basis: verbatim user authorization + operator all-complete return + independent closure `PASS_WITH_COMMENTS` + independent focused numerical review PASS (all gate arithmetic recomputed). Executed once per freeze on SHG `_1` (`20260113_SHG_Type2PPLN_3s`), decoder-free, descriptive/non-claim; reproduction bit-exact vs Phase-A closure. **δ-tail gate FAIL**: p̂ = 0.0050202 (1005/200,192), U = 0.0052879 vs B_tail = 2.0e-4 (~25× over) at (N) W_P=500 / LINEAR_ONLY / skip-702 ⇒ ±1 premise does NOT hold on SHG `_1`; preregistered STOP ⇒ **NO G2**. NLL gate PASS but near-uninformative by design (matched 32-frame CAL; Δ = 1.950667 vs Δ_min 0.020; matched-CAL M0 is not the incumbent — 1024-frame arm A1 never ran; this NLL never promotes M2).
- Mechanism vs premise [decision]: M0 sparse-table pathology REPLICATES on both populations (matched-CAL NLL gap ~1.95 on SHG `_1` and frozen-session remainder: pooled 1.957900, per-segment 1.978823/2.024941/1.791009) — the floor-mishandling MECHANISM is not refuted; what failed is the M2 ±1 FIX's premise on new data. Frozen 32-frame CAL q_rest = 0.0 (tail ≈ 0) vs SHG `_1` 0.50% — the ±1 premise is source-dependent. Bounded-negative framing is exact: NOT a refutation of the mechanism, NOT a refutation of M2 on all sources, NOT a route selection.
- Route [decision]: OpenSpec `nbpolar-prior-rebaseline` T5 complete; **T6/T7 (G2) BLOCKED** pending main-thread route decision; G3 (SHG `_2`, reserved, untouched) void unless a new candidate emerges; M2 remains CANDIDATE, NOT validated; no FER/efficiency claim anywhere. Candidate directions recorded, none selected: (i) richer parametric δ model with explicit tail mass; (ii) preregistered SHG-tail-nature investigation; (iii) unmeasured incumbent 1024-frame arm.
- Diagnostics [observation, recorded not adjudicated]: SHG 0.50% tail BELOW uniform-accidental prediction from census w=500 estimate (1.44%); q−1 on SHG `_1` ~2.5× frozen-session value.
- Carried non-blocking [procedure]: per-venv test record (timetagger venv 15/15 G1 + Stage-1 uncollectable from pre-existing pandas absence; sibling venv 36/36 incl. real pytest) per packet closure/operator records; evidence root lacks primary `.ttbin` identity pin (future freezes pin it; adjudication §3c); T12i edit lives in untracked Stage-1 test file — commit Stage-1 files at next opportunity.

## 2026-09-21 M2 Stage-1 implementation accepted (decoder-free surface EXISTS; M2 still CANDIDATE; G1/G2/G3 unauthorized)

- Delivered [repo-observed]: packet `.workbuddy/queue/NBPOLAR-M2-PRIOR-STAGE1-IMPLEMENTATION/` accepted, label `NBPOLAR_M2_PRIOR_STAGE1_IMPLEMENTATION_COMPLETE_ACCEPTED` (records `REVIEW.md` independent focused review PASS_WITH_COMMENTS zero blocking + `MAIN_THREAD_ACCEPTANCE.md` + `STATUS.yaml` all S1-1..S1-8 pass, `implementation_authorized: true`, `execution_authorized: false`). Additive-only surface: NEW `comparison_bench/src/comparison_bench/formal_ir/prior_m2.py` (strict ±1 adapter beside — never inside — `formal_ir/nbpolar/`; zero nbpolar imports; FLOOR=1e-15 literal; MOD LINEAR_ONLY default; M0 switch bit-exact `diff==0.0` to the P7 raw-MLE-plus-floor rule) + NEW `comparison_bench/tests/test_nbpolar_prior_m2.py` (T1–T12, 21 green) + `scripts/m2_prior_validation.py` skeleton → full `--freeze-config` CLI contract (all 19 TO-FREEZE keys enforced null/absent⇒exit 2, `--authorized` store_true, K pinned 319/6492, out-root confined to `workspace/`, stage bodies exit 3 STAGE_BODY_PENDING_FREEZE zero data contact) + `docs/SECURITY_MODEL.md` +39 lines (CAL/prior-accounting note + 0-bit inventory skeleton).
- Scope [decision]: OpenSpec `nbpolar-prior-rebaseline` Stage-1 implementation scope of T2/T3/T4 delivered; T1 (plan review) and T5–T8 (G1/G2 freeze, execute, adjudicate) remain open — checkbox flips owned elsewhere. Scientific status UNCHANGED: M2 is a CANDIDATE pending real-data validation; S9 stays synthetic-only and is never FER/efficiency evidence; no K decision (fixed-K vs fixed-f DEFERRED); G1/G2/G3 unauthorized. SHG `_2` reserved for G3 with participation disclosure (window choice DATA-INFORMED from it; never prior fitting/decoder/model selection) — full text in parent packet `.workbuddy/queue/NBPOLAR-M2-PRIOR-REALDATA-VALIDATION/STATUS.yaml` `reserved_for_G3`, decision detail in `docs/decision-log.md`.
- Next gate [procedure]: Stage-2 G1 freeze preparation (19 TO-FREEZE values; parent-packet recommendations: W_P=500, W_S=200, skip=702, MOD=LINEAR_ONLY, NCHAR≥200k, B_tail=2.0e-4, Δ_min=0.020, RULE-FIT32-SCORE-HELDOUT, g2_blocks=16, tag_master at freeze time) — gated on independent freeze review + explicit user authorization. Carried non-blocking: cosmetic selfcheck fn rename; out-root forbidden-tree guard note (concurrent tiny coder-fast task).
