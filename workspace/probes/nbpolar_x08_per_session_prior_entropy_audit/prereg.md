# X08 Tier-X prereg — per-session prior entropy audit (frozen before artifact open)

Probe id: nbpolar_x08_per_session_prior_entropy_audit. Tier X (non-claim, descriptive, decoder-free).
Packet: .workbuddy/queue/NBPOLAR-X08-PER-SESSION-PRIOR-ENTROPY-AUDIT/TASK_PACKET.md.
Authorization: .workbuddy/queue/NBPOLAR-X08-PER-SESSION-PRIOR-ENTROPY-AUDIT/AUTHORIZATION_PROMPT.md.
...
- npz path (sole artifact content open, read-only, no reopen):
  .workbuddy/queue/NBPOLAR-PHASE4-P20H-PER-SESSION-CALIBRATION/per_session_calibration/calibrated_prior.npz
  loaded once via np.load(allow_pickle=False).
- canonical digest pin: e8dd078a5367ba3af8eba91022d58aeffc114bc30b847b13cd4bb890e5dde43b
  computed via per_session_calibration.canonical_prior_digest (inline fallback uses
  exactly the documented recipe: sorted keys, key + shape + dtype + C-order bytes
  joined by single zero bytes, sha256).
- exact key set PRIOR_NPZ_KEYS: counts_ab, f_prior, p1, p2, p_b, lambda_star,
  floor_value, h1, h2, h_total.
- floor 1e-15 per cell + column renormalize; control lambda 137.3823795883264;
  raw rule lambda 0.0 (zero columns fall back to p_global).
- stored control literals: h1 2.006647056368773, h2 1.9017235959286112.
- N 32768; K1 levels 319/447/575/831 at 5 bits; frozen key-bits reference 34119;
  K_total formula floor((1.3*32768*H_total - 64)/5) clipped [0,65536].
- entropy functionals identical to P20H calibrate_array: H1 = sum_b p_b entropy_bits(p1);
  H2 = sum_b p_b sum_u1 p1 entropy_bits(p2).

## Exact command (run from the repo root)

  cd /mnt/d/Code/HD-QKD_Polar_Comparison-nbpolar && export OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 MALLOC_ARENA_MAX=2 && timeout 120 /mnt/d/Code/HD-QKD_Polar_Comparison/.venv/bin/python workspace/probes/nbpolar_x08_per_session_prior_entropy_audit/body.py

Interpreter pin: /mnt/d/Code/HD-QKD_Polar_Comparison/.venv/bin/python (if numpy is
missing there, the operator falls back to .venv/bin/python in this checkout or the
sibling venv and records which; bare python/python3 is never used).

## Write scope

Writes only: workspace/probes/nbpolar_x08_per_session_prior_entropy_audit/prereg.md,
body.py, results.json, plus the two packet docs above. No protected counts NPZ, no
parquet, no sibling-checkout artifact, no decoder/RNG/tag, no calibration refit, no
lambda beyond the control and 0, no accepted-module edits, no commit/push, no claims.
...
## Frozen analysis body (body.py must be byte-identical to this block)
