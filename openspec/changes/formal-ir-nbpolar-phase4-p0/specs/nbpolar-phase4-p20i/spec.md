# Delta spec — P20I L1-disclosure +128 single-factor on type2_1p5M_20260121_183806

## Prior reuse (zero new calibration)

P20I varies exactly ONE input against the frozen P20H point: L1
disclosure +ΔK1=128 via the frozen order-prefix rule. The prior is
...
P20H DEV 0..383, any 1M split, reserved 2M, Model-F CAL artifact). This
packet performs zero calibration opens (split-counted counts 0/0).

## Extension-step freeze

`C1_L1plus` SHALL disclose exactly ΔK1 = +128 L1 symbols beyond the
frozen base (K1 319 → 447; K_total 6811 → 6939; K2 stays 6492) by
FROZEN-ORDER-PREFIX EXTENSION: the disclosed L1 set is the first 447
...
data (P20I DEV, P20H DEV, closed 1M ranges, 2M). Positions are fixed by
the construction file and SHALL never be selected on closed blocks, the
consumed 1M pool, P20H DEV 0..383, the reserved 2M file, or the new DEV
data. The P19 `l1_plus` arm (K1 319→447, leakage 34759) is the minimal
empirical dose precedent for this quantum. The step SHALL be frozen
before execution and unchanged after; there SHALL be no tag-guided
selection, no evidence reuse, no post-hoc step-picking, and no second
tier (`C1b`) without a new packet. `C1_L1plus` SHALL invoke the same 2
SC calls and 1 tag per block as `C0_sc_base`.
...
384..767 / 768..1151) and the P20H DEV blocks (1.5M TRAIN 0..383) are
CONSUMED or CLOSED and SHALL NOT supply P20I blocks; the ENTIRE 1M
pool is fail-closed against P20I DEV selection; the reserved 2M
session file SHALL NOT be opened, statted, listed, or read under this
packet (pristine by non-access). Stage B SHALL run on the declared
population only: the NEXT 384 TRAIN frames of the 1.5M file in
(frame_id, pair_idx) order after the consumed P20H DEV prefix (frozen
TRAIN base 0: DEV frames 384..767 → DEV blocks 384..511 / 512..639 /
640..767 at N=32768; remainder 768..1659, 892 frames / 228352 pairs,
counted, never decoded). Exclusion is enforced by a fail-closed
quadruple gate in frozen order: (a) CROSS-FILE gate — the DEV
...
the run refuses before any protected content open; (d)
...
bits base operational (`5*(319+6492)+64`), 34759 bits `C1_L1plus`
(`5*(447+6492)+64`, delta exactly +640 = 5·128 vs base), 32524 bits
...
invoked: key 304206 = 3*34119 + 3*34759 + 3*32524 (operational 206634
+ oracle 97572); public 2949687 = 9*327743. The caps sit at
34119/327680 (~10.41%) and 34759/327680 (~10.61%) of raw input bits per
...
`resource_abort` via the P20A resource path. `C1` records SHALL carry
the frozen disclosure triple (`l1_delta_k1_applied`,
`l1_disclosure_rule`, `key_bit_delta_vs_base`).
`C2_true_l1_diagnostic` carries oracle provenance, is
...
qualification threshold. L1-restoration (`c1_restored_count`: C0 fail →
C1 exact on the new-segment blocks) and maintenance counts are
...
BLOCK, never succeed. Reads are split-counted continuing P20H mode:
...
itself. Tag domains are new P20I (master 2026092230, prefix
`nbpolar-p20i-l1-disclosure-1p5m-seed`).

## Scope

P20I SHALL use injected data and temporary roots in Stage A with zero
...
rules themselves), disclosure beyond the frozen +128 tier,
...
1M pool (any split, any subrange), P20H DEV 0..383 or the reserved 2M
file; calibrate on DEV or refit after any DEV contact; overwrite
