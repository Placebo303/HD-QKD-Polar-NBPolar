# Delta spec — P20J L1-dose-escalation +256 single-factor on type2_1p5M_20260121_183806

## Prior reuse (zero new calibration)

P20J varies exactly ONE input against the frozen P20H point: L1
disclosure +ΔK1=256 via the frozen order-prefix rule. The prior is
...
P20H DEV 0..383, P20I DEV 384..767, any 1M split, reserved 2M, Model-F
CAL artifact). This packet performs zero calibration opens
(split-counted counts 0/0).

## Extension-step freeze

`D1_L1plus` SHALL disclose exactly ΔK1 = +256 L1 symbols beyond the
frozen base (K1 319 → 575; K_total 6811 → 7067; K2 stays 6492) by
FROZEN-ORDER-PREFIX EXTENSION: the disclosed L1 set is the first 575
...
are fixed by the construction file and SHALL never be selected on
closed blocks, the consumed 1M pool, P20H DEV 0..383, P20I DEV
384..767, or the new DEV data. The step doubles the P20I +128 L1 dose
(K1 447 → 575); the P19 `l1_plus` arm (K1 319→447) is the minimal-dose
precedent of this lineage. The step SHALL be frozen before execution
and unchanged after; there SHALL be no tag-guided selection, no
evidence reuse, no post-hoc step-picking, and no second tier (`D1b`)
without a new packet. `D1_L1plus` SHALL invoke the same 2 SC calls
and 1 tag per block as `D0_sc_base`.
...
384..767 / 768..1151), the P20H DEV blocks (1.5M TRAIN 0..383) and the
P20I DEV blocks (1.5M TRAIN 384..767) are CONSUMED or CLOSED and SHALL
NOT supply P20J blocks; the ENTIRE 1M pool is fail-closed against P20J
DEV selection; the reserved 2M session file SHALL NOT be opened,
statted, listed, or read under this packet (pristine by non-access).
Stage B SHALL run on the declared population only: the NEXT 384 TRAIN
frames of the 1.5M file in (frame_id, pair_idx) order after the
consumed P20H DEV prefix and the consumed P20I DEV segment (frozen
TRAIN base 0: DEV frames 768..1151 → DEV blocks 768..895 / 896..1023 /
1024..1151 at N=32768; remainder 1152..1659, 508 frames / 130048 pairs,
counted, never decoded). Exclusion is enforced by a fail-closed
quintuple gate in frozen order: (a) CROSS-FILE gate — the DEV
content-open path plus size/sha pin must match the 1.5M pairs identity
(size 1869178 B, sha `ca351e52…a06b` provenance pin; source-tag +
digest pin is the identity, never frame numbers alone); any 1M path,
any 2M path, or any digest/size mismatch refuses before any SC call;
(b) INTRA-FILE gate — DEV ranges must overlap NONE of the 1.5M
VAL/HOLD frame sets (1660..2212 / 2213..2766, VAL checked first), else
the run refuses before any protected content open; (c) P20H-DEV
...
else the run refuses before any protected content open; (e)
...
bits base operational (`5*(319+6492)+64`), 35399 bits `D1_L1plus`
(`5*(575+6492)+64`, delta exactly +1280 = 5·256 vs base), 32524 bits
...
invoked: key 306126 = 3*34119 + 3*35399 + 3*32524 (operational 208554
+ oracle 97572); public 2949687 = 9*327743. The caps sit at
34119/327680 (~10.41%) and 35399/327680 (~10.80%) of raw input bits per
...
`resource_abort` via the P20A resource path. `D1` records SHALL carry
the frozen disclosure triple (`l1_delta_k1_applied`,
`l1_disclosure_rule`, `key_bit_delta_vs_base`).
`D2_true_l1_diagnostic` carries oracle provenance, is
...
qualification threshold. L1-restoration (`d1_restored_count`: D0 fail →
D1 exact on the new-segment blocks) and maintenance counts are
...
BLOCK, never succeed. Reads are split-counted continuing P20I mode:
...
itself. Tag domains are new P20J (master 2026092240, prefix
`nbpolar-p20j-l1-dose-escalation-1p5m-seed`).

## Scope

P20J SHALL use injected data and temporary roots in Stage A with zero
...
rules themselves), disclosure beyond the frozen +256 tier,
...
or the reserved 2M file; calibrate on DEV or refit after any DEV
contact; overwrite `results/` or
