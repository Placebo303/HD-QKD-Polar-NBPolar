# P20J freeze — L1-dose-escalation +256 single-factor on type2_1p5M_20260121_183806 (Stage A implementation)

Packet: `NBPOLAR-PHASE4-P20J-L1-DOSE-ESCALATION-1P5M` (`TASK_PACKET.md`, 486
lines, frozen). Predecessor: P20I terminal
...
instrumentation). Dose decision (frozen in the packet §2, not re-argued
here): +128 (P20I) moved first-error coordinates but restored nothing
— the +128 minimal dose is falsified as a fix while proving L1
sensitivity (coordinates move when L1 disclosure grows); +256 doubles
the P20I dose (K1 319→575, keyΔ +1280 = 5·256) before any larger jump,
preserving single-step dose-response resolution. Strategy parent:
...
the §4 quintuple gate) plus a separate pasted Stage-B authorization.

## 1. Mission (one frozen L1-dose-escalation question, zero further tuning)

> With ONE preregistered change against P20H/P20I — L1 disclosure
> +ΔK1=256 via the frozen order-prefix rule (Stage-A frozen, Stage-B
> read-only) — and everything else carried over from P20H
> (per-session prior digest-pinned read-only, K2 base 6492, floor
> 1e-15, N=32768, greedy SC only, new P20J tag domains only), zero
> further tuning, does +256 L1 restore any complete operational block
> on three NEW 1.5M TRAIN blocks (768..1151) — and where does the first
> error move when L1 disclosure grows while L2 disclosure stays at
> base?

P20H DEV 0..383 and P20I DEV 384..767 are CONSUMED (9+9 records, DEV
1/1 + attempt 1/1 SPENT each) and SHALL NOT supply P20J blocks. All
branches after this packet are deferred to §16 of the packet and must
not be prejudged here.
...
for any fitting: 1.5M DEV (768..1151 and remainder), 1.5M VAL/HOLD,
P20H DEV 0..383, P20I DEV 384..767, ANY 1M split, reserved 2M,
Model-F CAL artifact. No calibration fallback inside Stage B. This
packet performs zero calibration opens (split-counted counts 0/0).
...
P20H consumed 0..383; P20I consumed 384..767; P20J DEV is the NEXT 384
frames 768..1151; remainder 1152..1659 (508 frames / 130048 pairs);
VAL 1660..2212, HOLD 2213..2766. The runner fail-closes on any layout
drift (`dev_population_exact` requires exactly frames 768..1151 with
256 rows each; the quintuple gate §4 refuses first), so a wrong base
...
0..383) and the P20I DEV blocks (1.5M TRAIN 384..767) are
CONSUMED/CLOSED and SHALL NOT supply P20J blocks. The ENTIRE 1M pool
is fail-closed against P20J DEV selection by the cross-file gate.
...
Stage B runs on the DECLARED population (P20I-UNCONSUMED next
segment):
...
| DEV frame rule | NEXT 384 TRAIN frames after P20I in (frame_id, pair_idx) order → frozen TRAIN base 0: `768..1151` |
...
Fail-closed quintuple gate (frozen in the runner, order
(a)→(b)→(c)→(d)→(e), VAL before HOLD): (a) CROSS-FILE — DEV
...
(e) CALIBRATION-IDENTITY — Stage-B prior/table/literal digest must
...
Planned totals if all invoked: key 306126 = 3*34119 (102357) +
3*35399 (106197) + 3*32524 (97572) (operational 208554 + oracle
97572); public 2949687 = 9*327743. D1 key-bit delta vs base exactly
+1280 = 5*256.

Meaningfulness bar: base 34119/327680 = 0.10412292 (~10.41%) and D1
35399/327680 = 0.10802917 (~10.80%) of raw input bits (10*N per block
...
## 6. Arms (frozen; prior + L1 step swapped per packet §§2-3, all else P20H/P20I-identical)

Run block-major (D0, D1, D2) per DEV block; checkpoint after every
(arm, block) record; 9 records total.

- `D0_sc_base`: frozen greedy SC at base disclosure/construction
  (operational; accepted `run_operational_block` called directly with
  k1=319/k2=6492; 2 SC + 1 P20J-domain tag per block).
- `D1_L1plus`: frozen greedy SC at base + the frozen +256 L1 step via
  §2 order-prefix extension, all else identical (operational; the
  single-factor candidate; accepted `run_operational_block` called
  directly with k1=575/k2=6492 as the first-575 prefix of the SAME
...
- `D2_true_l1_diagnostic`: true-L1-conditioned L2 at BASE disclosure
  ...
  and a P20J-domain tag closure; provenance ORACLE, deployable=false,
  1 SC + 1 tag per block; excluded from every operational aggregate;
  never described as a correction result).

No other arms. Tag domain: master 2026092240, prefix
...
P20G, P20H and P20I domains; raw seed bits are never persisted.

## 7. Thresholds and labels (descriptive only)

Outcome label
`TARGET_EMPIRICAL_N32768_DEV_L1_DOSE_ESCALATION_1P5M_COMPLETE` iff ALL
21 integrity gates hold — regardless of exact-count. Any exact count
...
only: "L1-restoration" = blocks where D0 fails and D1 is exact on the
new-segment blocks (`d1_restored_count` analogue, count + per-block
endpoint rows); "maintain" = D1 exact where D0 exact, reported the
same way. Either pattern (or its absence) counts as COMPLETE; the
D1-restoration / D1-zero-restoration / maintain-only branch decision
...
taxonomy, plus D1 disclosure fields (`l1_delta_k1_applied` 256/0,
`l1_disclosure_rule` frozen-order-prefix-extension/base,
`key_bit_delta_vs_base` 1280/0).
...
- Single attempt: `attempts_allowed: 1`, `attempts_used: 0` until
  ...
  the identical freeze, never as tuning. P20H's and P20I's spent
  attempts do NOT transfer (independent packet).
- Reads (split-counted, continuing P20I mode): counts-calibration
...
- Wall/RSS ceilings: 600 s external timeout, 2 GiB virtual limit
  ...
  / ~533 MB; P20J plans the identical 15 SC / 9-tag decoder budget,
...
- Strategy stop rules restated as binding: no tuning on closed blocks;
  ...
  DEV 0..383, P20I DEV 384..767, or the reserved 2M file; no third
  factor after an inconclusive single-factor result; no near-raw
...
- Five files: `frozen_plan.json`,
  ...
  loss, true-H vs candidate-H L2 NLL, taxonomy, D1 disclosure triple
...
- Accounting: key/public/tag disclosure + independent recount
  ...
  `d1_restored_count` analogue recomputed descriptively (D0 fail→D1
...
- Integrity gates in frozen order (all true or BLOCKED):
  ...
  arm in block-major slot order + never-used remainder 508 frames /
  130048 pairs, counted from the pool but never decoded);
  ...
  + D1 disclosure triple); `oracle_isolation`;
...
/mnt/d/Code/HD-QKD_Polar_Comparison/.venv/bin/python -m pytest \
  ...
  -q -p no:cacheprovider --basetemp=$PWD/workspace/p20j/<uuid>/new-suites
```
→ 36/36 green (injected/synthetic inputs + stub loaders, fresh
additive temp root; initial run 34/36 — two test-side assertion
typos, runner untouched — fixed and rerun 36/36 on a fresh basetemp).
Packet-scoped predecessors (same settings, fresh basetemps
`workspace/p20j/<uuid>/pred-A|pred-B|pred-C`):
...
`test_nbpolar_holdout_backoff_diagnostic.py` (33) → 378/378 green
(chunk A 197 in ~121 s; chunk B 68 in ~41 s; chunk C 113 in ~839 s;
...
Stage B); three hardcoded arms with the +256 L1 prefix-extension
pinned (never CLI-tunable); `--source` vocabulary frozen to `1p5M`
only; `--dev-frames 768 1151` / `--remainder-frames 1152 1659`
...
- Output root absent: the Stage-B root `l1_dose_escalation_1p5m/`
  does not exist. The queue dir holds only `TASK_PACKET.md`,
  `STATUS.yaml`, `PROMPT.md`, `AUTHORIZATION_PROMPT.md`,
  `P20J_FREEZE.md` (this file), `P20J_IMPLEMENTATION_NOTES.md`.
...
- P16 digest recomputed in this session: `055c90...c3faea1b`, equal
  ...
  untouched.
- Fresh P20J tag master 2026092240 and focused-test seeds
  2026092241..2026092247 appear in NO tracked file outside the P20J
  runner/tests/packet/spec documents (`git grep` verified: zero
  tracked hits before the tasks.md append; after the append the only
  tracked holder is the P20J `tasks.md` section itself, same bar as
  P20I) and in NO other text file outside the P20J
  ...
  seeds) — same bar as P20I.
- Focused suite: `test_nbpolar_l1_dose_escalation_1p5m.py` 36/36
  green (fresh temp basetemp, pinned interpreter, `-p
  no:cacheprovider`).
- Packet-scoped predecessor suites 378/378 green (same settings,
...
remainder/1M consumed/closed frames/P20H DEV 0..383/P20I DEV 384..767
or the reserved 2M file, a second L1 tier (`D1b`), a second L2 step,
an alternative-construction second factor, per-DEV prior refit,
production benchmark, `results/` or
`comparison_bench/outputs_comparison/` writes, P12-P20I code or
old-root modification, official prior/probe seed reuse, commit/push.
Scope is the frozen +256 L1 order-prefix step + implementation +
frozen planning artifacts for a descriptive L1-dose-escalation
...
evidence, and D2 is never operational.
