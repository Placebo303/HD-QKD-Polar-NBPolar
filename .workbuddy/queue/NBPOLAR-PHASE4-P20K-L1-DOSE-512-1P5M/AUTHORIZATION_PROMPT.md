...
`NBPOLAR-PHASE4-P20K-L1-DOSE-512-1P5M` exactly as frozen in its
...
L1-dose-512 runner (three hardcoded arms `E0_sc_base` / `E1_L1plus512` with the frozen +512
L1 order-prefix step / `E2_true_l1_diagnostic`, loading the frozen prior read-only behind
a calibration-identity digest gate, with cross-file source-tag+digest, intra-file
DEV-vs-VAL/HOLD, P20H-DEV-exclusion, P20I-DEV-exclusion, P20J-DEV-exclusion, and calibration-identity fail-closed gates) plus
focused injected tests under a fresh additive `workspace/p20k/<uuid>/` temp root; the P20K
OpenSpec delta at `openspec/changes/formal-ir-nbpolar-phase4-p0/specs/nbpolar-phase4-p20k/spec.md`
plus the `tasks.md` P20K section; the Stage-A freeze (`P20K_FREEZE.md`) + implementation notes.
...
consumed-1M-pool / P20H-DEV / P20I-DEV / P20J-DEV / reserved-2M tuning or peeking, NO second L1 tier or L2 step,
...
explicitly adjudicating the §3 reuse pins and the §4 sextuple gate), a separate pasted
