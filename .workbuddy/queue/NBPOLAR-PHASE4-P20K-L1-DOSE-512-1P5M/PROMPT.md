...
`.workbuddy/queue/NBPOLAR-PHASE4-P20K-L1-DOSE-512-1P5M/TASK_PACKET.md`
(P20K L1-dose-512 +512 single-factor on type2_1p5M_20260121_183806, Tier-Y planning-frozen).
...
L1-dose-512 runner reusing P18 loading/block formation and P16 operational helpers
unchanged, three hardcoded arms carried over per §§5-6 (`E0_sc_base` base K1=319/K2=6492 /
`E1_L1plus512` with the FROZEN +512 L1 order-prefix-extension step K1=831/K2=6492 /
`E2_true_l1_diagnostic` oracle at base) loading the §3 frozen prior read-only behind a
...
`1152..1535` / `1152..1279/1280..1407/1408..1535` / `1536..1659` or replace with a written equivalent
+ justification; JSON-manifest provenance reads only for the population part;
SEXTUPLE gate: CROSS-FILE source-tag+digest first, then INTRA-FILE DEV-vs-VAL/HOLD
(VAL first), then P20H-DEV 0..383 exclusion, then P20I-DEV 384..767 exclusion, then
P20J-DEV 768..1151 exclusion, then CALIBRATION-IDENTITY), the preregistered per-arm disclosure caps (34119 / 36679 / 32524 key bits + 327743 public, keyΔ 2560 = 5·512,
ratios-vs-raw ~10.41%/~11.19%), the exact Stage-B command (new P20K tag master 2026092250,
...
`workspace/p20k/<uuid>/` temp root, `pytest -p no:cacheprovider`); (5) write the Stage-A
freeze (`P20K_FREEZE.md`) + implementation notes; (6) stage the P20K OpenSpec delta at
`openspec/changes/formal-ir-nbpolar-phase4-p0/specs/nbpolar-phase4-p20k/spec.md` plus
the `tasks.md` P20K section (no seed literals in tracked files; tag master 2026092250
and test seeds 2026092251..2026092257 verified absent elsewhere by repo grep).
...
384..767, P20J DEV 768..1151, or 2M; no prior refit/resmoothing/relambda, no calibration on DEV, no NPZ loader inside the
Stage-B runner; no second L1 tier (`E1b`), no second L2 step, no alternative-construction
...
Report acceptance IDs P20K-R1..R8 (stage-appropriately), changed files, exact test
...
§4 sextuple gate) plus a separate pasted Stage-B authorization.
