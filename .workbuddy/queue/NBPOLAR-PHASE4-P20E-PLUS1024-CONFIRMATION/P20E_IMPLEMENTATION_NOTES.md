# P20E implementation notes (Stage A operator record)

## 1. Files added (no shared file modified)

| file | purpose |
|---|---|
| `comparison_bench/src/comparison_bench/formal_ir/nbpolar/plus1024_confirmation.py` | thin three-arm confirmation runner (new; ~2440 lines incl. docstrings/gates/CLI, derived from the P20C runner) |
| `comparison_bench/tests/test_nbpolar_plus1024_confirmation.py` | 36 focused injected tests (new) |
| `openspec/changes/formal-ir-nbpolar-phase4-p0/specs/nbpolar-phase4-p20e/spec.md` | P20E OpenSpec delta (new) |
| `openspec/changes/formal-ir-nbpolar-phase4-p0/tasks.md` (P20E section) | P20E task section appended to the umbrella change (edit; no seed literals in the tracked file, same bar as P20C) |
| `.workbuddy/queue/NBPOLAR-PHASE4-P20E-PLUS1024-CONFIRMATION/P20E_FREEZE.md` | freeze (new) |
| `.workbuddy/queue/NBPOLAR-PHASE4-P20E-PLUS1024-CONFIRMATION/P20E_IMPLEMENTATION_NOTES.md` | this file (new) |

`git status` shows no modification to any file outside this manifest.
In particular: no change to `operational_f13.py`,
`operational_f13_replication.py`, `holdout_microcheck.py`,
...
- `run_oracle_control_block` + `OracleControlResult` (B2) — `is
  holdout_backoff_diagnostic` objects; P20E tag domain injected through
  the documented `tag_fn` closure (same trick P19/P20B/P20C used).
...
- Zero-tuning reference: the P20C module `l2_disclosure_backoff` is
  imported read-only by ONE test (`test_zero_tuning_carry_over_from_p20c`)
...
- Banned from the new module (asserted absent): sampling/RNG, genie,
  ...
  argmin selection), plus the P20C module/token names themselves
  (`l2_disclosure_backoff`, `L2DisclosureBackoff`).
...
- `form_dev_blocks`: the accepted `form_holdout_blocks` is HOLD-range
  ...
  line-for-line under P20E ranges. Proven by test: calling the accepted
...
- `verify_dev_manifest`: accepted manifest check plus the TRAIN
  1200/307200 pool requirement from the same manifest (new
  `FROZEN_MANIFEST_TRAIN_*` constants — the manifest pool is unchanged,
  only the DEV subrange is new).
- `verify_dev_block_identity`: TRIPLE fail-closed overlap gate —
  consumed P20C DEV 0..383 checked first, then consumed VAL 1200..1599,
  then closed HOLD 1600..1983 — plus the P20E literal pins. Ordering is
  pinned by tests (a range touching two pools reports the earlier pool).
  `form_dev_blocks` repeats the triple overlap checks on the computed
  ranges.
- Declared-remainder adaptation (the one structural delta from P20C):
  P20C's remainder sat INSIDE its DEV selection, so one closure check
  (`dev_last == rem_last`) and one filtered measurement covered it. P20E's
  remainder (768..1199) lies OUTSIDE the frozen DEV selection (384..767)
  by design, so (i) the closure check pins `remainder starts where the
  three DEV blocks end` (the TRAIN-tail end is already literal-pinned at
  the top of formation), and (ii) remainder presence/span (432 frames
  768..1199, 110592 rows) is measured from the UNFILTERED pool before DEV
  slicing — counted, never decoded. Remainder per-frame/per-row shape is
  deliberately not re-validated (unused region; presence + span +
  row-count is the proportionate check). Covered by
...
- `candidate_nll_bits`: retained as a pure diagnostic scorer (feeds the
  ...
  deleted.
- Gates mirror the accepted P19/P20B/P20C gate semantics field-for-field
  ...
  resource checks) with P20E names/counts (9 records, 15 SC, 9 tags,
  block-major slot order, new TRAIN-subrange population, per-arm K pins +
  B1 disclosure triple, triple overlap gate).
...
- carry-over: ΔK2 = +1024 (`FROZEN_L2_DELTA_K2`), order-prefix extension
  ...
  P20C (pinned literally against the P20C module); no tag-guided
...
- population: same-file TRAIN subrange 384..767, blocks
  384..511/512..639/640..767, remainder 768..1199 (432 frames /
  110592 pairs) never used (counted, never decoded), consumed P20C DEV
  0..383 + consumed VAL 1200..1599 + closed HOLD 1600..1983 disjoint by
  triple fail-closed gate (P20C first); same file/digest identity as
  P18/P19/P20B/P20C (no new digest).
...
- command: §10 of `P20E_FREEZE.md` verbatim (16 required flags,
...
comparison_bench/tests/test_nbpolar_plus1024_confirmation.py -q
-p no:cacheprovider --basetemp=$PWD/workspace/p20e/096c7b10-ebb2-433b-b532-4ca03fb8e3ce`
→ **36/36 green** (~26 s).

Packet-scoped predecessors (same settings, fresh basetemp
`workspace/p20e/bc799607-9e3b-459e-847c-2baaa9f11418`):
...
`test_nbpolar_holdout_backoff_diagnostic.py` (33) → **181/181 green**
(~905 s; the predecessor suites include real-N=32768 SC runs).
...
fixed, none touching shared code): P20C closure check
(`dev_last == rem_last`) and filtered remainder measurement do not carry
over when the remainder sits outside the DEV selection — fixed by the §3
declared-remainder adaptation (caught by the slicing test before any
other suite ran); two triple-gate test ranges touched two pools at once
and correctly reported the earlier pool — fixed by using single-pool
ranges (runner order confirmed, not changed); test seeds/master
re-pointed to the P20E 2026092101..2107 / 2026092100 domain with the P20C
master added to the never-use set.
...
- No NPZ/parquet content open or stat in this stage. Only JSON
  ...
  freeze) — per P18/P19/P20B/P20C Stage-A precedent, not TRAIN/HOLD
  content reads.
- Real-mode tests used throwaway stub files with patched loaders only;
  guard globals were restored afterwards.
- Stage-B root `plus1024_confirmation/` does not exist.
...
- Manifest JSON gives pool counts, not frame numbering; TRAIN 0..1199
  ...
  1600..1999 ranges within the 2000-frame file, and the P20E DEV subrange
  384..767 is the packet-preregistered enablement of P20C's never-used
  remainder. The runner fail-closes on any layout drift
  (`dev_population_exact` + triple overlap gate), so a wrong inference
...
- For B2, candidate-H NLL equals true-H NLL by construction (truth
  ...
  independent evidence (same convention as P19/P20B/P20C).
