# P20F freeze — N=32768 +1024 extension on the last same-file TRAIN tranche (Stage A implementation)

Packet: `NBPOLAR-PHASE4-P20F-PLUS1024-EXTENSION` (`TASK_PACKET.md`, 357
lines, frozen). Predecessor: P20E
...
## 1. Mission (one frozen extension question, zero tuning)

> With construction / disclosure / order / prior / floor / kernel /
> representation / SC all carried over byte-identical from P20C/P20E
> (K1=319, K2 base 6492 / B1 7516 via frozen order-prefix extension,
> floor 1e-15, N=32768, new P20F tag domains only), zero tuning, does
> +1024 maintain exact on the remaining new blocks — and does a
> P20C-style restoration event (B0 fail → B1 exact) get a second
> opportunity to replicate, or not?

P20C's TRAIN 0..383 signal was descriptive development evidence;
P20E's TRAIN 384..767 confirmation held maintain-exact (B0 3/3, B1 3/3,
B2 3/3; operational 6/6) with the restoration-event denominator at 0 —
neither replicated nor falsified. This packet asks only whether the
maintain pattern holds on blocks the candidate has still never seen,
and whether any restoration opportunity appears. Positive
(restoration replicates / maintain holds) directs a later
independent-session round; genuine negative (B0 fails unrestored by B1
despite opportunities) returns to the P20D alternative-construction
candidate. All branches are deferred and must not be prejudged here.
...
- Carried-over +1024 step: `B1` discloses ΔK2 = +1024 L2 symbols beyond
  ...
  pool, the consumed P20C/P20E DEV blocks, or the new DEV data. No second
...
- Verification: one final 64-bit Toeplitz tag per block/record under new
  P20F tag domains (§6); verification never selects a candidate.

VARIED: NOTHING. The only deliberate differences from P20C/P20E are the
...
elimination. The P20F DEV subrange 768..1151 sits inside TRAIN; the
runner additionally fail-closes (`dev_population_exact` requires
exactly frames 768..1151 with 256 rows each; the quadruple overlap gate
§4 refuses first), so any layout drift blocks before any SC call. Declared
never-used remainder frames 1152..1199 (48 frames, 12288 pairs) are
...
remainder 1584..1599) is CONSUMED and SHALL NOT supply P20F blocks.
Cross-packet same-pool tuning is forbidden.

The P20C DEV blocks (frames 0..383) and the P20E DEV blocks (frames
384..767) are CONSUMED and SHALL NOT supply P20F blocks. Cross-packet
same-block tuning is forbidden.

Stage B runs on the DECLARED new-block population (preregistered
enablement of P20E's never-used remainder):
...
| unused remainder | frames `1152..1199`, 48 frames / 12288 pairs, recorded never used (counted, never decoded) |
| P20C-consumed range | 0..383 — disjoint by fail-closed gate (checked FIRST) |
| P20E-consumed range | 384..767 — disjoint by fail-closed gate (checked SECOND) |
| consumed range | 1200..1599 — disjoint by fail-closed gate (checked third) |
| closed range | 1600..1983 — disjoint by fail-closed gate (checked fourth) |
...
touches DEV, P20C-consumed, P20E-consumed, closed, or consumed ranges. If
Pre-EXECUTE rejects the isolation, Stage A substitutes a declared
independent session (or other registered split) with the same 3-block
cost class — same packet, new freeze values, re-review.

## 5. Preregistered disclosure cap (normative, carried over)

Cap rule: caps are CARRIED OVER unchanged from P20C/P20E (no growth, no
...
- `B0_sc_base`: frozen greedy SC at base disclosure/construction
  ...
  k1=319/k2=6492; 2 SC + 1 P20F-domain tag per block).
...
- `B2_true_l1_diagnostic`: true-L1-conditioned L2 at BASE disclosure
  ...
  P20F-domain tag closure; provenance ORACLE, deployable=false, 1 SC +
  1 tag per block; excluded from every operational aggregate; never
  described as a correction result).

No other arms. Tag domain: master 2026092200, prefix
...
tokens differ from the P16, P17, P18, P19, P20B, P20C and P20E domains; raw
seed bits are never persisted.

## 7. Thresholds and labels (descriptive only)

Outcome label `TARGET_EMPIRICAL_N32768_DEV_PLUS1024_EXTENSION_COMPLETE`
...
- Wall/RSS ceilings: 600 s external timeout, 2 GiB virtual limit
  ...
  P20E observed ~100.1 s / ~554 MB at the identical budget; P20F plans
  the identical 15 SC / 9-tag budget, strictly inside the same class).
- Strategy stop rules restated as binding: no tuning on closed blocks; no
  reuse of the consumed VAL pool or the consumed P20C/P20E DEV blocks; no
...
- Integrity gates in frozen order (all true or BLOCKED):
  ...
  `dev_block_range_identity` (QUADRUPLE overlap pre-check: consumed P20C DEV
  0..383 first, then consumed P20E DEV 384..767, then consumed VAL 1200..1599,
  then closed HOLD 1600..1983); `target_population_contract` (7/7 preconditions);
  ...
  in block-major slot order + never-used remainder 48 frames / 12288
...
/mnt/d/Code/HD-QKD_Polar_Comparison/.venv/bin/python -m pytest \
  comparison_bench/tests/test_nbpolar_plus1024_extension.py \
  -q -p no:cacheprovider --basetemp=$PWD/workspace/p20f/<uuid>
```
...
- Output root absent: the queue dir holds only `TASK_PACKET.md`,
  `STATUS.yaml`, `PROMPT.md`, `AUTHORIZATION_PROMPT.md`, `P20F_FREEZE.md`,
  `P20F_IMPLEMENTATION_NOTES.md`. `plus1024_extension/` does not
  exist.
- Zero protected content opens: module one-open guards
  ...
  P18/P19/P20B/P20C/P20E Stage-A precedent).
...
- Manifest verified in this session: schema + 1M TRAIN 1200/307200 +
  ...
  elimination with accepted VAL/HOLD ranges; the P20F DEV subrange
  768..1151 sits inside TRAIN, fail-closed in the runner.
- Fresh focused-test seeds 2026092201..2207 and frozen tag master
  2026092200 appear in NO tracked file (`git grep` empty — same bar as
  P20C/P20E); untracked hits are exactly the new P20F runner, tests, spec
  delta, packet docs and this freeze (plus git-ignored `__pycache__`
  bytecode from the test runs).
- Focused suite `test_nbpolar_plus1024_extension.py`: 36/36 green
  (fresh temp basetemp, pinned interpreter, `-p no:cacheprovider`).
- Packet-scoped predecessor suites: `test_nbpolar_plus1024_confirmation.py`
  (36) + `test_nbpolar_l2_disclosure_backoff.py` (34) +
  ...
  `test_nbpolar_holdout_backoff_diagnostic.py` (113): green (same
...
writes, P12-P20E code or old-root modification, official prior/probe seed
reuse, commit/push. Scope is implementation + frozen planning artifacts
for a descriptive zero-tuning extension diagnostic on new blocks; it is
