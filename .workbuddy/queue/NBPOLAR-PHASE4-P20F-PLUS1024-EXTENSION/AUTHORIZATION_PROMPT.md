...
`NBPOLAR-PHASE4-P20F-PLUS1024-EXTENSION` exactly as frozen in its
`TASK_PACKET.md` (§§3-6, §12).

This authorization permits: read-only inventory of the packet-§12 predecessor helpers;
a new thin plus1024-extension runner (three hardcoded arms `B0_sc_base` /
`B1_L2plus` carried-over +1024 order-prefix step / `B2_true_l1_diagnostic`) plus
focused injected tests under a fresh additive `workspace/p20f/<uuid>/` temp root; the
P20F OpenSpec delta at
`openspec/changes/formal-ir-nbpolar-phase4-p0/specs/nbpolar-phase4-p20f/spec.md`
plus the `tasks.md` P20F section; `P20F_FREEZE.md` + `P20F_IMPLEMENTATION_NOTES.md`.
...
block / consumed-P20E-block tuning or peeking, NO efficiency tuning, NO SCL/new kernel/model/schema, NO
...
`P20F_FREEZE.md`, an independent Pre-EXECUTE review (PASS), a separate pasted Stage-B
