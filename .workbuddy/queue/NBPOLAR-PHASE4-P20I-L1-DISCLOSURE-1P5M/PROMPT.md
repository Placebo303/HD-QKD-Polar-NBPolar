...
`.workbuddy/queue/NBPOLAR-PHASE4-P20I-L1-DISCLOSURE-1P5M/TASK_PACKET.md`
(P20I L1-disclosure +128 single-factor on type2_1p5M_20260121_183806, Tier-Y planning-frozen).
...
L1-disclosure runner reusing P18 loading/block formation and P16 operational helpers
unchanged, three hardcoded arms carried over per §§5-6 (`C0_sc_base` base K1=319/K2=6492 /
`C1_L1plus` with the FROZEN +128 L1 order-prefix-extension step K1=447/K2=6492 /
`C2_true_l1_diagnostic` oracle at base) loading the §3 frozen prior read-only behind a
...
`384..767` / `384..511/512..639/640..767` / `768..1659` or replace with a written equivalent
+ justification; JSON-manifest provenance reads only for the population part;
QUADRUPLE gate: CROSS-FILE source-tag+digest first, then INTRA-FILE DEV-vs-VAL/HOLD
(VAL first), then P20H-DEV 0..383 exclusion, then CALIBRATION-IDENTITY), the preregistered
per-arm disclosure caps (34119 / 34759 / 32524 key bits + 327743 public, keyΔ 640 = 5·128,
ratios-vs-raw ~10.41%/~10.61%), the exact Stage-B command (new P20I tag master 2026092230,
...
`workspace/p20i/<uuid>/` temp root, `pytest -p no:cacheprovider`); (5) write the Stage-A
freeze (`P20I_FREEZE.md`) + implementation notes; (6) stage the P20I OpenSpec delta at
`openspec/changes/formal-ir-nbpolar-phase4-p0/specs/nbpolar-phase4-p20i/spec.md` plus
the `tasks.md` P20I section (no seed literals in tracked files; tag master 2026092230
and test seeds 2026092231..2026092237 verified absent elsewhere by repo grep).
...
success-point selection on closed blocks, the consumed 1M pool, P20H DEV 0..383, or 2M;
no prior refit/resmoothing/relambda, no calibration on DEV, no NPZ loader inside the
Stage-B runner; no second L1 tier (`C1b`), no second L2 step, no alternative-construction
...
Report acceptance IDs P20I-R1..R8 (stage-appropriately), changed files, exact test
...
§4 quadruple gate) plus a separate pasted Stage-B authorization.
